...
impl ConversationMemory {
	...
	pub fn context_for_prompt(&self) -> String {
		...
		for turn in &self.turns {
			...
			ctx.push_str(&format!(
				"- Turn {}: \"{}\" → {} ({})\n",
				turn.turn_id, turn.user_prompt, outcome,
	...
	fn summarize_old_turns(&mut self) {
		...
		let to_summarize: Vec<ConversationTurn> =
			self.turns.drain(..num_to_summarize).collect();
		...
		let prompt_summaries: Vec<String> = to_summarize
			.iter()
			.map(|t| t.user_prompt.clone())
			.collect();
...
impl RevisionHistory {
	...
	pub fn diff(&self, from_id: &str, to_id: &str) -> Option<RevisionDiff> {
		...
		let from_track_ids: std::collections::HashSet<&str> =
			from.project_data.tracks.iter().map(|t| t.id.as_str()).collect();
		let to_track_ids: std::collections::HashSet<&str> =
			to.project_data.tracks.iter().map(|t| t.id.as_str()).collect();
		...
		let from_clip_set: std::collections::HashSet<&str> =
			from_clips.keys().copied().collect();
		let to_clip_set: std::collections::HashSet<&str> =
			to_clips.keys().copied().collect();
		...
		let clip_count_change =
			to_clips.len() as i64 - from_clips.len() as i64;
...
mod tests {
	...
	fn test_slash_parser_edit() {
		...
		assert!(matches!(cmd.command, SlashCommand::Edit { clip_name_or_id } if clip_name_or_id == "intro_clip"));
	}
	...
	fn test_slash_parser_export_with_format() {
		...
		match cmd.command {
			SlashCommand::Export { format, bitrate_kbps } => {
