...
impl TimelineAudit {
	...
	fn detect_speed_ramp_opportunities(project: &ProjectData, _now_ms: u64) -> Vec<AuditFinding> {
		...
		let action_keywords = [
			"action", "fight", "chase", "explosion", "jump", "run",
			"race", "sport", "stunt", "drone", "fast",
		];

		for track in &project.tracks {
			for clip in &track.clips {
				let name_lower = clip.name.to_lowercase();
				let is_action = action_keywords
					.iter()
		...
		for track in &project.tracks {
			for clip in &track.clips {
				...
				if let Some(channel) = clip.animations.get("volume") {
					let all_frames: Vec<u32> = channel
						.keyframes()
						.iter()
						.map(|kf| kf.frame)
						.collect();
		...
		for track in &project.tracks {
			...
			for i in 1..track.clips.len() {
				...
				if gap < 5 && prev.clip_type == "video" && curr.clip_type == "video" {
					...
					let has_transition = prev.animations.iter().any(|(k, _)| {
						k.starts_with("transition_")
					}) || curr.animations.iter().any(|(k, _)| {
						k.starts_with("transition_")
					});
...
impl UserPreferenceStore {
	...
	fn default_path() -> PathBuf {
		...
		PathBuf::from(home).join(".lazynext").join("preferences.json")
	}

	/// Record that a suggestion was dismissed by the user.
	pub fn record_dismissal(&mut self, category: &str, suggestion_id: &str) {
		*self.dismissed_categories.entry(category.to_string()).or_insert(0) += 1;
	...
	pub fn record_acceptance(&mut self, category: &str) {
		*self.accepted_categories.entry(category.to_string()).or_insert(0) += 1;
	}
...
impl RepairStrategies {
	...
	pub fn repair_strategies(failed_action: &str, error: &str) -> Vec<String> {
		...
		if lower.contains("transcription") || lower.contains("transcribe") {
			return vec![
				"Try a different language model (switch from whisper-large to whisper-medium)".to_string(),
...
impl BackgroundAgent {
    ...
    pub async fn trigger_analysis(&self) -> Vec<AgentSuggestion> {
        ...
        match analyze_timeline(&self.nle, &self.client, &cfg).await {
            Ok(suggestions) => {
                let _ = self.event_tx.send(AgentEvent::SuggestionsReady(suggestions.clone()));
    ...
    pub async fn process_conversation_turn(
        ...
        if conv.proactive_mode {
            ...
            if let Some(follow_up) =
                ConversationContext::generate_follow_up(&last_action, project)
            {
    ...
    pub async fn run_full_audit(&self) -> TimelineAudit {
        ...
        for finding in &audit.findings {
            let _ = self.event_tx.send(AgentEvent::SuggestionsReady(vec![
                AgentSuggestion {
                    id: finding.id.clone(),
    ...
    pub async fn repair_and_retry(
        ...
        for strategy in &strategies {
            let mut conv = self.conversation.write().await;
            conv.add_agent_message(
                &format!("Attempting repair: {}", strategy),
                false,
            );
...
async fn analyze_timeline(
    ...
    suggestions.extend(detect_silence_issues(project, config.silence_threshold_secs, now_ms));
    suggestions.extend(detect_audio_leveling_issues(project, config.audio_level_tolerance_db, now_ms));
            ...
            if let Some(pe) = prev_end {
                ...
                if gap_secs >= threshold_secs {
                    suggestions.push(AgentSuggestion {
                        id: uuid::Uuid::new_v4().to_string(),
                        title: format!(
                            "{}s silence gap in track '{}'",
                            gap_secs as u32, track.id
                        ),
                        description: format!(
...
async fn execute_suggestion_impl(
    ...
    let success = match &suggestion.category {
        ...
        SuggestionCategory::PacingAdjustment => {
            suggestion.risk == RiskLevel::Low
        }
        SuggestionCategory::ClipHealth => {
            false
        }
        _ => false,
    };
...
mod tests {
    ...
    fn test_repair_strategies_transcription() {
        let strategies =
            RepairStrategies::repair_strategies("transcription", "model not found");
    ...
    fn test_repair_strategies_export() {
        let strategies =
            RepairStrategies::repair_strategies("export failed", "out of memory");
        assert!(!strategies.is_empty());
    ...
    fn test_repair_strategies_color_match() {
        let strategies =
            RepairStrategies::repair_strategies("color match", "no reference frame");
        assert!(!strategies.is_empty());
    ...
    fn test_repair_strategies_timeout() {
        let strategies =
            RepairStrategies::repair_strategies("unknown_op", "timed out after 30s");
        assert!(!strategies.is_empty());
    ...
    fn test_repair_strategies_generic_fallback() {
        let strategies =
            RepairStrategies::repair_strategies("completely_unknown", "weird error");
        assert!(!strategies.is_empty());
    ...
    fn test_conversation_context_follow_up_captions() {
        ...
        let follow_up =
            ConversationContext::generate_follow_up("Added captions", &project);
    ...
    fn test_conversation_context_follow_up_transitions() {
        let project = create_test_project();
        let follow_up =
            ConversationContext::generate_follow_up("Added transitions", &project);
        assert!(follow_up.is_some());
    ...
    fn test_conversation_context_follow_up_none() {
        let project = create_test_project();
        let follow_up =
            ConversationContext::generate_follow_up("Unknown action", &project);
    ...
    fn test_repair_strategies_load_fallback() {
        let strategies =
            RepairStrategies::repair_strategies("load media", "decode error");
        assert!(!strategies.is_empty());
