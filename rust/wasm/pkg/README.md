...
```bash
npm install lazynext-wasm
```

## Usage

```ts
import { formatTimecode, mediaTimeFromSeconds } from "lazynext-wasm";

const ticks = mediaTimeFromSeconds(1.5);
const label = formatTimecode({ ticks });
```

All exports are documented in the [TypeScript definitions](./lazynext_wasm.d.ts).
...
The web app depends on the published `lazynext-wasm` package by default. If you are editing the WASM source in this repo and want `apps/web` to use your local build instead:
