...
fn fragment_main(input: VertexOutput) -> @location(0) vec4f {
    ...
    if (uniforms.luma_key_threshold > 0.0) {
        ...
        var key_alpha = 1.0;
        if (luma < uniforms.luma_key_threshold - uniforms.luma_key_tolerance) {
            key_alpha = 0.0;
        } else if (luma < uniforms.luma_key_threshold) {
            key_alpha = (luma - (uniforms.luma_key_threshold - uniforms.luma_key_tolerance)) / max(0.0001, uniforms.luma_key_tolerance);
        }
        layer.a = layer.a * key_alpha;
    }
