...
export const DESKTOP_INFERENCE_CHANNELS = {
  capabilities: 'lazynext:inference-capabilities',
  setEnabled: 'lazynext:inference-set-enabled',
  preloadAsr: 'lazynext:inference-preload-asr',
  transcribe: 'lazynext:inference-transcribe',
  semantic: 'lazynext:inference-semantic',
  clap: 'lazynext:inference-clap',
  rhythm: 'lazynext:inference-rhythm',
  cancel: 'lazynext:inference-cancel',
  progress: 'lazynext:inference-progress',
} as const;
...
function validSourcePath(value: unknown): value is string {
  ...
  try {
    const parsed = new URL(value, 'http://lazynext.local');
    return parsed.origin === 'http://lazynext.local'
      && parsed.search === ''
      && parsed.hash === ''
