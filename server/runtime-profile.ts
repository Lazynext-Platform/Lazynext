...
export const DEV_PROFILE_ID_ENV = 'LAZYNEXT_DEV_PROFILE_ID';
export const DATA_DIR_ENV = 'LAZYNEXT_DATA_DIR';
const DEV_PROFILE_ENV_PREFIX = 'LAZYNEXT_DEV_PROFILE_';
...
export function resolveRuntimeProfile(
  ...
  if (profileId) {
    // Only an explicit LAZYNEXT_DATA_DIR reaches here (the pointer file is
    ...
    const rootDir = dataDir ?? join(home, '.lazynext', 'dev-profiles', profileId);
  ...
  const rootDir = dataDir ?? join(home, '.lazynext');
  const authOverride = env.LAZYNEXT_PROJECT_STORE_AUTH_DIR?.trim();
  const generationOverride = env.LAZYNEXT_GENERATION_JOB_STORE;
  const base = profileBase(
    rootDir,
...
export function defaultRootDir(
  ...
  return isIsolatedDevProfile(profile)
    ? join(home, '.lazynext', 'dev-profiles', profile.id)
    : join(home, '.lazynext');
}
