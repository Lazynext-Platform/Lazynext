...
const tempDir = await mkdtemp(join(tmpdir(), 'lazynext-mobile-upload-'));
...
try {
  ...
  assert.match(await page.text(), /Lazynext/);
  ...
  assert.match(await (await fetch(englishSession.urls[0]!)).text(), /Send media to Lazynext/);
