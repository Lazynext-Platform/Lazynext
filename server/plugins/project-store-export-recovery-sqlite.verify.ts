...
const CHILD_MODE = process.env.LAZYNEXT_EXPORT_RECOVERY_RACE_CHILD === '1';
...
function spawnChild(home: string): ChildProcess {
  return fork(fileURLToPath(import.meta.url), [], {
    env: {
      ...process.env,
      HOME: home,
      LAZYNEXT_EXPORT_RECOVERY_RACE_CHILD: '1',
    },
...
async function seedRecovery(home: string): Promise<void> {
  const path = join(home, '.lazynext', 'project-store-v1.sqlite3');
...
async function runParent(): Promise<void> {
  const home = await mkdtemp(join(tmpdir(), 'lazynext-export-recovery-race-'));
