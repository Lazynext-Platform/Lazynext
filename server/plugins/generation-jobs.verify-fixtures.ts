...
export async function setupGenerationJobsFixture(): Promise<GenerationJobsFixture> {
  const storeRoot = await mkdtemp(join(tmpdir(), 'lazynext-generation-jobs-'));
  const storePath = join(storeRoot, 'operations.json');
  const previousStorePath = process.env.LAZYNEXT_GENERATION_JOB_STORE;
  process.env.LAZYNEXT_GENERATION_JOB_STORE = storePath;
  ...
  const deleteExportName = `lazynext-export-job-${deleteAssetId}.mp4`;
  const expiryExportName = `lazynext-export-job-${expiryAssetId}.mp4`;
...
export async function cleanupGenerationJobsFixture(fixture: GenerationJobsFixture): Promise<void> {
  if (fixture.previousStorePath === undefined) delete process.env.LAZYNEXT_GENERATION_JOB_STORE;
  else process.env.LAZYNEXT_GENERATION_JOB_STORE = fixture.previousStorePath;
