...
const directory = await mkdtemp(join(tmpdir(), 'lazynext-extract-audio-'));
const previousProbe = process.env.LAZYNEXT_FFPROBE;
...
try {
  ...
  process.env.LAZYNEXT_FFPROBE = join(directory, 'missing-ffprobe');
  ...
  if (previousProbe === undefined) delete process.env.LAZYNEXT_FFPROBE;
  else process.env.LAZYNEXT_FFPROBE = previousProbe;
