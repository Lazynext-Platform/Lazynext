// install_skill: download a GitHub skill repo into ~/.lazynext/skills/<slug>/
...
function apiHeaders(): Record<string, string> {
  const headers: Record<string, string> = { Accept: 'application/vnd.github+json', 'User-Agent': 'lazynext-skill-install' };
...
export async function installGitHubSkill(
  ...
  return { slug, installedAt: join('~', '.lazynext', 'skills', slug), files: result.files, source: result.source };
}

export function skillInstallPlugin(): Plugin {
  return {
    name: 'lazynext-skill-install',
    configureServer(server) {
      server.middlewares.use('/api/skills/install', async (req, res) => {
        ...
        try {
          ...
          sendJson(res, 200, { ok: true, ...result, note: 'Skill installed to the user skill directory; the panel will display it automatically.' });
        } catch (error) {
