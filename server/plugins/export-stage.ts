...
const STAGE_PREFIX = 'lazynext-export-stage-';
const STAGE_EXTENSIONS = new Set(['.mp4', '.webm']);
const MAX_STAGE_BYTES = 64 * 1024 ** 3;
export const EXPORT_STAGE_RETENTION_MS = 30 * 60_000;
const STAGE_FILENAME = /^lazynext-export-stage-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(?:mp4|webm)$/i;
const STAGE_PARTIAL_FILENAME = /^lazynext-export-stage-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}\.(?:mp4|webm)\.part$/i;
...
function stageExtension(req: IncomingMessage): string | null {
  const url = new URL(req.url ?? '/', 'http://lazynext.local');
