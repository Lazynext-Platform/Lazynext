...
const directory = await mkdtemp(join(tmpdir(), 'lazynext-hf-proxy-'));
...
if (!process.env.HF_PROXY_SKIP_INTEGRATION) {
  const integrationDir = await mkdtemp(join(tmpdir(), 'lazynext-hf-proxy-integration-'));
  try {
    __resetModelMissingState();
...
assert.equal(modelCacheDir(), join(directory, '.lazynext', 'asr-models'));
