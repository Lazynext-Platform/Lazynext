...
const exportDir = await mkdtemp(join(tmpdir(), 'lazynext-export-cleanup-'));
try {
  const now = Date.now();
  ...
  const prefixedUserName = 'lazynext-export-job-project.mp4';
  await Promise.all([
...
const staleOutput = join(tmpdir(), `lazynext-retime-check-${randomUUID()}.mp4`);
await writeFile(staleOutput, 'stale partial output');
await assert.rejects(
  retimeFps('/definitely/missing/lazynext-input.mp4', staleOutput, 30, 'vp8', 4_000_000),
