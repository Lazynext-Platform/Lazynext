...
function addUnavailableWarning(warnings: string[], target: SearchTarget): void {
  const message = target.platform === 'freesound'
    ? 'Freesound is not configured, unable to search audio or music'
  ...
  for (const target of planned.targets) {
    if (target.platform === 'pexels' && options.pexelsApiKey) {
      ...
      if (orientation === 'square') {
        const warning = 'Pixabay official API does not support square orientation filter, other filters retained';
      ...
      server.middlewares.use('/api/stock-search', async (req, res) => {
        ...
        try {
          ...
          sendJson(res, 200, {
            configured: true,
            results: [],
            warnings: [`Stock search temporarily unavailable: ${message}`],
            searchedPlatforms: [],
