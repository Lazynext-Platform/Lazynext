...
const originalToken = process.env.LAZYNEXT_MCP_TOKEN;
const originalEditorUrl = process.env.LAZYNEXT_EDITOR_URL;
try {
  process.env.LAZYNEXT_MCP_TOKEN = 'mcp-secret';
  delete process.env.LAZYNEXT_EDITOR_URL;
  ...
  process.env.LAZYNEXT_EDITOR_URL = 'https://editor.example';
  ...
  const wrongCapabilityHeaders = {
    'X-Lazynext-Editor-Registration': 'w'.repeat(43),
  };
  ...
  for (const [operation, path, init] of bridgeRequests.slice(1)) {
    const response = await requestBridge(path, {
      ...
      headers: {
        ...
        'X-Lazynext-Editor-Registration': registrationCapability,
      },
    });
    assert(
  ...
  process.env.LAZYNEXT_EDITOR_URL = origin;
  ...
  delete process.env.LAZYNEXT_MCP_TOKEN;
  delete process.env.LAZYNEXT_EDITOR_URL;
  ...
  if (originalToken === undefined) delete process.env.LAZYNEXT_MCP_TOKEN;
  else process.env.LAZYNEXT_MCP_TOKEN = originalToken;
  if (originalEditorUrl === undefined) delete process.env.LAZYNEXT_EDITOR_URL;
  else process.env.LAZYNEXT_EDITOR_URL = originalEditorUrl;
  server.close();
