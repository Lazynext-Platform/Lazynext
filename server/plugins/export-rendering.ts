...
export async function renderExportPlan(
  ...
  try {
    ...
    throw new ExportFailureError(createExportFailure({
      ...
      targetPath: filepath,
      message: oom
        ? 'Out of memory during export. Please close other programs, shorten the export range, or lower the resolution and try again; if it still fails, restart the app and retry.'
        : message,
    }));
  }
}
