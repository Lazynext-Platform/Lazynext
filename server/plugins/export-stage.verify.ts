...
const stageIndex = pluginNames.indexOf('lazynext-export-stage');
const exportIndex = pluginNames.indexOf('lazynext-export');
assert.ok(stageIndex >= 0 && exportIndex >= 0 && stageIndex < exportIndex, 'stage route must precede /export catch-all');

const directory = await mkdtemp(join(tmpdir(), 'lazynext-export-stage-cleanup-'));
try {
  const now = Date.now();
  const staleStage = 'lazynext-export-stage-00000000-0000-4000-8000-000000000001.mp4';
  const stalePartial = 'lazynext-export-stage-00000000-0000-4000-8000-000000000002.webm.part';
  const freshStage = 'lazynext-export-stage-00000000-0000-4000-8000-000000000003.webm';
  const prefixedUserFile = 'lazynext-export-stage-project.mp4';
  await Promise.all([
