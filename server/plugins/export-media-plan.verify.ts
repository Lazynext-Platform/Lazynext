...
const directory = await mkdtemp(join(tmpdir(), 'lazynext-export-preflight-'));
try {
  ...
  assert.match(
    ...
    /^\/media\/uploads\/lazynext-render-media-[0-9a-f-]+\.mp4$/,
  );
  ...
  for (const entrypoint of ['render-still', 'render-clip', 'export-job']) {
    ...
    assert.match(localSource ?? '', /^\/media\/uploads\/lazynext-render-media-[0-9a-f-]+\.mp4$/);
  ...
  assert.deepEqual(
    (await readdir(directory)).filter((name) => name.startsWith('lazynext-render-media-')),
    [],
