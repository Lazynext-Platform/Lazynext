...
const fixture = await mkdtemp(join(tmpdir(), 'lazynext-keystore-profile-'));
const checkout = join(fixture, 'checkout');
const home = join(fixture, 'home');
const profileRoot = join(home, '.lazynext', 'dev-profiles', profileId);
const checkoutEnv = join(checkout, '.env.local');
const profileEnv = join(profileRoot, 'settings.env');
const previousCwd = process.cwd();
const previousHome = process.env.HOME;
const previousProfile = process.env.LAZYNEXT_DEV_PROFILE_ID;
...
try {
  ...
  process.env.LAZYNEXT_DEV_PROFILE_ID = profileId;
  ...
  if (previousProfile === undefined) delete process.env.LAZYNEXT_DEV_PROFILE_ID;
  else process.env.LAZYNEXT_DEV_PROFILE_ID = previousProfile;
