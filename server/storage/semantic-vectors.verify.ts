...
async function main(): Promise<void> {
  ...
  const previousEnv = process.env.LAZYNEXT_SQLITE_STORE;
  process.env.HOME = root;
  process.env.LAZYNEXT_SQLITE_STORE = '1';
  ...
  try {
    ...
    if (previousEnv === undefined) delete process.env.LAZYNEXT_SQLITE_STORE;
    else process.env.LAZYNEXT_SQLITE_STORE = previousEnv;
