...
async function main(): Promise<void> {
  ...
  const previousStore = process.env.LAZYNEXT_GENERATION_JOB_STORE;
  const previousSwitch = process.env.LAZYNEXT_SQLITE_STORE;
  const customJobsPath = join(home, 'custom-profile', 'jobs-ledger.json');
  process.env.HOME = home;
  process.env.LAZYNEXT_GENERATION_JOB_STORE = customJobsPath;
  delete process.env.LAZYNEXT_SQLITE_STORE;
  ...
  try {
    ...
    if (previousStore === undefined) delete process.env.LAZYNEXT_GENERATION_JOB_STORE;
    else process.env.LAZYNEXT_GENERATION_JOB_STORE = previousStore;
    if (previousSwitch === undefined) delete process.env.LAZYNEXT_SQLITE_STORE;
    else process.env.LAZYNEXT_SQLITE_STORE = previousSwitch;
