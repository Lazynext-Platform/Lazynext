...
const fixtureHome = mkdtempSync(join(tmpdir(), 'lazynext-key-probes-'));
process.env.HOME = fixtureHome;
process.env.USERPROFILE = fixtureHome;
delete process.env.LAZYNEXT_DEV_PROFILE_ID;
delete process.env.LAZYNEXT_DATA_DIR;
...
const EXPECTED_PAGES = [
  ...
  'image/openai', 'image/gemini', 'image/vertex', 'image/pollinations', 'image/minimax', 'image/wavespeed', 'image/byteplus',
  ...
  'video/veo', 'video/seedance', 'video/kling', 'video/hailuo', 'video/byteplus',
...
assert.match(networkMessage(new TypeError('fetch failed')), /Network unreachable[\s\S]*does not indicate an invalid Key/);
assert.match(networkMessage(Object.assign(new Error('The operation was aborted due to timeout'), { name: 'TimeoutError' })), /timeout/);
...
// 6. runProbe exits early: Unknown page / Key is not configured and does not connect to the network (keystore is empty, synchronization will return).
{
  ...
  assert.match(unknown.message, /does not yet support/);
...
{
  const unset = await runProbe('storage/projects', { LAZYNEXT_DATA_DIR: '' });
  assert.equal(unset.ok, true);
  assert.match(unset.message, /default directory .*\.lazynext/); // machine-dependent absolute path, anchor the tail only

  const relative = await runProbe('storage/projects', { LAZYNEXT_DATA_DIR: 'relative/saves' });
  assert.equal(relative.ok, false);
  assert.match(relative.message, /absolute path/);
  assert.doesNotMatch(relative.message, /HTTP/);

  const target = join(fixtureHome, 'Saves');
  const writable = await runProbe('storage/projects', { LAZYNEXT_DATA_DIR: target });
  ...
  mkdirSync(join(fixtureHome, '.lazynext'), { recursive: true });
  writeFileSync(
    join(fixtureHome, '.lazynext', 'data-dir.json'),
  ...
  process.env.LAZYNEXT_DATA_DIR = target;
  try {
    const pinned = await runProbe('storage/projects', { LAZYNEXT_DATA_DIR: join(fixtureHome, 'Elsewhere') });
    assert.equal(pinned.ok, false);
    assert.match(pinned.message, /fixed/);
  } finally {
    delete process.env.LAZYNEXT_DATA_DIR;
  }
}
