...
const fixture = await mkdtemp(join(tmpdir(), 'lazynext-media-profile-'));
try {
  ...
  const profileA = resolveRuntimeProfile({
    LAZYNEXT_DEV_PROFILE_ID: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
  }, { homeDir, cwd });
  const profileB = resolveRuntimeProfile({
    LAZYNEXT_DEV_PROFILE_ID: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',
  }, { homeDir, cwd });
  ...
  assert.deepEqual(await checkMediaDir(forbiddenProbe, profileA), {
    ok: false,
    error: 'Isolated dev config uses a fixed independent media directory; MEDIA_DIR cannot be changed',
  });
