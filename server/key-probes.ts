...
export async function runDataDirProbe(overrides: Record<string, unknown>): Promise<ProbeResult> {
  ...
  if (process.env[DATA_DIR_ENV]?.trim()) {
    return { ok: false, message: `Directory is fixed by ${DATA_DIR_ENV} and cannot be changed in settings` };
  }
...
export async function runProxyProbe(overrides: Record<string, unknown>): Promise<ProbeResult> {
  ...
  if (!proxyUrl) return { ok: false, message: 'No proxy address entered, and no system proxy environment variable detected' };
  let dispatcher: ProxyAgent;
  try {
    dispatcher = new ProxyAgent(proxyUrl);
  } catch {
    return { ok: false, message: 'Invalid proxy address format. Please enter http://host:port or https://host:port' };
  }
  const started = Date.now();
  try {
    const response = await fetch(PROXY_PROBE_URL, { signal: t(), dispatcher } as RequestInit);
    const latencyMs = Date.now() - started;
    if (!response.ok) return { ok: false, status: response.status, latencyMs, message: `Proxy connected, but external probe returned HTTP ${response.status}` };
    return { ok: true, status: response.status, latencyMs, message: `Proxy connection successful · external network reachable · ${latencyMs}ms` };
  } catch (error) {
...
function proxyNetworkMessage(error: unknown): string {
  const message = networkMessage(error).replace(/, does not indicate an invalid Key$/, '');
...
export function classifyStatus(status: number, bodyText: string): ProbeResult {
  if (status === 401 || status === 403) {
    return { ok: false, status, message: `Authentication failed (HTTP ${status}) · Key is invalid, expired, or lacks permission for this endpoint` };
  }
  if (status === 404) {
    return { ok: false, status, message: 'Probe endpoint 404 · Base URL may be incorrect (or this service does not recognize the probe path)' };
  }
  if (status === 429) {
    return { ok: true, status, message: 'Authentication passed (HTTP 429 rate-limited, which indicates the Key is valid)' };
  }
...
export function networkMessage(error: unknown): string {
  ...
  if (/timeout|abort/i.test(raw)) {
    return 'Connection timeout · service unreachable or network restricted (proxy may be required), does not indicate an invalid Key';
  }
  return `Network unreachable · ${sanitize(raw)} · this machine cannot connect to the service (proxy may be required), does not indicate an invalid Key`;
}
...
export async function runProbe(page: string, overrides: Record<string, unknown>): Promise<ProbeResult> {
  ...
  if (!probe) return { ok: false, message: 'This vendor does not yet support connection testing' };
  ...
  try {
    ...
    if (response.ok) {
      ...
      const modelText = models
        ? models.length > 0 ? ` · ${models.length} models loaded` : ' · API returned no model list'
        : '';
