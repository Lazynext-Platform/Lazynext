...
export async function syncUploadDirectories(
  ...
  if (copied > 0) log(`[media-dir] Migrated ${copied} media files:${source} → ${target}`);
  return copied;
}
...
export async function syncLegacyUploads(
  ...
  try {
    ...
    log(`[media-dir] Legacy media sync failed:${err instanceof Error ? err.message : String(err)}`);
  }
}
...
export async function checkMediaDir(
  ...
  if (isIsolatedDevProfile(profile)) {
    return { ok: false, error: 'Isolated dev config uses a fixed independent media directory; MEDIA_DIR cannot be changed' };
  }
  if (!raw.trim()) return { ok: true, note: `Not set · using default directory ${profile.mediaDir}` };
