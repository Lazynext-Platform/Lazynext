// User-visible skill files under ~/.lazynext/skills/<slug>/SKILL.md —
...
/** Override for tests and unusual installs; defaults to ~/.lazynext/skills. */
export function skillFilesRoot(): string {
  return getKey('LAZYNEXT_SKILLS_DIR') || join(homedir(), '.lazynext', 'skills');
}
...
/** Scan ~/.lazynext/skills/ for SKILL.md files (user-dropped or mirrored). */
...
/** Mirror one skill to ~/.lazynext/skills/<slug>/SKILL.md (idempotent). */
...
/** Human-readable install location for tool responses: ~/.lazynext/skills/<slug>. */
export function displaySkillPath(slug: string): string {
  return join('~', '.lazynext', 'skills', slug);
}
