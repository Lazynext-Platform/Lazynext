...
export const LAZYNEXT_SKILL_BASELINE = '2026-08-10.1';
...
const PROJECT_SELECTOR = {
  type: 'string',
  description: 'Lazynext project id. It must match the project bound to this MCP transport session.',
};
...
async function callControlTool(
  ...
  if (name === 'lazynext_status') return mcpStatus(session);
...
function makeServer(baseUrl: string, session: McpSession): Server {
  const server = new Server(
    { name: 'lazynext', version: '1.0.0' },
    {
      capabilities: { tools: { listChanged: true }, prompts: {} },
      instructions: [
        `Lazynext external skill baseline: ${LAZYNEXT_SKILL_BASELINE}. Update with npx skills update lazynext when the installed skill is older.`,
        ...
        'The target response and lazynext_status report bindingMode. Offline bindings expose only server-direct data tools and require approvalMode="auto".',
