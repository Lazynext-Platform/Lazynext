...
const controls = { lazynext_status: true } as const;
...
assert.equal(requestedMcpToolExposure({
  url: '/api/external-mcp/mcp',
  headers: { 'x-lazynext-tool-exposure': 'progressive' },
} as never), 'progressive');
const catalog: Tool[] = [
  { name: 'lazynext_status', inputSchema: { type: 'object' } },
...
assert.deepEqual(
  ...
  ['lazynext_status', 'ToolSearch', 'load_skill', 'read_project'],
);
