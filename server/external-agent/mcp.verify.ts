...
try {
  ...
  const boundA = await connectClient(mcpUrl, 'lazynext-mcp-binding-a');
  clients.push(boundA);
  let notify!: () => void;
  ...
  const exposureHeaders = { 'x-lazynext-tool-exposure': 'progressive' };
  const progressiveA = await connectClient(mcpUrl, 'lazynext-progressive-a', exposureHeaders);
  const progressiveB = await connectClient(mcpUrl, 'lazynext-progressive-b', exposureHeaders);
  ...
  const boundB = await connectClient(mcpUrl, 'lazynext-mcp-binding-b');
  clients.push(boundB);
  ...
  const switchClient = await connectClient(mcpUrl, 'lazynext-mcp-switch');
  ...
  const closeClientConnection = await connectClient(mcpUrl, 'lazynext-mcp-close');
  ...
  const expiredClient = await connectClient(mcpUrl, 'lazynext-mcp-expired');
  ...
  for (let index = 0; index < MCP_SESSION_COUNT_LIMIT; index += 1) {
    cappedClients.push(await connectClient(mcpUrl, `lazynext-mcp-cap-${index}`));
  }
  ...
  cappedClients.push(await connectClient(mcpUrl, 'lazynext-mcp-cap-overflow'));
