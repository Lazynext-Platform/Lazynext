...
const CODEX_HOME = join(homedir(), '.lazynext', 'codex');
...
export class CodexAppServerClient {
  ...
  private async startProcess(): Promise<void> {
    ...
    const runtimeCwd = await mkdtemp(join(tmpdir(), 'lazynext-codex-runtime-'));
    ...
    try {
      await this.rawRequest('initialize', {
        clientInfo: { name: 'lazynext', title: 'Lazynext', version: '1' },
  ...
  private handleServerRequest(id: number | string, method: string, params: Record<string, unknown>): void {
    ...
    if (method === 'execCommandApproval' || method === 'applyPatchApproval') {
      this.writeMessage({ id, result: { decision: { denied: { rejection: 'Use the Lazynext proposal review.' } } } });
      return;
    }
    ...
    for (const handler of this.serverRequests) {
      try {
        ...
        request.reject(-32603, 'Lazynext could not handle this request.');
        return;
      }
    }
    request.reject(-32601, 'Method not supported by Lazynext.');
  }
