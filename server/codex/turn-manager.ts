...
function codexErrorNotificationSummary(params: Record<string, unknown>): string {
  ...
  if (reason === 'usageLimitExceeded' || /usage limit|quota|credits|billing/i.test(detail)) {
    const base = 'Codex call failed: your OpenAI Codex usage quota is exhausted. Please visit chatgpt.com/codex/settings/usage to check and top up, or wait for the quota to reset before retrying; you can also switch to another model (e.g. DeepSeek).';
    return detail ? `${base}\n(${detail})` : base;
  }
  if (detail) return `Codex call failed: ${detail}`;
  return 'Codex call failed. Please retry later, or switch to another model (e.g. DeepSeek) from the model dropdown.';
}
...
function browserFailureSummary(result: unknown): string {
  ...
  return 'Lazynext tool execution failed.';
}
...
function baseInstructions(request: CodexTurnRequest): string {
  const mode = request.askOnly
    ? [
        ...
        'You may call the provided read-only Lazynext tools when current skill or project evidence is needed.',
      ].join(' ')
    : [
        'Use only the provided Lazynext editor tools when the task needs project reads or changes.',
        'Their results come from the existing Lazynext proposal workflow; never claim a change succeeded before its tool result confirms success.',
      ].join(' ');
  return [
    request.system.trim(),
    'Lazynext runtime rules:',
...
export class CodexTurnManager {
  ...
  private serverRequest(session: TurnSession, request: CodexServerRequest): boolean {
    ...
    if (!callId || !name || !session.toolNames.has(name) || session.pendingTools.has(callId)) {
      const message = 'This Lazynext tool call is unavailable.';
  ...
  private abortPendingTools(session: TurnSession): void {
    for (const pending of session.pendingTools.values()) {
      pending.request.respond({
        contentItems: [{ type: 'inputText', text: 'Lazynext cancelled this tool call.' }],
        success: false,
      });
    }
