...
/** Platform-aware hardware decode acceleration parameters. When the encoder is
 * known, uses the same API (decode/encode share the same hardware context);
 * otherwise falls back to a platform-generic choice. The internal hwaccel
 * automatically falls back to software decoding when the stream is unsupported,
 * with no format risk: decoded frames are output to system memory (nv12),
 * fully compatible with the existing CPU filter chain. */
...
/** Returns hardware decode acceleration parameters after probing ffmpeg build support; returns an empty array (software decode) when unsupported. */
...
export function resolveVaapiDevice(
  value = process.env.LAZYNEXT_VAAPI_DEVICE,
): string {
...
function disabledByEnvironment(): boolean {
  return /^(?:1|true|yes)$/i.test(process.env.LAZYNEXT_DISABLE_HARDWARE_ENCODING ?? '');
}
...
export function resolveH264Encoder(
  ...
  const forcedValue = process.env.LAZYNEXT_H264_ENCODER?.trim();
