...
// ── llmHeaders: Inject upstream authentication according to the protocol (google=x-goog-api-key;anthropic=x-api-key; the rest Bearer) ──
{
  ...
  const reqFor = (provider: string) => ({ headers: { 'x-lazynext-provider': provider } } as never);
  ...
  assert.match(llmErrorMessage(401, reqFor('gemini')), /Gemini.*authentication failed.*API Key/, '认证错误给设置入口');
  assert.match(llmErrorMessage(429, reqFor('openai')), /insufficient quota.*try again later/, '限流错误给额度提示');
}
...
const upstream = createServer(async (req, res) => {
  ...
  seen.push({
    ...
    provider: typeof req.headers['x-lazynext-provider'] === 'string'
      ? req.headers['x-lazynext-provider']
      : undefined,
...
try {
  const first = await fetch(`http://127.0.0.1:${proxyPort}/llm/chat/completions?stream=true`, {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-lazynext-provider': 'kimi' },
  ...
  await fetch(`http://127.0.0.1:${proxyPort}/llm/responses`, {
    method: 'POST',
    headers: { 'x-lazynext-provider': 'kimi', cookie: 'session=must-not-leak' },
    body: '{"model":"openai"}',
  });
