...
const fixture = await mkdtemp(join(tmpdir(), 'lazynext-data-dir-'));
try {
  ...
  assert.equal(dataDirPointerPath(home), join(home, '.lazynext', 'data-dir.json'));
  ...
  assert.equal(
    relocatedMediaDestination(null, join(fixture, 'home', '.lazynext'), defaultUploads),
    defaultUploads,
  );
