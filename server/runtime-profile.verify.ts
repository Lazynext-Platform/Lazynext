...
const globalRoot = join(homeDir, '.lazynext');
...
const overriddenDefault = resolveRuntimeProfile({
  LAZYNEXT_PROJECT_STORE_AUTH_DIR: ` ${customAuth} `,
  LAZYNEXT_GENERATION_JOB_STORE: customGeneration,
}, { homeDir, cwd });
...
assert.equal(resolveRuntimeProfile({ LAZYNEXT_GENERATION_JOB_STORE: '' }, {
  homeDir,
  cwd,
}).generationJobStore, '');
...
assert.throws(
  () => resolveRuntimeProfile({ LAZYNEXT_DEV_PROFILE_ROOT: isolatedRoot }, { homeDir, cwd }),
...
const dataDirHome = mkdtempSync(join(tmpdir(), 'lazynext-runtime-data-dir-'));
try {
  const chosen = join(dataDirHome, 'Saves', 'Lazynext');
  ...
  mkdirSync(join(dataDirHome, '.lazynext'), { recursive: true });
  ...
  assert.equal(
    ...
    join(dataDirHome, '.lazynext', 'dev-profiles', profileAId),
  ...
  assert.equal(defaultRootDir(envDataDir, dataDirHome), join(dataDirHome, '.lazynext'));
  assert.equal(
