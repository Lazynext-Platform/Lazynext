...
async def generative_fill_service(req: GenerativeFillRequest):
    ...
    if not tts_engine:
        raise HTTPException(status_code=503, detail="TTS Engine (Coqui or ElevenLabs) unavailable")

    return {
        "success": True,
        "avatar_video_url": f"file://{audio_path.replace('.wav', '.mp4')}" if os.path.exists(audio_path) else None,
        "script": req.script,
        "method": method,
        "tts_engine": tts_engine,
        "audio_duration_seconds": round(audio_duration, 2),
        "lip_sync_keyframes_count": len(lip_sync_keyframes),
        "lip_sync_keyframes": lip_sync_keyframes[:50],
