...
async fn read_ws_msg(
    ws: &mut (impl StreamExt<Item = Result<Message, impl std::error::Error>> + Unpin),
) -> Option<Value> {
...
async fn test_ws_connection_with_valid_token_can_switch_rooms() {
    ...
    ws.send(Message::Text(
        json!({"join": {"project_id": "room-a", "token": token}}).to_string(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
    ...
    ws.send(Message::Text(
        json!({"join": {"project_id": "room-b", "token": token}}).to_string(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
    ...
    ws.send(Message::Text(
        json!({"crdt_delta": {"project_id": "room-b", "delta": {"key": "val"}}}).to_string(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn test_ws_rejected_with_garbage_token() {
    ...
    ws.send(Message::Text(
        json!({"join": {"project_id": "proj-1", "token": "not-a-valid-jwt"}}).to_string(),
    ))
    .await
    .unwrap();

    let response = read_ws_msg(&mut ws).await.expect("Expected error response");
    assert!(
...
async fn test_ws_rejected_with_expired_token() {
    ...
    ws.send(Message::Text(
        json!({"join": {"project_id": "proj-1", "token": token}}).to_string(),
    ))
    .await
    .unwrap();
    ...
    let response = read_ws_msg(&mut ws)
        .await
        .expect("Expected error for expired token");
    assert!(
        response.get("error").is_some(),
        "Expected error, got: {response}"
    );
...
async fn test_ws_rejected_with_token_signed_by_different_secret() {
    ...
    ws.send(Message::Text(
        json!({"join": {"project_id": "proj-1", "token": wrong_token}}).to_string(),
    ))
    .await
    .unwrap();
    ...
    let response = read_ws_msg(&mut ws)
        .await
        .expect("Expected error for wrong secret");
    assert!(
        response.get("error").is_some(),
...
async fn test_ws_rejected_with_invalid_json() {
    ...
    let response = read_ws_msg(&mut ws)
        .await
        .expect("Expected error for invalid JSON");
    assert!(
        response.get("error").is_some(),
        "Expected error, got: {response}"
    );
...
async fn test_crdt_delta_processed_when_joined() {
    ...
    // Join a room
    ws.send(Message::Text(
        json!({"join": {"project_id": "proj-delta", "token": token}}).to_string(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn test_crdt_delta_ignored_for_unjoined_room() {
    ...
    ws.send(Message::Text(
        json!({"crdt_delta": {"project_id": "no-room", "delta": {"x": 1}}}).to_string(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn test_multiple_clients_join_same_room() {
    ...
    ws_a.send(Message::Text(
        json!({"join": {"project_id": "shared-room", "token": token_a}}).to_string(),
    ))
    .await
    .unwrap();

    ws_b.send(Message::Text(
        json!({"join": {"project_id": "shared-room", "token": token_b}}).to_string(),
    ))
    .await
    .unwrap();
...
async fn test_webrtc_offer_accepted() {
    ...
    // Join a room
    ws.send(Message::Text(
        json!({"join": {"project_id": "proj-webrtc", "token": token}}).to_string(),
    ))
    .await
    .unwrap();
    tokio::time::sleep(Duration::from_millis(50)).await;
...
async fn test_webrtc_signaling_with_two_clients() {
    ...
    // Both join the same room
    ws_a.send(Message::Text(
        json!({"join": {"project_id": "proj-webrtc-dual", "token": token_a}}).to_string(),
    ))
    .await
    .unwrap();

    ws_b.send(Message::Text(
        json!({"join": {"project_id": "proj-webrtc-dual", "token": token_b}}).to_string(),
    ))
    .await
    .unwrap();
