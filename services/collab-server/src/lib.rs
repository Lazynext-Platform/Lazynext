...
//!
//! Public library API. See `main.rs` for the binary entrypoint.
...
pub struct AppState {
	...
	pub rooms: DashMap<String, tokio::sync::broadcast::Sender<String>>,
	/// Maps authenticated peer IDs to their current project room.
	pub peer_rooms: DashMap<String, String>,
			...
			ClientMessage::Join {
				...
				let sub = match verify_token(&token) {
					...
					Err(e) => {
						let _ = socket
							.send(Message::Text(
								send_msg(&ServerMessage::Error { message: e })
									.into(),
							))
							.await;
						continue;
				...
				state
					.peer_rooms
					.insert(peer_id.clone(), project_id.clone());
...
/// Build the full Axum router for the collab server.
