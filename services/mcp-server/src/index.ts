...
function resolveResourceContent(uri: string): string {
  switch (uri) {
...
function resolvePromptContent(name: string, args?: Record<string, string>): Array<{ role: string; content: { type: string; text: string } }> {
  switch (name) {
...
End with pros/cons text overlay and final rating score.`
    ...
    case "edit/wedding": {
      ...
      const durationMap: Record<string, string> = { teaser: "1-2 minutes", highlight: "5-8 minutes", feature: "15-25 minutes" };
...
For ${style === "vlog" ? "vlogs" : style === "educational" ? "educational content" : style} style: keep energy high, cut pauses, use dynamic zoom. End with subscribe call-to-action.`
    ...
    case "edit/montage": {
      ...
      const paceMap: Record<string, string> = { fast: "0.5-1s per clip, rapid cuts", medium: "1-2s per clip, smooth cuts", slow: "2-4s per clip, lingering shots" };
...
server.setRequestHandler(GetPromptRequestSchema, async (request) => {
  ...
  return {
    messages: resolvePromptContent(name, promptArgs as Record<string, string> | undefined),
  };
});
