...
function buildExecuteBody(args: ExecuteArgs): Record<string, unknown> {
	...
	if (Array.isArray(args.attachedImages) && args.attachedImages.length > 0)
		body.attachedImages = args.attachedImages;
	if (Array.isArray(args.flaggedIssues) && args.flaggedIssues.length > 0)
		body.flaggedIssues = args.flaggedIssues;
	if (args.captionTemplatePreset)
		body.captionTemplatePreset = args.captionTemplatePreset;
	if (args.captionTemplateMode)
		body.captionTemplateMode = args.captionTemplateMode;
...
function flattenResult(d: Record<string, unknown>): ExecuteResult {
	return {
		...
		message:
			(d.message as string | undefined) ?? (d.reply as string | undefined),
		data: d.data,
...
export async function executeStreaming(
	...
	return new Promise<ExecuteResult>((resolve, reject) => {
		const req = transport.request(
			...
			(res) => {
				if (res.statusCode === 401 || res.statusCode === 403) {
					reject(
						new Error(
							`Streaming auth rejected (HTTP ${res.statusCode}). Check LEVEA_API_KEY.`,
						),
					);
					return;
				}
		...
		req.setTimeout(600_000, () =>
			req.destroy(new Error("Streaming timed out after 10m")),
		);
		req.write(body);
		req.end();
	});
...
export async function saveCaptionTemplate(
...
export async function saveCurrentCaptionTemplate(
...
export async function applyCaptionTemplate(
	body: Record<string, unknown>,
): Promise<unknown> {
	try {
...
export async function deleteCaptionTemplate(
	templateId: string,
): Promise<unknown> {
	try {
		const res = await http2().delete(
			v1(`/caption-templates/${encodeURIComponent(templateId)}`),
		);
		return res.data;
	} catch (err) {
...
export async function getTaskStatus(taskId: string): Promise<unknown> {
	try {
		const res = await http2().get(
			v1(`/task-status/${encodeURIComponent(taskId)}`),
		);
		return res.data;
	} catch (err) {
...
export async function getActiveTask(projectId: string): Promise<unknown> {
	try {
		const res = await http2().get(
			v1(`/projects/${encodeURIComponent(projectId)}/active-task`),
		);
		return res.data;
	} catch (err) {
...
export async function assetUploadUrl(
	body: Record<string, unknown>,
): Promise<unknown> {
	try {
...
export async function deleteAsset(fileName: string): Promise<unknown> {
	try {
		const res = await http2().delete(
			v1(`/assets/${encodeURIComponent(fileName)}`),
		);
		return res.data;
	} catch (err) {
