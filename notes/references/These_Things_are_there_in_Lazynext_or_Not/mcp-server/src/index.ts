...
const AUTONOMOUS_EDIT_SCHEMA = {
	...
	properties: {
		...
		scene: {
			type: "object",
			description:
				"Optional existing scene/timeline state to continue editing (advanced).",
		},
		requirePlanApproval: {
			type: "boolean",
			description:
		...
		attachedImages: {
			...
			items: { type: "string" },
			description:
				"Optional base64-encoded reference images the agent should consider while planning.",
		},
		flaggedIssues: {
			type: "array",
			items: { type: "string" },
			description:
				'Optional list of specific user-flagged problems for this run (e.g. ["caption #3 misspelled"]).',
		},
		...
		core_only: {
			type: "boolean",
			description:
				"If true, response carries only rendering-essential scene fields (no debug metadata).",
		},
		brandId: {
			type: "string",
			description:
		...
		projectBrandId: {
			type: "string",
			description:
				"Optional default brand for the project. Persists; per-call brandId overrides.",
		},
		editedPlan: {
			type: "object",
			description:
				"Optional pre-edited plan object returned by a prior awaiting_approval response.",
		},
	},
	required: ["prompt"],
} as const;
...
const TOOLS: Tool[] = [
	...
	// ── Job / task polling ───────────────────────────────────────────────────
	{
		...
		description:
			"Poll an export / async job by jobId (numeric). Supports wait:true to block until terminal.",
		inputSchema: {
			type: "object",
			properties: {
	...
	},
	{
		...
		description:
			"Poll a queued edit task by taskId (returned by queue_edit). Separate from job_id (exports).",
		inputSchema: {
			type: "object",
			properties: {
				task_id: { type: "string", description: "Task id from queue_edit." },
			},
			required: ["task_id"],
		},
	},
	{
		...
		description:
			"For a given project, return the currently-running edit task (if any). Useful for reconnecting MCP clients.",
		inputSchema: {
			type: "object",
			properties: {
	...
	// ── Caption templates ────────────────────────────────────────────────────
	{
		...
		description:
			"List the user's caption templates (builtin + saved). Returns slugs, display names, and style summaries.",
		inputSchema: { type: "object", properties: {}, required: [] },
	},
	{
		name: "apply_caption_template",
		description:
			"Apply a caption template to a scene synchronously (without going through autonomous_edit).",
		inputSchema: {
			type: "object",
			properties: {
				template_id: {
					type: "string",
					description: "Template slug / id from list_caption_templates.",
				},
				project_id: {
					type: "string",
					description: "Project / scene id to apply to.",
				},
				scene: {
					type: "object",
					description: "Inline scene (alternative to project_id).",
				},
				mode: {
					type: "string",
					enum: [
	...
	},
	{
		...
		description:
			"Save a new caption template definition the user can re-apply later.",
		inputSchema: {
			type: "object",
			properties: {
				...
				style: {
					type: "object",
					description:
						"Style payload (font, color, position, animation, etc.).",
				},
				tags: {
					type: "array",
					items: { type: "string" },
	...
	},
	{
		...
		description:
			"Snapshot the caption styling currently applied to a project / scene as a reusable template.",
		inputSchema: {
			type: "object",
			properties: {
				project_id: {
					type: "string",
					description: "Project to snapshot from.",
				},
	...
	// ── Brand kits ───────────────────────────────────────────────────────────
	{
		...
		description:
			"List the user's brand profiles (palette, fonts, logo, voice, gradeBias). Returns ids you pass to autonomous_edit's brandId / projectBrandId.",
		inputSchema: { type: "object", properties: {}, required: [] },
	},
	{
		name: "get_brand_kit",
	...
	},
	{
		...
		description:
			"Create a brand profile. Fields: name, palette {primary,secondary,accent,neutral,extras[]}, fonts {primary,secondary}, gradeBias (warm/cool/neutral/vivid/muted), forbiddenColors[], logoAssetUrl, voiceTone, enforcement (soft|hard).",
		inputSchema: {
			type: "object",
			properties: {
				name: { type: "string" },
	...
	},
	{
		...
		description:
			"Update an existing brand profile by id. Body has the same shape as create_brand_kit.",
		inputSchema: {
			type: "object",
			properties: {
		...
		inputSchema: {
			...
			properties: {
				...
				fileType: {
					type: "string",
					description:
						"video | image | audio (heuristic from extension if omitted).",
				},
				contentType: {
					type: "string",
				...
				projectId: {
					type: "string",
					description: "Optional project to associate the asset with.",
				},
			},
			required: ["fileName"],
		},
	},
	{
		...
		inputSchema: {
			...
			properties: {
				...
				fileType: {
					type: "string",
					description: "Optional filter: video|image|audio.",
				},
			},
			required: [],
		},
	},
	{
		name: "delete_asset",
		description:
			"Delete an asset by file name (the path returned by asset_upload_url).",
		inputSchema: {
			type: "object",
	...
	},
	{
		...
		description:
			"Force transcription of an existing asset. Returns transcript words + timing.",
		inputSchema: {
			type: "object",
			properties: {
				...
				language: {
					type: "string",
					description: "Optional ASR language hint.",
				},
			},
			required: ["fileName"],
		},
	...
	// ── Diagnostics ──────────────────────────────────────────────────────────
	{
		name: "editor_health",
		description:
			"Connectivity diagnostic against LEVEA_API_URL. Unauthenticated.",
		inputSchema: { type: "object", properties: {}, required: [] },
	},
];
...
function pickExecuteArgs(a: Record<string, unknown>): ExecuteArgs {
	...
	if (a.requirePlanApproval !== undefined)
		out.requirePlanApproval = !!a.requirePlanApproval;
	if (a.workingMemory !== undefined)
		out.workingMemory = a.workingMemory as Record<string, unknown>;
	if (Array.isArray(a.attachedImages))
		out.attachedImages = a.attachedImages as string[];
	if (Array.isArray(a.flaggedIssues))
		out.flaggedIssues = a.flaggedIssues as string[];
	if (a.captionTemplatePreset)
		out.captionTemplatePreset = String(a.captionTemplatePreset);
	if (a.captionTemplateMode)
		out.captionTemplateMode = String(a.captionTemplateMode);
	...
	if (a.editedPlan !== undefined)
		out.editedPlan = a.editedPlan as Record<string, unknown>;
	return out;
}
...
function summarizeExec(r: ExecuteResult): string {
	...
	const lines = [
		`${head} ${r.message || r.reply || (r.success ? "Done" : "Failed")}`,
	];
...
function summarizeAny(label: string, data: unknown): string {
	if (data && typeof data === "object") {
		...
		if (d.success === false)
			return `❌ ${label} failed: ${d.error || d.message || "unknown"}`;
		const count = Array.isArray((d as any).items)
			? (d as any).items.length
			: Array.isArray((d as any).templates)
				? (d as any).templates.length
				: Array.isArray((d as any).brands)
					? (d as any).brands.length
					: Array.isArray((d as any).projects)
						? (d as any).projects.length
						: Array.isArray((d as any).assets)
							? (d as any).assets.length
							: Array.isArray((d as any).data)
								? ((d as any).data as unknown[]).length
								: undefined;
...
async function dispatch(
	...
	switch (name) {
		case "autonomous_edit": {
			if (!str(a.prompt))
				return fail("❌ `prompt` is required for autonomous_edit.");
		...
		case "autonomous_edit_streaming": {
			if (!str(a.prompt))
				return fail("❌ `prompt` is required for autonomous_edit_streaming.");
			const events: SSEEvent[] = [];
			let stepN = 0;
			const r = await executeStreaming(pickExecuteArgs(a), (ev) => {
				events.push(ev);
				const dataObj = (
					typeof ev.data === "object" ? ev.data : { raw: ev.data }
				...
				if (ctx.notifyProgress && typeStr !== "heartbeat") {
					...
					ctx
						.notifyProgress(stepN, undefined, `${typeStr}: ${msg}`)
						.catch(() => undefined);
				}
			});
		...
		case "queue_edit": {
			if (!str(a.prompt))
				return fail("❌ `prompt` is required for queue_edit.");
			const r = await queueEdit({
				tool: "autonomous_edit",
				params: {
					prompt: a.prompt,
					...(a.video_url ? { video_url: a.video_url } : {}),
					...(Array.isArray(a.assets) ? { assets: a.assets } : {}),
				},
		...
		case "check_job_status": {
			...
			const job =
				a.wait === true ? await waitForJob(id) : await getJobStatus(id);
			return ok(
		...
		case "save_caption_template": {
			...
			const r = await saveCaptionTemplate({
				name: nm,
				style: a.style,
				tags: a.tags,
			});
		...
		case "save_current_caption_template": {
			const nm = str(a.name);
			if (!nm) return fail("❌ `name` is required.");
			const r = await saveCurrentCaptionTemplate({
				name: nm,
				projectId: str(a.project_id),
				scene: a.scene,
			});
