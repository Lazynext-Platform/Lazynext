...
export async function handleEditorOperation(
	...
	if (request.requirePlanApproval !== undefined)
		topLevel.requirePlanApproval = request.requirePlanApproval;
	if (request.workingMemory !== undefined)
		topLevel.workingMemory = request.workingMemory;
	if (
		Array.isArray(request.attachedImages) &&
		request.attachedImages.length > 0
	)
		topLevel.attachedImages = request.attachedImages;
	if (Array.isArray(request.flaggedIssues) && request.flaggedIssues.length > 0)
		topLevel.flaggedIssues = request.flaggedIssues;
	if (request.captionTemplatePreset)
		topLevel.captionTemplatePreset = request.captionTemplatePreset;
	if (request.captionTemplateMode)
		topLevel.captionTemplateMode = request.captionTemplateMode;
	...
	if (request.editedPlan !== undefined)
		topLevel.editedPlan = request.editedPlan;
	...
	try {
		...
		const { success, data, message, videoUrl, jobId, status } =
			response.data || {};
		return {
...
function handleError(error: any): EditorResponse {
	...
	if (status === 401 || status === 403) {
		return {
			success: false,
			message:
				"❌ Authentication failed. Check LEVEA_API_KEY at https://studio.livecore.ai/.",
		};
	}
	if (status === 400) {
		return {
			success: false,
			message: `❌ ${error.response?.data?.message || error.response?.data?.error || "Invalid request"}`,
		};
	}
	if (status === 429) {
		return {
			success: false,
			message: "⏱️ Rate limit exceeded. Please try again shortly.",
		};
	}
	if (error.code === "ECONNABORTED" || error.code === "ETIMEDOUT") {
		return {
			success: false,
			message:
				"⏱️ Timeout. The edit may be running — check back via the project.",
		};
	}
