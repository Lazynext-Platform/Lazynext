...
The client never accepts a _full path_ in `LEVEA_API_URL` — set only the host. The path `/api/v1/misc/openclaw/v1/execute` is appended automatically. Do not point at `/api/v1/misc/editor/` (different route, different auth model).
...
- Repo: https://github.com/Lazynext-Corporation/openclaw-ai-video-editor
