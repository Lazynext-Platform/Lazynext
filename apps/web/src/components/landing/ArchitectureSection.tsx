				...
				<div className="grid lg:grid-cols-2 gap-16 items-center">
					...
					<div>
						...
						<h2 className="text-3xl font-extrabold text-zinc-900 dark:text-white md:text-4xl mb-6 leading-tight">
							17 Enterprise Microservices.<br />
							One Unified Timeline.
						</h2>
						<p className="mt-4 text-zinc-600 dark:text-zinc-400 text-lg leading-relaxed max-w-2xl mx-auto">
							We didn't just build an editor. We built an AI Copilot that edits footage for you based on text prompts. Lazynext acts as the central nervous system bridging Python ML servers, Rust core binaries, and WebAssembly to execute your natural language commands.
						</p>
						...
						<ul className="space-y-4">
							<li className="flex items-start gap-3">
								...
								<div>
									<h4 className="font-semibold text-zinc-900 dark:text-white">GPU-Accelerated Inference</h4>
									<p className="text-sm text-zinc-600 dark:text-zinc-400">Seamlessly runs tensor operations via PyTorch and ONNX, directly managed by our FastAPI orchestration layer.</p>
								</div>
							</li>
							<li className="flex items-start gap-3">
								...
								<div>
									<h4 className="font-semibold text-zinc-900 dark:text-white">Modular Architecture</h4>
									<p className="text-sm text-zinc-600 dark:text-zinc-400">Easily swap out any model. Upgrade Whisper to Deepgram, or swap SDXL for Midjourney without breaking the timeline structure.</p>
								</div>
							</li>
						</ul>
					</div>
						...
						<div className="flex flex-col gap-6 transform rotate-[-2deg] scale-110">
							...
							<div className="flex gap-4 animate-[scroll_40s_linear_infinite]">
								{[...PROJECTS, ...PROJECTS].slice(0, 10).map((p, i) => (
									<div key={`r1-${i}`} className="whitespace-nowrap rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 px-6 py-3 font-mono text-sm text-zinc-700 dark:text-zinc-300 backdrop-blur-md shadow-sm dark:shadow-none">
										{p}
									</div>
								))}
							</div>
							...
							<div className="flex gap-4 animate-[scroll_50s_linear_infinite_reverse]">
								{[...PROJECTS, ...PROJECTS].slice(5, 15).map((p, i) => (
									<div key={`r2-${i}`} className="whitespace-nowrap rounded-xl border border-indigo-200 dark:border-indigo-500/20 bg-indigo-50 dark:bg-indigo-500/10 px-6 py-3 font-mono text-sm text-indigo-700 dark:text-indigo-300 backdrop-blur-md shadow-sm dark:shadow-none">
										{p}
									</div>
								))}
							</div>
							...
							<div className="flex gap-4 animate-[scroll_35s_linear_infinite]">
								{[...PROJECTS, ...PROJECTS].slice(10, 20).map((p, i) => (
									<div key={`r3-${i}`} className="whitespace-nowrap rounded-xl border border-zinc-200 dark:border-white/10 bg-white dark:bg-white/5 px-6 py-3 font-mono text-sm text-zinc-700 dark:text-zinc-300 backdrop-blur-md shadow-sm dark:shadow-none">
										{p}
									</div>
								))}
							</div>
