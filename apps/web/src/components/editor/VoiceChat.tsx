...
export function VoiceChat({ isVoiceActive, startVoice, stopVoice, peers }: VoiceChatProps) {
  return (
    <div className="absolute bottom-6 right-6 z-50">
      <div className="bg-neutral-900/80 backdrop-blur-xl border border-neutral-800 p-4 rounded-2xl shadow-2xl flex flex-col gap-4 min-w-[200px]">
        ...
        <div className="flex items-center justify-between border-b border-neutral-800 pb-2">
          ...
          <span className="text-xs text-neutral-500">{Object.keys(peers).length + (isVoiceActive ? 1 : 0)} Online</span>
        </div>
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="w-8 h-8 rounded-full bg-neutral-800 flex items-center justify-center border border-neutral-700">
        <span className="text-neutral-400 text-xs font-bold">{id.substring(0,2).toUpperCase()}</span>
      </div>
      <span className="text-sm text-neutral-300 font-mono text-xs truncate max-w-[100px]">{id}</span>
    </div>
  );
}
