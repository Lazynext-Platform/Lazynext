...
/** React component rendering MediaPoolSidebar. */
		...
		>
			...
			{mediaPoolPos.floating && (
				...
				<div
					...
					onMouseDown={(e: React.MouseEvent) => {
						...
						const onMove = (ev: MouseEvent) =>
							setMediaPoolPos(
								(p: { floating: boolean; x: number; y: number }) => ({
									...p,
									x: ev.clientX - startX,
									y: ev.clientY - startY,
								}),
				...
				>
					<div className="flex items-center gap-2">
						...
						<label
							className="flex items-center gap-1.5 cursor-pointer ml-4"
							title="Split Audio/Video on Import"
						>
							<input
								type="checkbox"
				...
				<div className="px-4 pt-4 pb-2 flex flex-col gap-3 shrink-0">
					...
					<div className="flex gap-1 overflow-x-auto custom-scrollbar mt-1">
						...
						{[
							...
							<button
								key={bin}
								className="shrink-0 px-2 py-0.5 text-[10px] bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 rounded-full hover:bg-indigo-500/20 transition-colors whitespace-nowrap"
							>
								{bin}
							</button>
						))}
					...
					<>
						...
						<button
							...
							onClick={() => {
								...
								if (vids.length < 2) {
									toast.error(
										"Need at least 2 video clips to create a Multi-Cam sequence.",
									);
									return;
								}
								const multiCamPromise = new Promise<void>((resolve) =>
						...
						{assets
							.filter((a: any) => {
								if (mediaFilter !== "all" && a.type !== mediaFilter)
									return false;
								if (
									mediaSearchQuery &&
				...
				{sidebarTab === "titles" &&
					[
						...
						},
						{
							...
							color: "#ffea00",
							font_size: 72,
							drop_shadow_distance: 10,
							drop_shadow_color: "#000000",
						},
						{
							name: "Cinematic",
							type: "text",
							text_content: "CINEMATIC",
							font_family: "Georgia",
							color: "#ffffff",
							font_size: 36,
							letter_spacing: 10,
						...
						<div
							...
							onDragStart={(e) => {
								e.dataTransfer.setData(
									"application/json",
									JSON.stringify({ type: "preset", preset, isEffect: false }),
								);
				...
				{sidebarTab === "effects" &&
					[
						...
						{ name: "Color Space Transform", effectType: "cst", icon: "🌈" },
						{
							...
							effectType: "content_aware",
							icon: "✨",
						},
						{ name: "Advanced 3D Keyer", effectType: "3d_keyer", icon: "🟩" },
						{
							name: "AI Depth of Field (Bokeh)",
							effectType: "ai_bokeh",
							icon: "📷",
						},
					].map((preset, i) => (
						<div
							...
							onDragStart={(e) => {
								e.dataTransfer.setData(
									"application/json",
									JSON.stringify({ type: "preset", preset, isEffect: true }),
								);
								e.dataTransfer.effectAllowed = "copy";
							}}
						...
						{[
							{
								...
								items: [
									...
									},
									{
										...
										effectType: "additive_dissolve",
										duration: 30,
										gradient: "from-amber-300/20 to-transparent",
									},
									{
										name: "Dip to Black",
										effectType: "dip_to_black",
										duration: 20,
										gradient: "from-black to-transparent",
									},
									{
										name: "Dip to White",
										effectType: "dip_to_white",
										duration: 20,
										gradient: "from-white/40 to-transparent",
									},
								],
							},
							{
								...
								items: [
									{
										name: "Constant Power",
										effectType: "audio_constant_power",
										duration: 30,
										gradient: "from-blue-400/20 to-transparent",
									},
									{
										name: "Constant Gain",
										effectType: "audio_constant_gain",
										duration: 30,
										gradient: "from-sky-300/20 to-transparent",
									},
									{
										name: "Exponential Fade",
										effectType: "audio_exponential_fade",
										duration: 30,
							...
							},
							{
								...
								items: [
									{
										name: "Wipe Left",
										effectType: "wipe_left",
										duration: 20,
										gradient: "from-indigo-500/20 to-transparent",
									},
									{
										name: "Wipe Right",
										effectType: "wipe_right",
										duration: 20,
										gradient: "from-transparent to-indigo-500/20",
									},
									{
										name: "Wipe Up",
										effectType: "wipe_up",
									...
									{
										name: "Wipe Down",
										effectType: "wipe_down",
										duration: 20,
										gradient: "from-transparent to-cyan-500/20",
									},
									{
										name: "Clock Wipe",
										effectType: "clock_wipe",
										duration: 30,
							...
							},
							{
								...
								items: [
									{
										name: "Push Left",
										effectType: "push_left",
										duration: 15,
										gradient: "from-emerald-500/20 to-transparent",
									},
									{
										name: "Push Right",
										effectType: "push_right",
										duration: 15,
										gradient: "from-transparent to-emerald-500/20",
									},
									{
										name: "Slide Over",
										effectType: "slide_over",
										duration: 20,
							...
							},
							{
								...
								items: [
									...
									},
									{
										...
										effectType: "zoom_out",
										duration: 15,
										gradient: "from-transparent to-amber-500/20",
									},
									{
										name: "Spin",
										effectType: "spin",
									...
									{group.items.map((item, ii) => (
										...
										>
											...
											<div className="h-12 relative overflow-hidden">
												<div
													className={`absolute inset-0 bg-gradient-to-r ${item.gradient} opacity-60 group-hover/card:opacity-100 transition-opacity`}
												/>
												<svg
													viewBox="0 0 80 32"
				...
				{sidebarTab === "fusion" && (
					<div className="flex flex-col h-full bg-background">
						<div className="px-3 py-2 border-b border-border flex items-center justify-between sticky top-0 bg-background z-10">
							...
							<div className="flex gap-1">
								<button
									className="text-[10px] bg-sky-500/20 hover:bg-sky-500/30 text-sky-300 border border-sky-500/30 px-2 py-1 rounded transition-colors"
						...
						<div className="flex-1 relative overflow-hidden bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05)_0%,transparent_100%)]">
							...
							<svg className="absolute top-0 left-0 w-full h-full pointer-events-none">
								<path
									d="M 64 25% C 100 25%, 80 50%, 120 50%"
									stroke="#eab308"
									strokeWidth="2"
									fill="none"
								/>
								<path
									d="M 180 50% C 220 50%, 200 75%, 240 75%"
									stroke="#3b82f6"
									strokeWidth="2"
									fill="none"
								/>
				...
				{sidebarTab === "plugins" && (
					<div className="flex flex-col h-full bg-background p-4 overflow-y-auto custom-scrollbar">
						...
						<button
							...
							onClick={() => {
								const pluginPromise = new Promise<void>((resolve) =>
									setTimeout(resolve, 2000),
								);
								pluginPromise.then(() => {
									setInstalledPlugins([
								...
								toast.promise(pluginPromise, {
									loading: "Installing plugin from local script...",
									success:
										"Script loaded and registered new extension API endpoints!",
								...
								{markers.length +
									(projectData.tracks?.reduce(
										(acc: number, t: any) =>
											acc +
											(t.clips?.reduce(
												(cAcc: number, c: any) => cAcc + (c.notes?.length || 0),
												0,
											) || 0),
										0,
									) || 0)}{" "}
			...
			{sidebarTab === "media" && (
				<div className="flex flex-col border-t border-border mt-auto">
					...
					<button
						className="mt-2 block w-full text-center text-[10px] font-bold text-sky-300 bg-sky-900/30 hover:bg-sky-800/50 border border-sky-500/50 rounded py-1.5 cursor-pointer transition-colors shadow-sm"
						onClick={() => handleTelepathicLink()}
					>
						🧠 Telepathic Import
					</button>
				</div>
