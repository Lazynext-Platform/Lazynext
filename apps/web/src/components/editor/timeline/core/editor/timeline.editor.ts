...
export class TimelineEditor {
	...
	protected setTimelineData({
		...
		const resolvedBackgroundColor =
			backgroundColor !== undefined
				? backgroundColor
				: prevTimelineData?.backgroundColor;
		...
		if (updatePlayerData) {
			...
			this.context?.setTimelineAction?.(TIMELINE_ACTION.UPDATE_PLAYER_DATA, {
				...
				version: updatedVersion,
				watermark:
					watermark != null ? (watermark as any).toJSON?.() : undefined,
				backgroundColor: resolvedBackgroundColor,
				metadata: resolvedMetadata,
			});
		}
	...
	getCaptionsTrack(): Track | null {
		const prevTimelineData = this.getTimelineData();
		const track = prevTimelineData?.tracks.find(
			(t) => t.getType() === TRACK_TYPES.CAPTION,
		);
		return track as Track | null;
	}
	...
	getTracksByType(type: string): Track[] {
		const prevTimelineData = this.getTimelineData();
		return (prevTimelineData?.tracks.filter(
	...
	getTracksByPredicate(
		...
		const prevTimelineData = this.getTimelineData();
		return (prevTimelineData?.tracks.filter((track, index) =>
			predicate(track, index),
		) ?? []) as Track[];
	}
	...
	upsertTrack(input: TrackUpsertInput): Track {
		...
		const created = this.addTrack(
			input.name,
			input.type ?? TRACK_TYPES.ELEMENT,
		);
		if (input.language !== undefined) {
	...
	async moveElementToNewTrackAt(
		...
		else if (elType === "caption" || elType === "text")
			trackType = TRACK_TYPES.ELEMENT;
		...
		const insertIndex = Math.max(
			0,
			Math.min(targetTrackIndex, currentTracks.length),
		);
		const newTracks = [
	...
	loadProjectSnapshot({
		...
		const timelineTracks = migratedProject.tracks.map((track) =>
	...
	getProject(): ProjectJSON {
		...
		return {
			...
			watermark:
				data.watermark != null ? (data.watermark as any).toJSON?.() : undefined,
