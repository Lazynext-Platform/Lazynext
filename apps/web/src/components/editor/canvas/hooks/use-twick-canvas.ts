...
export const useTwickCanvas = ({
	...
	const buildCanvas = ({
		...
		if (twickCanvasRef.current) {
			...
			twickCanvasRef.current.off(
				"selection:created",
				applyMarqueeSelectionControls,
			);
			twickCanvasRef.current.off(
				"selection:updated",
				applyMarqueeSelectionControls,
			...
			switch (action) {
				...
				case "rotate": {
					...
					const result = handler?.updateFromFabricObject?.(
						object,
						currentElement ??
							({ id: elementId, type: "text", props: {} } as CanvasElement),
						context,
					);
					if (result) {
	...
	const applyZOrder = (
		...
		if (element)
			elementMap.current[elementId] = { ...element, zIndex: newZIndex };
		onCanvasOperation?.(CANVAS_OPERATIONS.Z_ORDER_CHANGED, {
			elementId,
			direction,
		});
		return true;
	...
	const sendBackward = (elementId: string) =>
		applyZOrder(elementId, "backward");
