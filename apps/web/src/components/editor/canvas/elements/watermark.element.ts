...
export const WatermarkElement: CanvasElementHandler = {
	...
	async add(params) {
		const { element, index, canvas, canvasMetadata, watermarkPropsRef } =
			params;
	...
	updateFromFabricObject(object, element, context) {
		...
		const props =
			...
			(object.scaleX != null || object.scaleY != null)
				? {
						...baseProps,
						width:
							baseProps.width != null && object.scaleX != null
								? baseProps.width * object.scaleX
								: baseProps.width,
						height:
							baseProps.height != null && object.scaleY != null
								? baseProps.height * object.scaleY
								: baseProps.height,
					}
				: baseProps;
		...
		return {
			element: {
				...element,
				props: { ...element.props, x, y, rotation, opacity, ...props },
			},
