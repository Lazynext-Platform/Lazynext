...
describe("getKeyframedValue", () => {
  ...
  it("caches sorted keyframes (calling twice returns same result)", () => {
    ...
    const a = getKeyframedValue({ clip, property: "opacity", defaultValue: 0, frame: 50 });
    const b = getKeyframedValue({ clip, property: "opacity", defaultValue: 0, frame: 50 });
    expect(a).toBe(b);
    expect(a).toBe(0.5);
  });
});
...
describe("hasKeyframe", () => {
  it("returns false when clip is null", () => {
    expect(hasKeyframe({ clip: null, property: "opacity", frame: 0 })).toBe(false);
  });

  it("returns false when clip has no keyframes", () => {
    expect(hasKeyframe({ clip: makeClip(), property: "opacity", frame: 0 })).toBe(false);
  });
