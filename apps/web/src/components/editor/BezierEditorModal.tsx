...
interface BezierEditorModalProps {
	...
	setBezierEditor: React.Dispatch<React.SetStateAction<BezierEditorState | null>>;
	projectData: any;
	commitState: (data: any) => void;
}

export function BezierEditorModal({ bezierEditor, setBezierEditor, projectData, commitState }: BezierEditorModalProps) {
	...
	const handleSaveCurve = () => {
		...
		const kf = clip?.keyframes?.find(
			(k: any) => k.property === bezierEditor.property && Math.abs(k.frame - bezierEditor.frame!) < 0.5
		);
		if (kf) {
				...
				<div className="grid grid-cols-2 gap-4 mb-4">
					<div className="flex flex-col gap-2 p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
						...
						<div className="flex items-center justify-between">
							...
							<input type="range" min="0" max="1" step="0.01" value={bezierEditor.curve[0]} onChange={(e) => setBezierEditor((prev: BezierEditorState | null) => prev ? { ...prev, curve: [parseFloat(e.target.value), prev.curve[1], prev.curve[2], prev.curve[3]] } : null)} className="flex-1 accent-indigo-500 mx-2 h-1" />
						...
						<div className="flex items-center justify-between">
							...
							<input type="range" min="-0.5" max="1.5" step="0.01" value={bezierEditor.curve[1]} onChange={(e) => setBezierEditor((prev: BezierEditorState | null) => prev ? { ...prev, curve: [prev.curve[0], parseFloat(e.target.value), prev.curve[2], prev.curve[3]] } : null)} className="flex-1 accent-indigo-500 mx-2 h-1" />
					...
					<div className="flex flex-col gap-2 p-3 bg-zinc-800/50 rounded-lg border border-zinc-700/50">
						...
						<div className="flex items-center justify-between">
							<span className="text-[10px] text-zinc-500 w-4">X</span>
							<input type="range" min="0" max="1" step="0.01" value={bezierEditor.curve[2]} onChange={(e) => setBezierEditor((prev: BezierEditorState | null) => prev ? { ...prev, curve: [prev.curve[0], prev.curve[1], parseFloat(e.target.value), prev.curve[3]] } : null)} className="flex-1 accent-pink-500 mx-2 h-1" />
							<span className="text-[10px] text-zinc-300 w-6">{bezierEditor.curve[2].toFixed(2)}</span>
						</div>
						<div className="flex items-center justify-between">
							<span className="text-[10px] text-zinc-500 w-4">Y</span>
							<input type="range" min="-0.5" max="1.5" step="0.01" value={bezierEditor.curve[3]} onChange={(e) => setBezierEditor((prev: BezierEditorState | null) => prev ? { ...prev, curve: [prev.curve[0], prev.curve[1], prev.curve[2], parseFloat(e.target.value)] } : null)} className="flex-1 accent-pink-500 mx-2 h-1" />
