...
export function AudioMixer({ projectData, setProjectData }: { projectData: any, setProjectData: any }) {
  ...
  const audioTracks = projectData.tracks?.filter((t: any) => t.type === 'audio' || t.type === 'video') || [];
  ...
  return (
    <div className="flex bg-zinc-900 border border-zinc-800 rounded-lg overflow-x-auto overflow-y-hidden h-64 shadow-xl">
      <div className="flex items-center justify-center p-2 border-r border-zinc-800 bg-zinc-950/50 min-w-[40px]">
        <span className="text-zinc-500 font-bold text-[10px] uppercase tracking-widest" style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }}>
          Fairlight Mixer
        </span>
      </div>
      ...
      <div className="flex flex-1 p-2 gap-4">
        {audioTracks.map((track: any, idx: number) => (
          <div key={track.id} className="flex flex-col items-center min-w-[80px] bg-zinc-950 rounded-lg p-2 border border-zinc-800">
            <span className="text-[10px] text-zinc-300 font-mono truncate w-full text-center">{track.name || `A${idx + 1}`}</span>
              ...
              <div className="w-2 h-full bg-black rounded-full overflow-hidden flex flex-col justify-end">
                <div 
                  ...
                  style={{
                    ...
                    backgroundColor: (meters[track.id] || -60) > -6 ? '#ef4444' : (meters[track.id] || -60) > -18 ? '#eab308' : '#22c55e'
                  }}
                />
              </div>
            ...
            <span className="text-[9px] font-mono text-zinc-500 mt-1">{track.volume ? track.volume.toFixed(2) : '1.00'}</span>
          </div>
        ))}
      </div>
      ...
      <div className="flex flex-col items-center min-w-[90px] bg-zinc-950 p-2 border-l border-zinc-800">
        <span className="text-[10px] text-amber-500 font-bold uppercase tracking-wider mb-2">Master</span>
        <div className="flex-1 flex gap-2 mb-2 relative">
             ...
             <div className="w-2 h-full bg-black rounded-sm overflow-hidden flex flex-col justify-end">
                <div 
                  ...
                  style={{
                    ...
                    backgroundColor: (meters['master_left'] || -60) > -6 ? '#ef4444' : (meters['master_left'] || -60) > -18 ? '#eab308' : '#22c55e'
                  }}
                />
              </div>
              <div className="w-2 h-full bg-black rounded-sm overflow-hidden flex flex-col justify-end">
                <div 
                  ...
                  style={{
                    ...
                    backgroundColor: (meters['master_right'] || -60) > -6 ? '#ef4444' : (meters['master_right'] || -60) > -18 ? '#eab308' : '#22c55e'
                  }}
                />
              </div>
           </div>
           ...
           <div className="w-4 h-full bg-black rounded-full relative group">
              ...
              />
              <div className="absolute left-1/2 -translate-x-1/2 w-8 h-4 bg-red-900 rounded cursor-pointer pointer-events-none group-hover:bg-red-800 border border-red-500" style={{ bottom: '50%' }} />
           </div>
        </div>
        <div className="flex gap-1 w-full justify-center">
            <button className="w-full h-6 rounded flex items-center justify-center bg-zinc-800 text-zinc-400 hover:text-white border border-transparent hover:border-zinc-700">
              <Sliders className="w-3 h-3 mr-1" /> <span className="text-[9px]">EQ</span>
            </button>
        </div>
      </div>
    </div>
