...
function applySongsFilters({
	...
	params.append("filter", "duration:[1.0 TO 600.0]");
	params.append("filter", `avg_rating:[${min_rating} TO *]`);
	...
	params.append(
		"filter",
		"tag:music OR tag:loop OR tag:melody OR tag:instrumental OR tag:beat OR tag:ambient OR tag:cinematic OR tag:electronic OR tag:orchestral OR tag:piano OR tag:guitar",
	);
}
