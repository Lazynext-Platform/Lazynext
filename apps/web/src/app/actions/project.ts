...
export async function getProjectData(projectId: string) {
  ...
  const trackResults = await db.select().from(tracks).where(eq(tracks.projectId, projectId));
  
  const tracksWithClips = await Promise.all(trackResults.map(async (track) => {
    const clipResults = await db.select().from(clips).where(eq(clips.trackId, track.id));
    return {
      ...track,
      clips: clipResults
    };
