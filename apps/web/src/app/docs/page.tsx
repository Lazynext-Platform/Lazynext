...
export const metadata = generateMetadata({
  ...
  description: "Complete API reference for Lazynext. 15 endpoints, AI agent chat, Stripe checkout, user profile, FFMPEG filter engine. REST API with Better Auth.",
  path: "/docs",
});
      ...
      <div className="mx-auto max-w-4xl px-4 py-16">
        ...
        <section className="mt-10">
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-zinc-800">
            {ENDPOINTS.map(ep => (
              <div key={ep.p+ep.m} className="flex items-center gap-4 border-b border-zinc-800 px-5 py-3 last:border-none">
                <span className={`rounded px-2 py-0.5 text-xs font-bold ${ep.m==="GET"?"bg-emerald-500/20 text-emerald-400":ep.m==="POST"?"bg-blue-500/20 text-blue-400":"bg-amber-500/20 text-amber-400"}`}>{ep.m}</span>
                <code className="flex-1 text-sm text-zinc-200">{ep.p}</code>
                <span className="text-xs text-zinc-500">{ep.d}</span>
                {ep.a && <span className="rounded bg-zinc-800 px-1.5 py-0.5 text-xs text-zinc-400">Auth</span>}
              </div>
            ))}
          </div>
        </section>
