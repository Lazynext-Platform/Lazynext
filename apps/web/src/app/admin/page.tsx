      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-neutral-900/50 border border-neutral-800 p-6 rounded-2xl flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-3xl font-bold">${metrics.monthlyRecurringRevenue}</p>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-neutral-800">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-neutral-800/50 transition-colors">
                  <td className="px-6 py-4 text-neutral-300 font-mono text-xs">{user.id}</td>
                  <td className="px-6 py-4 font-medium">{user.email}</td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      user.role === 'admin' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-neutral-800 text-neutral-400'
                    }`}>
                      {user.role}
                    </span>
                  </td>
