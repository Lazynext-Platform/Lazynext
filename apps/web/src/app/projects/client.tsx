...
export default function ProjectsClient({ initialProjects }: { initialProjects: any[] }) {
	...
	useEffect(() => {
		...
		} else if (!editor.project.getIsInitialized()) {
			editor.project.loadAllProjects();
		}
				...
				<main className="mx-auto px-4 pt-6 pb-12 flex flex-col gap-6 max-w-[1400px]">
					{isLoading || !isInitialized ? (
						...
						<div
							className={
								viewMode === "grid"
									? "xs:grid-cols-2 grid grid-cols-1 gap-8 sm:grid-cols-3 lg:grid-cols-4 px-4"
									: "flex flex-col gap-3"
							}
						>
...
function ProjectItem({
	...
	const gridContent = (
		<Card className="bg-zinc-900/40 backdrop-blur-xl border border-white/10 overflow-hidden p-0 transition-all duration-300 hover:scale-[1.02] hover:shadow-[0_0_30px_-5px_rgba(139,92,246,0.15)] group rounded-2xl">
