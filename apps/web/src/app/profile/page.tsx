...
export default function ProfilePage() {
	...
	if (!profile) return <div className="min-h-screen bg-background flex items-center justify-center text-[var(--accent-primary)]">Loading profile...</div>;
		...
		<div className="min-h-screen bg-background text-foreground font-sans p-8 md:p-24 selection:bg-[var(--accent-primary)]/30 selection:text-[var(--accent-primary)] relative overflow-hidden">
			...
			>
				<motion.div variants={itemVariants} className="mb-12">
					<h1 className="text-4xl md:text-5xl font-bold mb-4 text-foreground tracking-tight">Your Profile</h1>
					<p className="text-muted text-lg">Manage your Lazynext creator identity and account security.</p>
				</motion.div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-8">
					<motion.div variants={itemVariants} className="bg-panel backdrop-blur-xl border border-border rounded-3xl p-8 shadow-lg hover:border-[var(--accent-primary)]/20 transition-all duration-300">
						...
						<div className="space-y-6">
							<div>
								<label htmlFor="profile-full-name" className="text-sm text-muted mb-2 flex items-center gap-2"><User className="w-4 h-4"/> Full Name</label>
								<input id="profile-full-name" type="text" defaultValue={profile.name} className="w-full bg-background border border-border rounded-xl px-4 py-3 text-foreground focus:outline-none focus:border-[var(--accent-primary)]/50 transition-colors" />
							</div>
							<div>
								<label htmlFor="profile-email" className="text-sm text-muted mb-2 flex items-center gap-2"><Mail className="w-4 h-4"/> Email Address</label>
								<input id="profile-email" type="email" defaultValue={profile.email} disabled className="w-full bg-background border border-border rounded-xl px-4 py-3 text-muted opacity-50 cursor-not-allowed" />
							</div>
						</div>
					</motion.div>
					...
					<div className="space-y-8">
						<motion.div variants={itemVariants} className="bg-panel backdrop-blur-xl border border-border rounded-3xl p-8 shadow-lg hover:border-[var(--accent-primary)]/20 transition-all duration-300">
							...
							<p className="text-muted mb-6 text-sm">Protect your account with multifactor authentication.</p>
							<button className="w-full bg-background hover:bg-hover border border-border text-foreground rounded-xl px-4 py-3 transition-colors font-medium">
								Setup 2FA
							</button>
						</motion.div>
						...
						<motion.div variants={itemVariants} className="bg-panel backdrop-blur-xl border border-border rounded-3xl p-8 shadow-lg hover:border-[var(--accent-primary)]/20 transition-all duration-300">
							...
							<div className="w-full bg-background rounded-full h-3 mb-2 overflow-hidden border border-border">
