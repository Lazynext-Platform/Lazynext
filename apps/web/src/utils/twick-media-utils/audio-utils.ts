			...
			audioContext.decodeAudioData(
				...
				(err) =>
					reject(
						err ||
							new Error(
								"Failed to decode audio: no audio track found or unsupported format",
							),
					),
			);
		});
...
const encodePcmToMp3 = async (buffer: AudioBuffer): Promise<Blob> => {
	...
	const mp3encoder = new lamejs.default.Mp3Encoder(
		channels,
		targetSampleRate,
		kbps,
	);
	...
	for (let i = 0; i < left.length; i += samplesPerFrame) {
		const leftChunk = left.subarray(
			i,
			Math.min(i + samplesPerFrame, left.length),
		);
		let mp3buf: Uint8Array;
		if (channels === 2 && right) {
			const rightChunk = right.subarray(
				i,
				Math.min(i + samplesPerFrame, right.length),
			);
