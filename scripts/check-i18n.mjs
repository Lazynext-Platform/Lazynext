...
function auditNativeAttribute(sf, node, initializers) {
  ...
  return literals.map((literal) => (
    issue(sf, literal, `native control attribute uses raw Chinese: ${JSON.stringify(literal.text)}`)
  ));
}
...
function auditJsxExpression(sf, node, relative, initializers) {
  ...
  const issues = directlyRenderedLiterals(node.expression, sf)
    ...
    .map((literal) => issue(sf, literal, `JSX expression renders raw Chinese: ${JSON.stringify(literal.text)}`));
  ...
  return [
    ...
    ...(initializer ? untranslatedLiterals(initializer, sf) : [])
      .map((literal) => issue(sf, literal, `variable ${node.expression.text} renders untranslated Chinese: ${JSON.stringify(literal.text)}`)),
  ];
}
...
function auditUiFile(filePath, keys) {
  ...
  function visit(node) {
    if (ts.isJsxText(node) && CJK.test(node.text.trim())) {
      issues.push(issue(sf, node, `renders raw Chinese: ${JSON.stringify(node.text.trim())}`));
    }
    ...
    if (
      ...
      issues.push(issue(sf, node, `unregistered Chinese UI literal: ${JSON.stringify(node.text)}`));
    }
...
const translationIssues = sourceFiles.flatMap((filePath) => {
  ...
  function visit(node) {
    if (
      ...
      issues.push(issue(sf, node.arguments[0], `missing from EN dictionary: ${JSON.stringify(node.arguments[0].text)}`));
    }
    ts.forEachChild(node, visit);
  }
  visit(sf);
...
console.log(`i18n check passed: ${uiFiles.length} UI files, ${keys.size} EN entries`);
