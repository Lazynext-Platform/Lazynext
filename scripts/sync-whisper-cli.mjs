...
//   LAZYNEXT_WHISPER_CLI or copied into public/whisper-cli/darwin-arm64/.
...
const PLATFORMS = {
  'darwin-arm64': {
    ...
    note: 'compile from source; LAZYNEXT_WHISPER_CLI overrides',
  },
...
async function buildFromSource(srcDir, buildDir, platformKey) {
  ...
  try {
    ...
    throw new Error('cmake is required to build whisper.cpp on this platform (brew install cmake); or set LAZYNEXT_WHISPER_CLI to a prebuilt binary');
  }
...
async function main() {
  ...
  const override = process.env.LAZYNEXT_WHISPER_CLI;
  if (override) {
  ...
  if (!spec.asset) {
    ...
    // LAZYNEXT_WHISPER_CLI still wins when set.
