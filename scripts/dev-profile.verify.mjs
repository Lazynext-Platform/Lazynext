...
const fixture = await mkdtemp(join(tmpdir(), 'lazynext-dev-profile-'));
...
try {
  ...
  assert.equal(first.rootDir, join(stablePaths.homeDir, '.lazynext', 'dev-profiles', UUID_A));
  assert.deepEqual(
  ...
  const childEnvironment = await profileChildEnvironment(first, {
    ...
    UNRELATED: 'inherited',
    LAZYNEXT_DEV_PROFILE_ID: UUID_B,
  ...
  assert.equal(childEnvironment.LAZYNEXT_DEV_PROFILE_ID, UUID_A);
