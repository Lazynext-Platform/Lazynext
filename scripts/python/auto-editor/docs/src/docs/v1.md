...
The v1 format is the simplest way to represent a timeline. It supports cutting and changing speed from `0.0` to `99999.0` inclusive. v1 is a stable format. Developers are welcome to use it to make cuts for Lazynext-Editor and to use it for their own programs.

You can generate a v1 timeline file with `Lazynext-Editor example.mp4 --export v1` and it would look something like this:
...
```
Lazynext-Editor input.v1 -o output.mkv
```
