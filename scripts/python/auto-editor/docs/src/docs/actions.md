...
Actions define what Lazynext-Editor does to different parts of your media. By default, inactive (silent) sections are cut out and active (loud) sections are kept unchanged. Actions give you fine-grained control over this behavior.
...
Lazynext-Editor video.mp4 --when-silent cut

# Keep normal sections unchanged (default behavior)
Lazynext-Editor video.mp4 --when-normal nil
```
...
Lazynext-Editor video.mp4 --when-silent nil
```
...
Lazynext-Editor video.mp4 --when-normal cut
```
...
Lazynext-Editor video.mp4 --when-normal speed:0.5
```
...
Lazynext-Editor video.mp4 --when-silent varispeed:2

# Create slow-motion effect with lower pitch
Lazynext-Editor video.mp4 --when-normal varispeed:0.5
```
...
Lazynext-Editor video.mp4 --when-silent volume:0.2

# Boost loud sections
Lazynext-Editor video.mp4 --when-normal volume:1.5
```
...
Lazynext-Editor video.mp4 --when-normal deesser:0.5

# Stronger de-essing with a higher max reduction and tuned frequency
Lazynext-Editor video.mp4 --when-normal deesser:0.8:0.7:0.4
```
...
```bash
Lazynext-Editor video.mp4 --when-silent invert
```
...
Lazynext-Editor video.mp4 --when-normal zoom:2

# Zoom out on silent sections
Lazynext-Editor video.mp4 --when-silent zoom:0.5
```
...
Lazynext-Editor video.mp4 --when-normal rotate:180

# Turn a 1920x1080 landscape video into a 1080x1920 portrait one
Lazynext-Editor video.mp4 --when-normal rotate:90 --resolution 1080,1920
```
...
Lazynext-Editor video.mp4 --when-normal spin:0/120

# Start at 90 degrees and spin counter-clockwise at 45 degrees/second
Lazynext-Editor video.mp4 --when-normal spin:90/-45
```
...
Lazynext-Editor video.mp4 --when-normal add:./logo.png,spin:0/-30
```
...
Lazynext-Editor video.mp4 --when-normal drawbox:100:100:400:200:red

# A black bar across part of the frame (e.g. to redact something)
Lazynext-Editor video.mp4 --when-normal drawbox:0:0:1920:200:#000000
```
...
Lazynext-Editor video.mp4 --when-normal add:./logo.png

# Put a logo at (600, 300)
Lazynext-Editor video.mp4 --when-normal add:./logo.png:600:300:1.0

# Shrink an overlay video to a quarter size in the corner
Lazynext-Editor video.mp4 --when-normal add:./pip.mp4:900:60:0.25

# Overlay only over a specific time range (frames where 1s..2s plays)
Lazynext-Editor video.mp4 --set-action add:./logo.png:600:300:1.0,1sec,2sec
```
...
`Lazynext-Editor song.mp3 -bg white -w:1 add:./cover.png` makes a video, but if the
...
Lazynext-Editor video.mp4 --when-silent speed:3,volume:0.5

# Combine speed and varispeed
# Effective speed: 1.25 × 1.25 = 1.5625x
Lazynext-Editor video.mp4 --when-normal varispeed:1.25,speed:1.25

# Triple action: speed, varispeed, and volume
Lazynext-Editor video.mp4 --when-silent speed:2,varispeed:1.5,volume:0.8
```
...
Lazynext-Editor video.mp4 --when-normal zoom:1..1.5

# Fade in (opacity 0 to 1)
Lazynext-Editor video.mp4 --when-normal opacity:0..1
```
...
Lazynext-Editor video.mp4 --when-normal zoom:1..1.5..1

# Fade in, hold, fade out
Lazynext-Editor video.mp4 --when-normal opacity:0..1..1..0
```
...
```bash
Lazynext-Editor video.mp4 --when-normal zoom:1..1.5:ease=inout
```
...
Lazynext-Editor video.mp4 --when-normal opacity:0..1:ease=in:2sec
```
...
Lazynext-Editor video.mp4 --when-normal ease:out,zoom:1..1.3,brightness:0..0.3
```
...
Lazynext-Editor video.mp4 --set-action nil,0,5sec

# Apply speed + varispeed from 30 seconds to the end
Lazynext-Editor video.mp4 --set-action speed:1.5,varispeed:1.5,30sec,end
```
...
Lazynext-Editor video.mp4 --when-normal varispeed:0.75

# Fast silent sections with reduced volume
Lazynext-Editor video.mp4 --when-silent speed:6,volume:0.4
```
...
```bash
# Old way (deprecated)
Lazynext-Editor video.mp4 --silent-speed 8 --video-speed 1

# New way (preferred)
Lazynext-Editor video.mp4 --when-silent speed:8 --when-normal speed:1
```
