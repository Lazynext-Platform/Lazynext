...
function excludeUserMediaFromBuild(): Plugin {
  ...
  return {
    name: 'lazynext-exclude-user-media',
    apply: 'build',
...
export default defineConfig(({ mode }) => {
  ...
  if (profile.mode === 'isolated-dev') {
    process.stdout.write(`[Lazynext] isolated profile ${profile.id} · ${profile.rootDir}\n`);
  }
        ...
        output: {
          codeSplitting: {
            groups: [
              ...
              { name: 'templates', test: /lazynext-templates\.json/, priority: 25, includeDependenciesRecursively: false },
