...
export function resolveDesktopDevOrigin({
  ...
  if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
    throw new Error('LAZYNEXT_DESKTOP_DEV_URL must use HTTP or HTTPS');
  }
  if (!isLoopbackHostname(parsed.hostname)) {
    throw new Error('LAZYNEXT_DESKTOP_DEV_URL must use a loopback host');
  }
  if (parsed.username || parsed.password) {
    throw new Error('LAZYNEXT_DESKTOP_DEV_URL must not contain credentials');
  }
