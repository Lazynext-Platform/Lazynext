...
function registerDesktopHandlers(trustedOrigin: string): void {
  ipcMain.handle('lazynext:select-directory', trustedDesktopHandler(trustedOrigin, async (event, requestedPath: unknown) => {
    const parent = BrowserWindow.fromWebContents(event.sender);
    const requested = typeof requestedPath === 'string' && isAbsolute(requestedPath)
      ? requestedPath
      : app.getPath('videos');
    const options: OpenDialogOptions = {
      title: 'Choose media save folder',
  ...
  ipcMain.handle('lazynext:select-export-directory', trustedDesktopHandler(trustedOrigin, async (event) => {
    const parent = BrowserWindow.fromWebContents(event.sender);
    const options: OpenDialogOptions = {
    ...
    if (!directory) throw new Error('The selected export folder is not available');
  ...
  ipcMain.handle('lazynext:select-export-file', trustedDesktopHandler(trustedOrigin, async (
    event,
  ...
  ipcMain.handle('lazynext:restore-export-directory', trustedDesktopHandler(trustedOrigin, async () => {
  ...
  ipcMain.handle('lazynext:transparent-mov-proxy', trustedDesktopHandler(trustedOrigin, async (_event, storedName: unknown) => {
  ...
  ipcMain.handle('lazynext:window-action', trustedDesktopHandler(trustedOrigin, (event, action: unknown) => {
  ...
  ipcMain.handle('lazynext:zoom-step', trustedDesktopHandler(trustedOrigin, async (event, step: unknown) => {
    const win = BrowserWindow.fromWebContents(event.sender);
    if (!win) return;
    ...
    applyResponsiveWindowScale(win);
    win.webContents.send('lazynext:ui-scale-changed', next);
  }));
  ipcMain.handle('lazynext:reveal-export', trustedDesktopHandler(trustedOrigin, async (
    _event,
...
async function boot(): Promise<void> {
  ...
  const desktopInference = installDesktopInferenceIpc(
    origin,
    join(app.getPath('home'), '.lazynext', 'asr-models'),
  );
