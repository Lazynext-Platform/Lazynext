...
export async function runDesktopSmokeProbe(
  ...
  if (!mcp.ok || !(await mcp.text()).includes('"name":"lazynext"')) {
  ...
  const pickerType = await win.webContents.executeJavaScript(
    'typeof window.lazynextDesktop?.selectDirectory',
  ) as unknown;
  ...
  const updaterType = await win.webContents.executeJavaScript(
    'typeof window.lazynextDesktop?.updates?.check',
  ) as unknown;
  ...
  const inference = await win.webContents.executeJavaScript(
    'window.lazynextDesktop?.inference?.getCapabilities()',
  ) as {
    version?: unknown;
