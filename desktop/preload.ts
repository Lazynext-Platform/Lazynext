...
export interface LazynextDesktopApi {
...
const api: LazynextDesktopApi = {
  getPathForFile: (file) => webUtils.getPathForFile(file) || undefined,
  platform: process.platform,
  selectDirectory: (defaultPath) =>
    ipcRenderer.invoke('lazynext:select-directory', defaultPath) as Promise<string | null>,
  selectExportDirectory: () =>
    ipcRenderer.invoke('lazynext:select-export-directory') as Promise<DesktopExportDirectoryGrant | null>,
  selectExportFile: (suggestedFilename) =>
    ipcRenderer.invoke('lazynext:select-export-file', suggestedFilename) as Promise<DesktopExportFileGrant | null>,
  restoreExportDirectory: () =>
    ipcRenderer.invoke('lazynext:restore-export-directory') as Promise<DesktopExportDirectoryGrant | null>,
  importLocalMedia: (file) => importLocalMediaFromFile(file, localMediaPreloadDependencies),
  prepareTransparentMovProxy: (storedName) =>
    ipcRenderer.invoke('lazynext:transparent-mov-proxy', storedName) as Promise<{ src: string } | null>,
  ...
  windowAction: (action) =>
    ipcRenderer.invoke('lazynext:window-action', action) as Promise<void>,
  zoomStep: (step) =>
    ipcRenderer.invoke('lazynext:zoom-step', step) as Promise<void>,
  subscribeUiScale: (listener) => {
    ...
    ipcRenderer.on('lazynext:ui-scale-changed', handleScale);
    return () => { ipcRenderer.removeListener('lazynext:ui-scale-changed', handleScale); };
  },
  ...
  revealExport: (destinationId, filename) =>
    ipcRenderer.invoke('lazynext:reveal-export', destinationId, filename) as Promise<void>,
...
contextBridge.exposeInMainWorld('lazynextDesktop', api);
