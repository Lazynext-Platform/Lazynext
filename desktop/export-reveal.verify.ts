...
assert.match(main, /lazynext:reveal-export/, 'Electron main must own the filesystem reveal operation');
...
assert.match(history, /r\.destinationId && window\.lazynextDesktop\?\.revealExport/, 'legacy and browser rows must not offer reveal');
...
assert.match(english, /'Open Folder': 'Open Folder'/, 'the new export action must stay localized in English');
