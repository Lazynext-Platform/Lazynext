...
for (const blockmap of [
  ...
  assert.ok(
    workflow.includes(`release-files/Lazynext-\${EXPECTED_VERSION}-${blockmap}`),
...
assert.doesNotMatch(
  workflow,
  /find release -type d -name Lazynext\.app|(?:mac|win|linux)-unpacked\b/,
...
assert.match(
  workflow,
  /"\$mounted_app\/Contents\/MacOS\/Lazynext"/,
...
assert.match(
  workflow,
  /Lazynext\.app\/Contents\/MacOS\/Lazynext/,
  'macOS update ZIP must contain the application executable',
);
assert.match(
  workflow,
  /Lazynext\.app\/Contents\/Frameworks\/Electron Framework\.framework\/Versions\/A\/Electron Framework/,
  'macOS update ZIP must contain the Electron runtime',
);
assert.match(
  workflow,
  /render_runtime="Lazynext\.app\/Contents\/Resources\/chrome-headless-shell\//,
