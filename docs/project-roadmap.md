...
| **24** | Browser Extension — real timeline import (push media via REST/IndexedDB), manifest/icon consistency, tests | 4.1–4.9 | Browser Extension | Medium | #02, #03 | 🟡 Genuinely remaining (real timeline import path + tests). 4.7 (dead dep) already removed; 4.4 (URL crash) already fixed in #14. |
...
| **27** | API Gateway — integration tests (auth→DB→response), OpenAPI spec, CSRF/rate-limit verification | 6.8–6.9 | API Gateway | Medium | #03 | 🟡 Genuinely remaining (integration tests + OpenAPI spec). Auth/PostgreSQL/Stripe/CSRF/rate-limit already real (Feature #09). |
