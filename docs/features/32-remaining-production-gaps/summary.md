...
- **Gap #3 — Generative fill**: Replaced HTTP 501 stub with Modal + OpenCV inpainting fallback
- **Gap #4 — Avatar dubbing**: Wired `avatar-prompt.tsx` to real Edge TTS API
- **Gap #5 — Local filesystem storage**: Unblocked uploads with graceful remote-storage fallback when external credentials are absent
...
- `services/generative-studio/src/` — Modal generative fill path
- `services/ai-agents/src/` — Edge TTS integration
- `apps/web/src/lib/storage.ts` — local storage with remote fallback
