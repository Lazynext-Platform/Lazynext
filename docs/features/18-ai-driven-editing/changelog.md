...
- **Built**:
  - D.2 (error visibility): EditorClient.tsx now shows per-tool errors in the chat (⚠️ prefixed) and specific error messages in toasts instead of the generic "Lazynext AI Agent AI encountered an error." Error responses from the API propagate verbatim.
