...
- [Document Templates](#-document-templates)
  ...
  - [Project Motto Doc](#18-project-motto-document)
  - [Feature Motto Doc](#19-feature-motto-document)
...
- Every action you take MUST trace back to a documented task, architecture decision, or explicit human instruction.
...
- Anticipate problems by reading the docs thoroughly BEFORE starting work — not by improvising solutions mid-stream.
- When you feel the urge to go beyond the documented scope: **STOP. Re-read the task. Do only that.**

Violating this rule — even with good intentions — is the single most common failure mode for AI agents. The framework exists precisely to prevent drift. Trust the docs, not your instincts.
...
- **Respect autonomy boundaries** — before any action, check the [Autonomy Boundaries](#autonomy-boundaries) table. Run a [Verification Cross-Check](#verification-cross-checks) before declaring a phase complete.
...
> **Note**: `project-context.md`, `project-roadmap.md`, `project-changelog.md`, and the root-level `AGENTS.md` are required but **not created in Step 1** — they are produced in later steps (3, 3.5, 4, 4.5) as outputs of the project discussion. The skeleton above is the minimal starting point.
...
### Step 5 — Create the Project Motto

Create `docs/project-motto.md` using Template #18. This is the project's **corrective reference** — a short (~20-40 lines) doc that distills the project's DO/DON'T boundaries into a format that can be re-read in 30 seconds.

The motto synthesizes `project-context.md` and `project-roadmap.md` into actionable guardrails. If writing the motto reveals inconsistencies or gaps in other docs, go back and fix them.

**Use case**: When an AI agent drifts or hallucinates, tell it "follow the motto" — it re-reads the doc and self-corrects.

### Step 6 — Start Feature 01
...
From here on, the full [Feature Lifecycle](#-the-workflow--feature-lifecycle) applies. The first new feature follows the complete Discuss → Design → Plan → Approve → Build → Ship → Reflect cycle.
...
├── project-motto.md            # 🧭 Project guardrails — DO/DON'T boundaries for agents (from context)
...
│   │   ├── motto.md               # 🧭 Feature guardrails — DO/DON'T for this feature
│   │   ├── changelog.md
│   │   └── review.md
│   └── ...
│
...
| **project-motto.md** | Project | Project guardrails — DO/DON'T boundaries and north star | After project-context.md and roadmap are created |
...
| **motto.md** | Feature | Feature guardrails — DO/DON'T boundaries for this feature | After plan approved (Stage 4), before Build |
...
| **project-motto** | ✅ Always | Never skip — this is the project's corrective reference when agents drift |
...
| **motto** | ✅ Always | Never skip — the corrective reference that keeps Build on track |
...
Every feature flows through **7 stages**. Each stage has a clear entry condition and exit condition.

```
┌───────────────────────────────────────────────────────────────────────────────┐
│                       FEATURE LIFECYCLE — 7 STAGES                            │
│                                                                               │
│  ┌────────┐  ┌────────┐  ┌────────┐  ┌────────┐                               │
│  │   1.   │  │   2.   │  │   3.   │  │   4.   │                               │
│  │DISCUSS │─▶│ DESIGN │─▶│  PLAN  │─▶│APPROVE │                               │
│  │   💬   │  │   🏗️   │  │   📋   │  │   ✅   │                               │
│  └────────┘  └────────┘  └────────┘  └────────┘                               │
│  discussion architecture   tasks +     human                                  │
│     doc         doc       testplan    signoff                                 │
│                                                                               │
│              ┌────────┐  ┌────────┐  ┌────────┐                               │
│              │   5.   │  │   6.   │  │   7.   │                               │
│              │ BUILD  │─▶│  SHIP  │─▶│REFLECT │                               │
│              │   🔨   │  │   🚀   │  │   🪞   │                               │
│              └────────┘  └────────┘  └────────┘                               │
│              changelog   merged to    review +                                │
│               updates      main       lessons                                 │
└───────────────────────────────────────────────────────────────────────────────┘
```
...
| Action | Detail |
|---|---|
| Read previous feature's `review.md` | Import any "Key Lessons to Carry Forward" into the new discussion's "Lessons from Previous Features" section. If this is feature #01, write "N/A — first feature". |
...
> **Entry**: All Stage 3 planning docs created and feature branch exists
> **Exit**: Human approval recorded in `tasks.md` (or `changelog.md`)

The Approve stage is the **explicit human signoff gate on the plan** before any code is written. It formalizes a checkpoint that exists informally in many teams and gives AI agents a clear, enforceable "STOP and wait for human signoff" boundary.

| Action | Detail |
|---|---|
| Reviewer reads planning docs | Discussion → Architecture → Tasks → Testplan (+ api/research if present) |
| Verify cross-document alignment | Discussion intent ↔ architecture design ↔ task breakdown ↔ testplan coverage |
| Raise blockers if any | Scope unclear, design suspect, risk unaddressed → return to Stage 1, 2, or 3 as needed |
| Record approval | Add `## Approval ✅` block to the top of `tasks.md` (preferred) or as a dated entry in `changelog.md` |
| Optional: assign builder | Note who (human or AI agent) will execute Build |

**Approval block format** (placed at the top of `tasks.md`):

```markdown
## Approval ✅
- **Approved by**: [Name / GitHub handle]
- **Date**: YYYY-MM-DD
- **Notes**: [Any conditions, scope clarifications, or "None"]
```

> **Rules**:
> - AI agents MUST NOT begin Stage 5 (Build) work until this Approval block is filled.
> - **Lightweight features self-approve** inside `lightweight.md` (no separate planning docs to gate on).
> - **Hotfixes skip Approve** entirely — they have their own abbreviated workflow.
> - Approval can be revoked: if a blocker is found mid-Build, return to Stage 2 or 3, fix, then re-approve.

#### Create Feature Motto (after Approve, before Build)

After approval is recorded, create `XX-feature-name/motto.md` using Template #19. This is the **final planning artifact** — a short doc (~20-40 lines) that distills the feature's essence into DO/DON'T boundaries.

The motto synthesizes discussion + architecture + tasks into a focused corrective reference. If writing the motto reveals gaps or inconsistencies in the planning docs, go back and fix them before starting Build.

> **Rule**: AI agents MUST create `motto.md` before beginning Build. If told "follow the motto" during Build, re-read the doc and self-correct.

### Stage 5 — Build 🔨

> **Entry**: Plan approved (Stage 4 signoff recorded), motto created, all planning docs done
...
Not everything can go through the full 7-stage lifecycle. Critical bugs in production need an abbreviated path.
...
```
┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│ IDENTIFY │─▶│  BRANCH  │─▶│   FIX    │─▶│  VERIFY  │─▶│  MERGE   │─▶│ DOCUMENT │
└──────────┘  └──────────┘  └──────────┘  └──────────┘  └──────────┘  └──────────┘
```
...
4. Update `project-roadmap.md` — flip the reverted feature's status from 🟢 Complete back to 🟡 In Progress (or 🔴 Not Started if fully abandoned)
5. Document the revert in `project-changelog.md` and the relevant feature changelog
6. Create a follow-up feature or hotfix to properly fix the issue before re-merging
...
Not every feature needs the full lifecycle docs. For planned changes that are docs-only, config-only, or trivially simple, the Lightweight Feature Variant provides an abbreviated path using a **single combined document** instead of the standard planning docs (discussion, architecture, tasks, testplan, motto, changelog, review).
...
- Lightweight features **do not create a separate `motto.md`** — the scope is trivial enough that guardrails are unnecessary
...
| **Doc filenames** | Simple type name inside the folder | `discussion.md`, `architecture.md`, `tasks.md`, `testplan.md`, `api.md`, `research.md`, `motto.md`, `changelog.md`, `review.md` |
...
Use this template when a feature meets ALL lightweight eligibility criteria (see [Lightweight Feature Variant](#-lightweight-feature-variant)). This single document replaces the standard planning docs.
...
### 18. Project Motto Document

**Filename**: `project-motto.md`
**Location**: `docs/project-motto.md`
**Purpose**: The project's corrective reference — a short, authoritative DO/DON'T document that keeps AI agents on track. Designed to be re-read in 30 seconds when agents drift or hallucinate.
**Created**: After `project-context.md` and `project-roadmap.md` exist (Step 5 in init). Synthesized from those docs.
**When to update**: When project scope, conventions, or boundaries change.
...
<!-- One sentence — the project's core purpose. Everything you do must serve this. -->

[Single sentence that captures the soul of the project.]

---

## DO ✅

<!-- Concrete patterns and behaviors to follow. 5-10 items max. -->

- [Positive pattern — what agents SHOULD do in this project]
- [Positive pattern — coding style, architecture approach, or workflow behavior]
- [Positive pattern — communication or decision-making rule]
- [Positive pattern]
- [Positive pattern]

## DON'T ❌

<!-- Concrete anti-patterns to avoid. 5-10 items max. Be specific. -->

- [Anti-pattern — what agents must NEVER do in this project]
- [Anti-pattern — common drift or hallucination to guard against]
- [Anti-pattern — scope creep, over-engineering, or wrong assumption]
- [Anti-pattern]
- [Anti-pattern]

## Boundaries 🚧

<!-- Hard limits that must never be crossed. Non-negotiable. -->

- [Hard limit — e.g., "Never modify the database schema without human approval"]
- [Hard limit — e.g., "Never add dependencies not listed in project-context.md"]
- [Hard limit — e.g., "Never bypass auth/validation even for convenience"]

## Success Looks Like 🎯

<!-- How you know you're on track. 3-5 measurable indicators. -->

- [Indicator — e.g., "Every commit compiles and passes tests"]
- [Indicator — e.g., "Code matches the architecture doc"]
- [Indicator — e.g., "Changelog is updated same-session"]
````

> **Key property**: This doc must be short enough to re-read in 30 seconds. If it grows beyond ~40 lines of content, you're putting too much in it — move details to `project-context.md` or `references/`.

---

### 19. Feature Motto Document

**Filename**: `motto.md`
**Location**: `docs/features/XX-feature-name/motto.md`
**Purpose**: The feature's corrective reference — a short, authoritative DO/DON'T document specific to this feature. Created as the final planning artifact after approval, before Build starts.
**Created**: After Stage 4 (Approve), before Stage 5 (Build). If writing the motto reveals planning gaps, fix them first.
**When to update**: If significant deviations occur during Build (update motto to reflect new reality).
...
<!-- One sentence — what this feature MUST achieve. The single most important outcome. -->

[Single sentence that captures the soul of this feature.]

---

## DO ✅

<!-- Concrete patterns for THIS feature. Distilled from discussion + architecture + tasks. -->

- [What to build — the core deliverable]
- [How to build it — the architectural pattern to follow]
- [What quality looks like — testing, validation, style]
- [What to prioritize — if time is tight, focus here]

## DON'T ❌

<!-- Feature-specific anti-patterns. Common ways agents drift on THIS feature. -->

- [Scope creep — what NOT to add even if it seems related]
- [Wrong pattern — architectural approach explicitly rejected in discussion/architecture]
- [Over-engineering — simplicity boundaries]
- [Assumption — thing that seems obvious but is wrong for this feature]

## Boundaries 🚧

<!-- Hard limits specific to this feature. Non-negotiable. -->

- [Hard limit — e.g., "Only modify files listed in architecture.md"]
- [Hard limit — e.g., "No new dependencies beyond what's in tasks.md"]
- [Hard limit — e.g., "Do not refactor existing code outside this feature's scope"]

## Success Looks Like 🎯

<!-- How you know this feature is built correctly. From testplan + acceptance criteria. -->

- [Key acceptance criterion from testplan]
- [Key acceptance criterion from testplan]
- [Architecture alignment indicator]
````

> **Key property**: This doc is the "final briefing" before Build. When told "follow the motto", the agent re-reads it and self-corrects. Keep it under ~40 lines of content. If a feature is too complex for a short motto, the feature may need to be split.
...
- [ ] Plan was approved at Stage 4 before Build started (approval entry exists in `tasks.md` or `changelog.md`)
...
```
 ...
 6. Create  AGENTS.md                                    (at project root, from Template #12)
 7. Create  docs/project-roadmap.md                      (plan the features)
 8. Create  docs/project-changelog.md                    (empty [Unreleased] section)
 9. Create  docs/project-motto.md                        (project guardrails — DO/DON'T)
10. Create  docs/features/                               (empty directory)
11. Create  docs/references/                             (empty directory)
12. Start Feature 01                                     (begin the lifecycle)
```
...
```
 ...
 3. Create  docs/mastery-compact.md                      (copy compact variant)
 4. Create  docs/project-discussion.md                   (reconstruct from codebase + history)
 5. Mark discussion COMPLETE                             (decisions already made)
 6. Create  docs/project-context.md                      (formalize current state)
 7. Create  AGENTS.md                                    (at project root)
 8. Create  docs/features/XX/summary.md                  (one per completed feature)
 9. Create  docs/project-roadmap.md                      (retroactive ✅ + future 🔴)
10. Create  docs/project-changelog.md                    (history of past releases + [Unreleased])
11. Create  docs/project-motto.md                        (project guardrails — DO/DON'T)
12. Start next feature normally                          (full lifecycle from here)
```
...
```
 ...
 2. Read previous feature's review.md → carry forward lessons
 3. Create  docs/features/XX-feature-name/               (create feature folder)
 4. Create  docs/features/XX-feature-name/discussion.md   (discuss — Stage 1)
 5. Discuss until fully understood → mark COMPLETE
 6. Create  docs/features/XX-feature-name/architecture.md (design — Stage 2)
 7. Finalize architecture → mark FINALIZED
 8. Create  docs/features/XX-feature-name/tasks.md        (plan — Stage 3)
 9. Create  docs/features/XX-feature-name/testplan.md     (define done)
10. Create  docs/features/XX-feature-name/api.md          (if has external interfaces)
11. Create  docs/features/XX-feature-name/research.md     (if deep research needed)
12. Create  docs/features/XX-feature-name/changelog.md    (empty, ready)
13. Create  git branch: feature/XX-feature-name
14. Get human approval on plan (Stage 4 — Approve)        (record in tasks.md before Build)
15. Create  docs/features/XX-feature-name/motto.md        (feature guardrails — final briefing)
16. Execute tasks, log in changelog                       (build — Stage 5)
17. Run test plan                                         (verify)
18. Human approval → merge to main, keep branch           (ship — Stage 6)
19. Update README / create release if versioned           (ship)
20. Create  docs/features/XX-feature-name/review.md       (reflect — Stage 7)
21. Update  project-roadmap.md progress tracker           (track)
```

> **Lightweight path**: If the feature meets ALL lightweight eligibility criteria, replace steps 4–21 with a single `lightweight.md`. See [Lightweight Feature Variant](#-lightweight-feature-variant).
...
| Correct a drifting agent (feature) | `docs/features/XX-feature-name/motto.md` |
