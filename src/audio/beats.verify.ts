...
// ── 120 BPM: BPM hit ±1.5, difference between beat point and real beat ≤ 45ms, high reliability ──
{
  ...
  assert.ok(r.confidence >= 2, `(${r.confidence})`);
  assert.ok(r.beats.length >= 24, `(${r.beats.length})`);
  const period = 60 / 120;
  const misses = r.beats.slice(2, 22).filter((t) => {
    const nearest = Math.round(t / period) * period;
    return Math.abs(t - nearest) > 0.045;
  });
  assert.equal(misses.length, 0, `dotalignment(offenders=${misses.length})`);
}
...
// ── Pure noise: credibility gatekeeping, no beats produced ──
{
  ...
  assert.equal(r.beats.length, 0, `noise(bpm=${r.bpm}, conf=${r.confidence})`);
}

// ── Too short/empty input ──
assert.equal(analyzeBeats(new Float32Array(SR), SR).bpm, 0, '1s assetshort');
assert.equal(analyzeBeats(new Float32Array(0), SR).bpm, 0, 'emptyinput');

console.log('beats.verify: ok (120/90 BPM//noise/shortinput)');
