...
assert.equal(spans.length, 1, 'long →');
const [span] = spans;
assert.ok(span!.startMs >= 2000 && span!.startMs <= 2000 + 300, `dotstart+(${span!.startMs})`);
assert.ok(span!.endMs <= 3500 - 100 && span!.endMs >= 3500 - 400, `dot(${span!.endMs})`);

// ── Short pause (< minSilenceMs) without moving ──
assert.equal(spansOf(synth([[0.3, 1000], [0.002, 400], [0.3, 1000]])).length, 0, '400ms');

// ──Customized minSilenceMs takes effect──
assert.equal(spansOf(synth([[0.3, 1000], [0.002, 400], [0.3, 1000]]), { minSilenceMs: 300 }).length, 1, 'lowminimumback 400ms');

// ── Music bed: continuous sound, zero life ──
assert.equal(spansOf(synth([[0.2, 5000]])).length, 0, '');

// ── Pure noise (no voice reference): no segment ──
assert.equal(spansOf(synth([[0.003, 5000]])).length, 0, 'voice →');
...
assert.equal(spansOf(softMid).length, 0, '-16dB');
const deadMid = synth([[0.3, 1500], [0.005, 1500], [0.3, 1500]]);
assert.equal(spansOf(deadMid).length, 1, '-36dB');

// ──Multiple pauses to hit one by one──
const multi = synth([[0.3, 1200], [0.002, 900], [0.3, 1200], [0.002, 800], [0.3, 1200]]);
assert.equal(spansOf(multi).length, 2, '→');

// ── Empty input / illegal window ──
assert.deepEqual(spansOf(new Float32Array(0)), [], 'emptyinput');
assert.deepEqual(detectSilentSpans(new Float32Array(0), 0), [], 'window');

console.log('silence.verify: ok (threshold//shortduration//)');
