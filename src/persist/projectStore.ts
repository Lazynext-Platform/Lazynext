...
export function docFromTimeline(ts: TimelineState, name = ' 1'): ProjectDoc {
 const id = tlId();
 ...
 * Use the back-up pathnormally use loadProject when reading projects.
 */
...
/** Capture and enqueue a projects document; writes for the same project never overlap. */
...
export async function duplicateProject(id: string, name?: string): Promise<ProjectMeta | null> {
 ...
 const copyName = (name?.trim() || `[Copy] ${src?.name ?? 'Project'}`);
