...
// Legacy lazynext-project@1 JSON remains readable for cross-version transfer.
...
export const PROJECT_EXPORT_FORMAT = 'lazynext-project@1';
export const PROJECT_STREAM_FORMAT = 'lazynext-project@2';
...
export function parseProjectEnvelope(
  ...
  if (!migratedDoc) return { error: 'Project data (doc) validation failed' };
  const doc = sanitizePortableProjectDoc(migratedDoc);
  if (!Array.isArray(r.media)) return { error: 'Project package is missing media manifest' };
  if (!r.media.every(isMediaEntry)) return { error: 'Project package media entry validation failed' };
  const media = r.media;
...
export async function buildProjectExport(id: string, name: string): Promise<ProjectExportResult> {
  ...
  const blob = await new Response(stream, {
    headers: { 'Content-Type': 'application/x-lazynext-project' },
  }).blob();
  ...
  try {
    for (const entry of envelope.media) {
      ...
      if (sameTarget && sameTarget.sha256 !== staged.sha256) {
        throw new Error(`Media safe-name conflict in project package: ${staged.src}`);
      }
      if (!sameTarget) {
    ...
    try {
      ...
      throw new AggregateError([error, cleanupError], 'Project package parsing failed, and temporary media cleanup also failed');
    }
    throw error;
  }
}
...
function streamManifest(
  ...
  if (!migratedDoc) throw new Error('Project data (doc) validation failed');
  const doc = sanitizePortableProjectDoc(migratedDoc);
...
async function publishStreamProjectImport(
  ...
  if (!mediaImport) throw new Error('Project package temporary media manifest is missing');
  ...
  try {
    ...
    try {
      ...
      throw new AggregateError([error, rollbackError], 'Project publication failed, and media rollback or temporary cleanup also failed');
    }
    throw error;
  }
  try {
