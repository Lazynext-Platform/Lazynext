...
async function* textLines(blob: Blob): AsyncGenerator<string> {
 ...
 while (newline >= 0) {
 if (newline > MAX_STREAM_LINE_CHARS) throw new Error('');
 ...
 if (pending.length > MAX_STREAM_LINE_CHARS) throw new Error('');
...
async function finishMediaEntry(
 ...
 if (current.bytes !== current.entry.bytes) throw new Error(`: ${current.entry.name}`);
...
class StreamImportState {
 ...
 if (!this.manifest) throw new Error(' manifest');
 if (this.current) throw new Error('');
...
export async function stageProjectStream(
 ...
 if (!line) continue;
 let record: unknown;
 try { record = JSON.parse(line); } catch { throw new Error('Invalid JSON'); }
 ...
 catch (cleanupError) { throw new AggregateError([error, cleanupError], 'ok'); }
 throw error;
 }
}
