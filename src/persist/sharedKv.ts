...
const STORE = 'kv';
const MIGRATION_KEY = '__lazynext_shared_store_v1__';
const PENDING_KEYS_KEY = '__lazynext_shared_pending_v1__';
...
async function localAgentRuntimeWrite(
 ...
 // CAS removed: local writes are serialized by the callers enqueue/lock;
...
async function setProjectDocument(key: string, value: unknown): Promise<void> {
 ...
 // semantics as kvGets local read fallback. Never hard-fail the editor.
 await localSet(key, value);
...
export async function kvDel(key: string): Promise<void> {
 ...
 if (!remoteCache) {
 if (requireSharedDelete) throw new Error('Unavailable');
 await localDel(key);
 return;
 }
