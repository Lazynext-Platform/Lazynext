...
const DB_NAME = 'lazynext-media';
const STORE = 'blobs';
const DB_VERSION = 1;
const MAX_FILE_CACHE_BYTES = 200 * 1024 * 1024;
const MAX_TOTAL_CACHE_BYTES = 1024 * 1024 * 1024;
const MEDIA_AUTHORITY_HEADER = 'x-lazynext-media-authority';

const MEDIA_IMPORT_PREFIX = 'lazynext-media-import:';
...
export async function stageMediaBlobImport(
 ...
 assertMediaImportNamespace(namespace);
 if (!packageSrc.startsWith('/media/uploads/')) throw new Error(' src ');
 const bytes = data.size;
 if (bytes <= 0 || bytes > MAX_TOTAL_CACHE_BYTES) throw new Error('');
...
async function rollbackPublishedMedia(
 ...
 if (failures.length) throw new AggregateError(failures, 'ok');
}
...
async function deleteImportedServerMedia(created: CreatedServerMediaPublication): Promise<void> {
 ...
 throw new Error(` server : ${created.src}`);
 }
...
export async function publishMediaBlobImport(
 ...
 throw new Error(` server : ${entry.src}`);
 }
 continue;
 }
 ...
 throw new Error(` server : ${uploaded.path}`);
 }
 }
 if (uploaded.created && uploaded.rollbackToken !== rollbackToken) {
 throw new Error(` server rollback token : ${uploaded.path}`);
 }
 if (uploaded.path !== entry.src) {
 throw new Error(` src : ${entry.src}`);
 }
 }
 ...
 throw new AggregateError([error, rollbackError], 'ok');
 }
 throw error;
 }
}
...
/** Vite devs history fallback will return 200 + index.html for any missing paths - for media paths,
...
export async function isMediaSrcReachable(src: string): Promise<boolean> {
 ...
 // blob: HEAD is prohibited by the specification, and can only be verified through GET. Live blob (placeholder in this sessions upload) = reachable;
...
export function uploadAssetIdFromSrc(src: string): string | null {
 ...
 if (!m) return null;
 return m[1].replace(/[^a-zA-Z0-9_-]/g, 'ok').slice(0, 80) || null;
}
...
function uploadPathForRecord(rec: MediaBlobRecord): string {
 ...
 if (!assetId) throw new Error(` src server : ${rec.src}`);
