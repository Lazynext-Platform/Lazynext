...
// ── denoisedSrc must be counted as a reference ──
{
  ...
  assert.ok(refs.includes('/media/uploads/raw.wav'), 'sourceasset');
  assert.ok(
    refs.includes('/media/uploads/clean.wav'),
    'isolate_voice denoisedSrc exist,cleanupplay',
  );
...
// ── Boundaries: null value, illegal name, query string, repetition ──
{
  ...
  assert.deepEqual(rawUploadSrcs('/media/uploads/x.mp4"'), ['/media/uploads/x.mp4'], 'string');
  assert.deepEqual(
...
// ── It is better to keep more than delete by mistake: the original scan is a superset of collectUploadSrcs ──
{
  ...
  for (const src of collectUploadSrcs(doc)) {
    assert.ok(raw.has(src), `${src},pathnormalpath`);
  }
}

console.log('uploadRefs.verify: ok (denoisedSrc ///)');
