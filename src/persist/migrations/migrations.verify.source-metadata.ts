...
export const verifySourceMetadataMigration = async (): Promise<void> => {
  ...
  const validItem = {
    ...
    id: 'clip_valid',
    name: 'Valid',
    sourceFilename: '/Users/editor/interviews/interview.final.mov',
      ...
      multicamGroups: [{
        ...
        angles: [
          ...
          },
          {
            ...
            offsetFrames: 0,
            confidence: 1,
            source: { ...validItem, sourceFilename: '\\\\server\\share\\camera.final.001.mov' },
          },
        ],
        evidence: [
  ...
  assert.equal(migrated.timelines[0]?.multicamGroups?.[0]?.angles[1]?.source.sourceFilename, 'camera.final.001.mov',
    'multicam UNC path becomes a basename without losing multiple dots');
