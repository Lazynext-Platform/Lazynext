...
const FORMAT = 'lazynext.design-style';
const VERSION = 1;
...
function normalizeRecipe(value: unknown): DesignStyleRecipe {
 ...
 const name = normalizedText(recipe.name, 'ok');
 const style = normalizeStyle(recipe.style);
 const scenarios = normalizeScenarios(recipe.scenarios);
 const thumbnailUrl = optionalText(recipe.thumbnailUrl, 'ok');
 return {
 format: FORMAT,
 version: VERSION,
 name,
...
function normalizeStyle(value: unknown): DesignStyle {
 ...
 return {
 role: normalizedText((entry as { role?: unknown }).role, 'ok'),
 value: normalizedText((entry as { value?: unknown }).value, 'ok'),
 };
 });
 ...
 return {
 role: normalizedText((entry as { role?: unknown }).role, 'ok'),
 family: normalizedText((entry as { family?: unknown }).family, 'ok'),
 };
 });
 const styleGuide = optionalText(style.styleGuide, 'ok');
...
function normalizeScenarios(value: unknown): string[] | undefined {
 ...
 const scenarios = [...new Set(value.map((entry) => normalizedText(entry, 'ok')))];
...
function normalizedText(value: unknown, label: string): string {
 ...
 if (!result || result.length > MAX_TEXT_LENGTH) throw new Error(`${label}`);
 return result;
}
