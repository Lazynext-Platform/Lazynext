...
// media bytes follow in bounded chunk records rather than one in-memory media[].
{
 ...
 resetMediaBlobMemory();
 const originalFilePath = '/Users/private-editor//.bin';
 ...
 assert.equal(manifestDoc.assets[0]?.sourceFilename, '.bin', 'portable original filenames remain available');
 assert.equal(manifestDoc.assets[0]?.originalFilePath, undefined);
 assert.equal(manifestDoc.timelines[0]?.items[0]?.sourceFilename, '.bin');
 ...
 assert.equal(manifestDoc.timelines[0]?.multicamGroups?.[0]?.angles[0]?.source.sourceFilename, '.bin');
 ...
 assert.equal(manifestBeforeDoc.assets[0]?.sourceFilename, '.bin');
