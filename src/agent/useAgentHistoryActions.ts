...
export async function clearAgentHistory(state: AgentHookState, projectId: string): Promise<void> {
  if (state.runningRef.current) {
    ...
    state.setMessages((current) => [...current, {
      role: 'error',
      text: t('Agent is still running; cannot clear chat. Wait for the run to finish or stop the current run, then try again.'),
    }]);
    return;
  }
  ...
  try {
    ...
    state.setMessages((current) => [...current, {
      role: 'error',
      text: `${t('Unable to clear context and run history.')}${detail}`,
    }]);
    return;
  }
