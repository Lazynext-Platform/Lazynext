...
async function settleStaleRecovery(
  ...
  try {
    ...
    state.appendMessage({
      role: 'error',
      text: `The previous server-side run was safely interrupted and recovery state cleared.${detail
    ...
    state.appendMessage({
      role: 'error',
      text: `The server-side run could not be safely recovered; recovery credentials were retained: ${error instanceof Error
...
async function openSubscription(
  ...
  if (typeof metadata.firstEventId === 'number' && cursor < metadata.firstEventId - 1) {
    throw permanentServerRunRecoveryError('Server-side run events are outside the recoverable window.');
  }
