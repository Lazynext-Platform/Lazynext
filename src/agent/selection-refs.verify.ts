...
assert.equal(refPromptToken({ name: '00:05.2 dot', kind: 'timepoint' }), '@t[00:05.2 dot]');
assert.equal(refPromptToken({ name: 'V1 00:03.0-00:07.0', kind: 'timerange' }), '@t[V1 00:03.0-00:07.0]');
assert.equal(refPromptToken({ name: '（2 clip）', kind: 'canvas-region' }), '@r[（2 clip）]');
assert.equal(refPromptToken({ name: '“…”（8 ）', kind: 'transcript-selection' }), '@q[“…”（8 ）]');
assert.equal(refPromptToken({ name: 'A.mp4', kind: 'item' }), '@[A.mp4]');
...
const clip: TimelineItem = { id: 'item_1', track: 'V1', startFrame: 30, durationInFrames: 120, name: 'A', kind: 'video', src: '/m/a.mp4' };
...
const laterClip: TimelineItem = { id: 'item_3', track: 'V2', startFrame: 200, durationInFrames: 50, name: 'back', kind: 'video', src: '/m/c.mp4' };
const hiddenClip: TimelineItem = { id: 'item_4', track: 'V2', startFrame: 0, durationInFrames: 300, name: 'hidden', kind: 'video', src: '/m/d.mp4' };
...
// ── transcript selection: →sourcemedia ms + →frame (keptSegments source) ──────────
