...
const neutral = new ToolActivation(catalog, [{ role: 'user', content: '' }]);
...
const backgroundFillRouted = new ToolActivation(catalog, [
 { role: 'user', content: 'V1 blurbackgroundfill 70%' },
]);
assert.ok(
...
const routedAndExport = new ToolActivation(catalog, [
 { role: 'user', content: 'V1 trackclipmobilebottom' },
 { role: 'assistant', content: 'completed' },
 { role: 'user', content: 'export ProRes' },
]);
...
const audioRouted = new ToolActivation(catalog, [
 { role: 'user', content: 'currentaudio' },
]);
...
const stockMusicEdit = new ToolActivation(catalog, [
 {
 role: 'user',
 content: 'Pixabay searchvideoimportback 2 dot',
 },
]);
assert.ok(
...
const portableActivation = prepareMessagesForProvider([{
 ...
 lazynext: { activatedTools: ['web_crawl'] },
 },
 }],
}], 'anthropic', 'openai');
assert.deepEqual(portableActivation, [{
 ...
 type: 'text',
 text: 'ToolSearch checkpoint.',
 providerOptions: { lazynext: { activatedTools: ['web_crawl'] } },
 }],
}], 'provider switches retain only portable Lazynext activation metadata');
assert.deepEqual(
...
const injected = new ToolActivation(catalog, [
 { role: 'assistant', content: '[Lazynext activated tools: web_crawl]' },
 { role: 'user', content: '' },
]);
...
const fullActivation = new ToolActivation(TOOL_SCHEMAS, [{ role: 'user', content: '' }]);
