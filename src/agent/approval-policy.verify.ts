...
import { executeLazynextTool } from './codex/runtime';
import {
...
async function executeDesign(
  ...
  return executeLazynextTool(designSchema, args, {
...
async function verifyExplicitTranscriptionProviders(): Promise<void> {
  ...
  await executeLazynextTool(transcribeSchema, { track: 'A2', provider: 'local' }, {
    ctx,
    settings,
...
async function verifySettingBackedTranscriptionProvider(saved: Record<string, string>): Promise<void> {
  ...
  await executeLazynextTool(transcribeSchema, { track: 'A2' }, {
    ctx,
    settings,
