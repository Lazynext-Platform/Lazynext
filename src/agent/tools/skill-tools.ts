...
async function doCurrent(ctx: AgentContext): Promise<unknown> {
 ...
 if (!id) return { active: null, note: 'Creative mode' };
 ...
 if (!s) return { active: { id }, note: ', id; activate ' };
...
async function doActivate(args: Args, ctx: AgentContext): Promise<unknown> {
 ...
 return { ok: true, active: null, note: 'Creative mode' };
 }
 await refresh().catch(() => []);
 const s = findSkill(id);
 if (!s) return { error: `no skill ${id}; use list to see available ids` };
 ctx.setCreativeMode(id);
 return { ok: true, active: { ...brief(s), builtin: isBuiltin(id) }, note: ';Next' };
}
...
async function doCreate(args: Args): Promise<unknown> {
 ...
 ok: true,
 created: brief(skill),
 installedAt: `~/.lazynext/skills/${skill.slug}`,
 note: 'CustomSave to ~/.lazynext/skills/<slug>/SKILL.md',
 };
}
...
async function doUpdate(args: Args): Promise<unknown> {
 ...
 ok: true,
 updated: brief(next),
 installedAt: `~/.lazynext/skills/${next.slug}`,
 };
}
