...
export function validateShaderSource(glsl: string): string | null {
  const src = glsl.trim();
  if (!src) return 'Generated shader is empty';
  ...
  if (!/\bmain\b/.test(src)) return 'Shader missing main() entry point';
  if (!/fragColor|gl_FragColor/.test(src)) return 'Shader must write color (fragColor / gl_FragColor)';
  ...
  if (unknown.length) return `Unknown samplers (runtime only provides u_input): ${unknown.join(', ')}`;
  return null;
}
...
/** Static verification for transition shader (double input contract). Returns null if valid, otherwise the reason. */
export function validateTransitionShaderSource(glsl: string): string | null {
  ...
  if (unknown.length) return `Unknown samplers (runtime only provides u_outgoing / u_incoming): ${unknown.join(', ')}`;
  return null;
}
