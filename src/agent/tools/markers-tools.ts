...
function resolveTranscriptSegments(state: TimelineState, spec: string, trackFilter?: string): SegmentAnchor | { error: string } {
  ...
  if (!sns) return { error: `transcriptSegments "${spec}" could not be parsed — use the [sN] numbers from read_script output, e.g. "3", "3-5" or "2,4-6"` };
  ...
  if (!candidates.length) {
    return { error: `No transcribed region contains all of segments ${sns.join(',')} — run read_script to verify [sN] numbers${trackFilter ? '' : ', or pass transcriptTrack to narrow the scope'}` };
  }
  if (candidates.length > 1) {
    return { error: `Segments ${sns.join(',')} appear in multiple transcribed regions (${candidates.map((c) => `${c.track}:${c.itemId.slice(0, 8)}`).join(' / ')}) — pass transcriptTrack to disambiguate, or provide fromFrame directly` };
  }

  const cand = candidates[0]!;
  const item = state.items.find((it) => it.id === cand.itemId);
  if (!hasOperationalTranscript(item)) return { error: `The clip ${cand.itemId} for the transcribed region no longer has a current transcript; run read_script again` };
  ...
  if (!f0 || !f1) return { error: `The words for segments ${sns.join(',')} have been deleted or are outside the playback range; cannot locate frames — verify with read_script and retry` };
