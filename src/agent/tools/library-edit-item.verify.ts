...
// ── 2. browse_library modes ────────────────────────────────────────────────
{
  ...
  assert.ok(root.categories.transitions >= 12, `browse root transitions ≥12 builtin (got ${root.categories.transitions}; clone extension)`);
...
// ── 4b. edit_item plugin: transition assetId(pluginregister custom registertable) ────
{
  ...
  registerCustomTransition({
    ...
    props: [{ key: 'soft', label: '', default: 0.3, min: 0, max: 1 }],
  });
  const d = makeDraft(base);
  const ctx = ctxOf(d);
  const r = await execEditItemTool('edit_item', {
  ...
  assert.ok(t.customFrag?.includes('u_progress'), 'frag snapshottop item');
  assert.strictEqual(t.customLabel, '');
...
// ── 4c. edit_item scale:plugincurve assetId + raw envelope + shape/envelope mutex ──
{
  const { registerCustomZoom, __resetCustomZooms } = await import('../../editor/customZooms');
  registerCustomZoom({ id: 'plugin:demo/elastic', label: '', envelope: [0, 1.1, 1], magnification: 1.6 });
  const d = makeDraft(base);
  const ctx = ctxOf(d);
  // plugincurve(propertyOverrides.magnification packageinnerdefault)
  ...
  assert.deepStrictEqual(z?.envelope, [0, 1.1, 1], 'packagesnapshottop item');
  assert.strictEqual(z?.magnification, 2, 'overrides');
  assert.strictEqual(z?.label, '');
  // agent curve:builtin:zoom + envelope (cleanOverrides )
  ...
  assert.strictEqual(z?.shape, undefined, 'envelope shape');
  // update shape → curve
  r = await execEditItemTool('edit_item', {
  ...
  assert.strictEqual(z?.envelope, undefined, 'shape envelope');
  // package( state)
  r = await execEditItemTool('edit_item', {
...
// ── 4d. fx register/register(ALL_FX /;inner) ─────────────────────
{
  ...
  assert.ok('plugin:p/f' in ALL_FX, 'register ALL_FX');
  assert.strictEqual(unregisterCustomFx('plugin:p/f'), true);
  assert.ok(!('plugin:p/f' in ALL_FX), 'registerback ALL_FX');
  assert.strictEqual(unregisterCustomFx('builtin:fx-invert'), false, 'inner');
  assert.ok('builtin:fx-invert' in ALL_FX, 'inner');
  console.log('fx register/register: OK');
}
