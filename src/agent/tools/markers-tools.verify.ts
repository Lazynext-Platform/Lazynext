...
// ── transcriptSegments():[sN] marker, read_script ──
// A1: "hello world." + "nice day."();V1: "hello again."(,startFrame 100)
{
 ...
 // + notePrefix:s2 = "nice day." → fromFrame 45(=1.5s@30fps),covering 27f
 {
 const { d, c } = mk();
 ...
 assert.equal(m.fromFrame, 45, 'fromFrame derived from the segment\'s first word ()');
 ...
 // : s1 → transcriptTrack; track V1 clip
 {
 const { d, c } = mk();
