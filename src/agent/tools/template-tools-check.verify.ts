...
// save currentpackagetemplate → get(list) columnget(detail) doc(MG + );
// list_assets ;apply applyDoc atomiccommit,resulttemplate
// MG clip;omitAssetIds clip;placement=replace inner;
// templateId → 
// template templateStore IndexedDB helper ,inner IndexedDB shim
// ( src/persist/version-store.check.ts), store module import before
