...
const state: TimelineState = {
 ...
 items: [
 { id: 'talk', track: 'V1', startFrame: 0, durationInFrames: 105, name: 'test', kind: 'video', src: '/talk.mp4', transcript: words },
 { id: 'ov', track: 'V2', startFrame: 60, durationInFrames: 30, name: 'MG', kind: 'motion-graphic', code: '' },
 { id: 'far', track: 'V2', startFrame: 200, durationInFrames: 30, name: 'MG', kind: 'motion-graphic', code: '' },
 ],
};
...
// ── 1) validateHighlights :/start>end/ ────────────────────
assert.strictEqual(validateHighlights({ nope: true }, 8, 5).length, 0, ' → ');
assert.strictEqual(
...
assert.strictEqual(overlap.length, 1, 'ok');
assert.deepStrictEqual([overlap[0].startWordIndex, overlap[0].endWordIndex], [0, 3], 'ok');
assert.strictEqual(
...
// ── 2) (stub LLM):canned JSON , ──────────
setHighlightSelector(async () => [
 { startWordIndex: 0, endWordIndex: 1, title: 'test' },
 { startWordIndex: 0, endWordIndex: 99, title: 'test', reason: 'bad' }, // out-of-range → dropped, not a crash
 { startWordIndex: 2, endWordIndex: 6, title: 'test', reason: '' },
]);
...
assert.strictEqual(res.shorts.length, 2, '3 → 2 ');
...
assert.strictEqual(doc.activeTimelineId, originalActiveId, 'activate:false → ');
assert.strictEqual(doc.timelines.length, 3, ' + 2 ');

// 9:16 (cover)
for (const s of res.shorts) {
 const tl = doc.timelines.find((t) => t.id === s.timelineId)!;
 assert.deepStrictEqual([tl.width, tl.height, tl.fit], [1080, 1920, 'cover'], `${s.title} 9:16 cover`);
}
...
// ,/; deletedWordIdx 
assert.strictEqual(talk.transcript!.length, 8, '(),↔');
assert.deepStrictEqual([...(talk.deletedWordIdx ?? [])].sort((a, b) => a - b), [0, 1, 7], '(0,1,7)');
assert.strictEqual(talk.transcript![2].text, 'this', 'ok');
assert.strictEqual(talk.transcript![2].start, 1000, ' ms ');
assert.strictEqual(talk.transcript![6].end, 3000, ' ms ');
assert.deepStrictEqual(talk.transcript!.map((w) => w.text), words.map((w) => w.text), 'ok');
// 0, = 
assert.strictEqual(talk.startFrame, 0, ' 0');
assert.strictEqual(talk.durationInFrames, 60, 'clip = spanEnd-spanStart = 90-30');

// clip: MG ; MG 
const ov = short.items.find((it) => it.id === 'ov')!;
assert.deepStrictEqual([ov.startFrame, ov.durationInFrames], [30, 30], ' MG -spanStart');
assert.strictEqual(short.items.find((it) => it.id === 'far'), undefined, ' MG ');
assert.strictEqual(short.items.length, 2, ' clip');

// [0,1] : MG → 
const opening = doc.timelines.find((t) => t.id === res.shorts[0].timelineId)!;
assert.strictEqual(opening.items.length, 1, ' clip');
assert.strictEqual(opening.items[0].durationInFrames, 30, ' = ');
