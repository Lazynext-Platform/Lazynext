...
async function manageTranscript(args: Args, ctx: AgentContext, track: TrackId, alias: string): Promise<unknown> {
 ...
 // retry_transcription: force a fresh ASR run (the only action that doesnt need an existing transcript).
...
// Execute a transcript/caption tool. Returns undefined if `name` isnt one of ours.
export async function execTranscriptTool(name: string, args: Args, ctx: AgentContext): Promise<unknown | undefined> {
 ...
 ? ' → → whisper DownloadedSettings → Transcription → Local models'
 : '';
 ...
 // When only changing the breathing port, keep the existing compression settings of the clip as they are, so they wont be cleared by cleanScript.
