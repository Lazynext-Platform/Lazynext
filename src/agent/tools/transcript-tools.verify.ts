...
// ── 6-action:list / read / ensure() / create·read lang / retry ──
// en ( LLM),;translation_ensure 
const varWords: TranscriptWord[] = [
 { text: '', start: 0, end: 300, speaker: 'A' },
 { text: '', start: 300, end: 600, speaker: 'A' },
];
...
};
{
 ...
 // translation_ensure: lang → , LLM(reused:true)
 ...
 const noVar = await execTranscriptTool('manage_transcript', { action: 'translation_read', itemId: 'clip', lang: '' }, c) as { error?: string };
 assert.ok(noVar.error, 'reading a missing variant errors');
}

// clean_script 」— clip,(E2E bug)
{
...
// ── find_transcript :fuzzy / includeWordTimestamps / limit / asset ──
{
 // "we use davinci um resolve daily" @30fps 'um' query token 
 ...
 // + (itemId/wordStart/wordCount/text/fromFrame/toFrame )
 ...
 // →:clip startFrame kept ,davinci 400ms → 60 + 12
 ...
 // fuzzy:token ,span davinci..resolve
 ...
 // includeWordTimestamps:match Words ( + )
...
// retry_transcription media src → error()
{
