...
export async function verifyUploadImportSession(fixture: UploadVerifierFixture): Promise<void> {
 ...
 assert.equal(safeSourceFilename('/Users/editor//..001.MOV'), '..001.MOV');
 assert.equal(safeSourceFilename('D:\\capture\\..001.MOV'), '..001.MOV');
 assert.equal(safeSourceFilename('\\\\server\\share\\..001.MOV'), '..001.MOV');
 ...
 assert.equal(slot.filename, '..png');
 ...
 assert.equal(replacedAsset?.name, '.jpg');
