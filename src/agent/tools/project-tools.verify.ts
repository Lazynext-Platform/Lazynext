...
const su = await execProjectTool('edit_project', { action: 'speaker-update', from: 'A', to: '' }, spCtx) as { ok?: boolean; itemsChanged?: number; wordsChanged?: number };
...
assert.strictEqual(st.items.find((i) => i.id === 'c1')!.transcript!.find((w) => w.text === 'hi')!.speaker, '', 'A→ applied');
...
const suById = await execProjectTool('edit_project', { action: 'speaker-update', id: 'B', to: '' }, spCtx) as { ok?: boolean; from?: string; wordsChanged?: number };
...
assert.strictEqual(spDraft.getState().items.find((i) => i.id === 'c1')!.transcript!.find((w) => w.text === 'yo')!.speaker, '', 'B→ via id');
