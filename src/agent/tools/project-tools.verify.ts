// Runnable: `npx tsx src/agent/tools/project-tools.check.ts`（ verify:agent-tools）
...
const su = await execProjectTool('edit_project', { action: 'speaker-update', from: 'A', to: 'C' }, spCtx) as { ok?: boolean; itemsChanged?: number; wordsChanged?: number };
...
assert.strictEqual(st.items.find((i) => i.id === 'c1')!.transcript!.find((w) => w.text === 'hi')!.speaker, 'C', 'A→C applied');
...
const suById = await execProjectTool('edit_project', { action: 'speaker-update', id: 'B', to: 'D' }, spCtx) as { ok?: boolean; from?: string; wordsChanged?: number };
...
assert.strictEqual(spDraft.getState().items.find((i) => i.id === 'c1')!.transcript!.find((w) => w.text === 'yo')!.speaker, 'D', 'B→D via id');
