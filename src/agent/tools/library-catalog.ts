...
export function buildLibraryItems(templates: Tpl[]): LibraryItem[] {
  ...
  items.push({
    ...
    group: 'voice',
    usage: 'isolate_voice itemId=<clip> action=apply strength?=70 — not edit_item (per-clip denoise, not a library place). action=clear removes. Library UI: Library → Audio FX.',
  });
  items.push({
