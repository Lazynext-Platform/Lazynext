...
// parts (per-source positions/priority/slots) this single-stream model cant hold.
...
function selectorToEntry(sel: Record<string, unknown>, s: TimelineState): CaptionSourceEntry | { error: string } {
 ...
 if (vKind || vLang) {
 if (vKind && vKind !== 'translation') return { error: `variantKind ${vKind} Unsupported( translation)` };
 if (!vLang) return { error: 'variant languageCode()' };
 const item = s.items.find((it) => it.id === itemId);
 if (!item?.variants || !findVariantByLang(item.variants, vLang, 'translation')) {
 return { error: `item ${itemId.slice(0, 8)} ${vLang} manage_transcript translation_ensure` };
 }
...
/** source_list current scope + layout policy + visual order + whats available. */
export function sourceList(c: CaptionsData, s: TimelineState): Result {
 ...
 note: 'auto-stack sources ();per-source / positions / source_update',
 };
}
...
export function sourceSet(json: Record<string, unknown>, c: CaptionsData, ctx: AgentContext, s: TimelineState): Result {
 ...
 return { ok: true, sourceMode: 'item', sources: null, note: 'cleared sourceItemId' };
 }
