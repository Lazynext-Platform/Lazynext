...
const psave = await execCaptionsTool('edit_captions', { action: 'preset_save', presetName: '' }, ctx3) as { ok?: boolean; presetId?: string };
...
assert.ok(plist.presets.some((p) => p.id === psave.presetId && p.name === ''), 'preset_list shows the saved preset');
