...
// :① paginate/applyWordOverrides (hidden///none);
// ② edit_captions action=display_text makeDraft updateCaptions,read_captions ;
// ③ source(resolveCaptionWords sort);④ edit_captions action=source_*(selectparse/
// /timeline);⑤ action:enable/disable、template columntable+、style→styleOverride(sizePx→
// 、highlightBackground object→、pacing route、map ignored)、layout dot、
// language_mode;(positions/layout_policy/source_update)( captions-lanes.check.ts)。
...
// none:(,arraynew),paginationoutput"before"
{
...
// ── 2) execCaptionsTool makeDraft/updateCaptions ────────────────
...
// edit_captions action=display_text:hidden idx1, idx2 ,idx3 (forcePageBreak)
...
// read_captions after:idx1 column(hidden visible, agent cancelhidden),idx2
...
// ( B1):text:null override = no-op, TypeError
...
// captions enable read_captions ,
// (docFromTimeline captions caption track,close)
...
// ── 3) source:resolveCaptionWords/resolveCaptionWordIndices ──────────
// fps=1000 frame ms (msToFrame(ms,1000)===ms),value、dot。
...
// sourcepath(none sources/sourceMode):"front"allpath。
{
...
// sources:['a','b'] → transcript,start timesort(concat:b first a )
{
...
// sourceMode:'timeline' → "alltranscript item"(c transcript,auto)
{
...
// video source:windowinner""——video play [srcIn,srcIn+dur),transcript
// hiddencaption( agent srcIn windownarrow,shortcaption;
// 2026-07-17 longshort)。caption wordOverrides, deletedWordIdx。
{
  const vid = {
    ...
    transcript: [
      { text: 'lead', start: 60, end: 80 },   // windowinner、 → ()
      { text: 'kept', start: 90, end: 110 },  // windowinner、
      { text: 'out', start: 200, end: 220 },  // outer(win=[50,150))
    ],
  };
  ...
  assert.deepEqual(words.map((w) => w.text), ['lead', 'kept'], 'video windowinner(),outer');
  assert.deepEqual(words.map((w) => Math.round(w.start)), [10, 40], 'mediaframe − srcIn + startFrame ,');
  assert.deepEqual(resolveCaptionWordIndices(capV, [vid], 1000), [0, 1], 'index(wordOverrides )');
}

console.log('captions-tools.check: multi-source merge + video ok');

// ── 4) edit_captions action=source_* :validate + + read_captions result ──
...
// sources(selectarray)→ item ids + wordCount back
...
// read_captions result:、start timesort
...
// source_add ;source_remove itemId (lane ids are random; itemId is stable)
...
// ── 5) new action:enable/disable · template · style(→styleOverride) · layout · unsupported ──
...
// enable:none captions new(transcriptsource)
...
// template none → columnallinner(21 source + default); → template,
...
// style:sizePx→fontSize 、color、highlightBackground styleOverride;maxLines not support→ignored
...
// layout:dot top-center + offset → CaptionsData.layout
...
// language_mode original(none LLM);translation none →
...
// (captions-lanes.ts; captions-lanes.check.ts)——
...
const psave = await execCaptionsTool('edit_captions', { action: 'preset_save', presetName: 'My Preset' }, ctx3) as { ok?: boolean; presetId?: string };
...
assert.ok(plist.presets.some((p) => p.id === psave.presetId && p.name === 'My Preset'), 'preset_list shows the saved preset');
...
console.log('captions-tools.check: actions (enable/template/style/layout/) ok');
