...
/** Source-seconds window of the video clip covered by the graphics frames. */
...
export async function execPlaceGraphicsTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
 ...
 return { error: ` clip ${requested}motion-graphic/text/solid`, available: graphics.map((g) => ({ itemId: g.id, name: g.name, kind: g.kind })) };
 }
 if (!targets.length) {
 return { ok: true, adjusted: 0, note: 'Timelinemotion-graphic/text/solid' };
 }
 ...
 if (!geometry) {
 skipped.push(`${item.name}Unavailable`);
 continue;
 }
