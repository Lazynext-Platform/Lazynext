...
export function matchEntries(entries: CaptionSourceEntry[], sel: Json, s: TimelineState): number[] | { error: string } {
  ...
  if (id) {
    ...
    return hits.length === 1
      ? hits
      : { error: hits.length ? `ambiguous sourceId "${id}"` : `no source with id "${id}" (use source_list to find sourceId)` };
  }
  ...
  if (str(sel.speakerId)) return { error: 'speakerId selector unsupported: no per-speaker lanes; select by track or item instead' };
  ...
  return { error: 'Missing selector: each entry needs one of index / sourceId / trackId / itemId / label / variant to locate the lane, e.g. {"index":0} or {"trackId":"A2"} or {"variant":{"languageCode":"en"}}; use source_list to find sourceId' };
}
...
export function execLayoutPolicy(json: Json, c: CaptionsData, ctx: AgentContext): Result {
  if (json.layoutPolicy === null) {
    ...
    return { ok: true, layoutPolicy: null, note: 'cleared — back to default auto-stack' };
  }
  ...
  if (mode) {
    if (mode === 'single-lane' || mode === 'auto-stack') {
      ...
      if (!raw?.length) return { error: 'manual-slots requires a slot table, e.g. {"mode":"manual-slots","slots":[{"id":"top","anchor":"top-center","offsetYRatio":0.08},{"id":"bottom","anchor":"bottom-center","offsetYRatio":-0.08}]}; then use source_update to pin lanes to slots' };
      const slots: CaptionSlot[] = [];
      for (const sl of raw) {
        const o = (sl ?? {}) as Json;
        const sid = str(o.id);
        const anchor = str(o.anchor);
        if (!sid || !ANCHORS.has(anchor)) return { error: `invalid slot: ${JSON.stringify(sl)} (needs id + 3×3 anchor)` };
  ...
  if (!('layoutPolicy' in patch) && !('perSource' in patch)) return { error: 'layout_policy params example: {"mode":"auto-stack","maxVisibleSources":2} (stacked top/bottom) / {"mode":"single-lane"} (only one shown at a time) / {"mode":"manual-slots","slots":[…]} / {"perSource":{"<sourceId>":{"maxLines":2}}} / {"layoutPolicy":null} to clear' };
  ctx.commands.updateCaptions(patch);
  return { ok: true, layoutPolicy: patch.layoutPolicy ?? c.layoutPolicy ?? { mode: 'auto-stack' }, ...(patch.perSource ? { perSource: patch.perSource } : {}), note: 'perSource.maxLines approximates maxLines×template words-per-page (pagination by word count)' };
}
...
export function execPositions(json: Json, c: CaptionsData, ctx: AgentContext, s: TimelineState): Result {
  ...
  if (!raw?.length) return { error: 'positions params example (copy and edit values): {"positions":[{"index":0,"anchor":"top-center","offsetYRatio":0.08},{"index":1,"anchor":"bottom-center","offsetYRatio":-0.08}]} — each entry = selector (index/sourceId/trackId/variant…) + anchor (3×3); same anchor stacks into one block' };
  const entries = ensureEntries(c, s);
  if (!entries.length) return { error: 'No caption sources yet: enable captions with edit_captions action=enable (or specify sources with source_set) before positioning' };
  const placed: Result[] = [];
  for (const p of raw) {
    const o = (p ?? {}) as Json;
    const anchor = str(o.anchor);
  ...
  return { ok: true, placed, note: 'Multiple sources with the same anchor stack as one normal caption block at that anchor; for pixel-level left/top use action=layout (whole block)' };
}
...
export function execSourceUpdate(json: Json, c: CaptionsData, ctx: AgentContext, s: TimelineState): Result {
  ...
  if (!raw?.length) return { error: 'source_update params example (copy and edit values): {"updates":[{"index":0,"anchor":"bottom-center","offsetYRatio":-0.08},{"trackId":"A2","visible":false},{"index":1,"style":{"sizePx":54,"color":"#fff"}}]} — each entry = selector + fields to change (visible/anchor/offsetXRatio/offsetYRatio/slotId/style/preset/variant); use source_list to find sourceId' };
  let entries = ensureEntries(c, s);
  if (!entries.length) return { error: 'No caption sources yet: enable captions with edit_captions action=enable (or specify sources with source_set)' };
  ...
  for (const u of raw) {
    ...
    for (const sourceId of matchedIds) {
      ...
      if (vKind || vLang) {
        if (vKind && vKind !== 'translation') return { error: `variantKind "${vKind}" unsupported (only translation)` };
        if (!vLang) return { error: 'variant switch requires a translation target language, e.g. {"variant":{"variantKind":"translation","languageCode":"en"}} or shorthand {"languageCode":"en"}' };
