...
function mapTypeToKind(type: string | undefined, url: string): PoolKind | null {
  ...
  switch (type) {
    ...
    case 'effect':
    case 'transition':
      return null; // not Lazynext media-pool assets
    default:
  ...
  if (!kind) {
    return {
      ...
      error: opts.type === 'effect' || opts.type === 'transition'
        ? `type=${opts.type} is not an Lazynext media-pool asset`
        : 'Could not detect media type from URL; pass type: video|image|audio|gif|svg|motion-graphic',
      url,
    };
  }
    ...
    if (!body.configured) {
      return {
        error: kind === 'audio' || kind === 'music'
          ? 'Audio stock library API key not configured (FREESOUND_API_KEY); use the built-in sound library or download_media / push_asset to import a URL directly'
          : 'Stock search credentials not configured (PEXELS_API_KEY / PIXABAY_API_KEY / UNSPLASH_ACCESS_KEY / FIRECRAWL_API_KEY); use download_media / push_asset to import a URL directly',
        results: [],
