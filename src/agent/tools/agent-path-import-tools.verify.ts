...
try {
  const browserResult = await execAgentPathImportTool('import_asset', { path: '/Volumes/asset/A.mp4' }, {} as AgentContext);
...
(globalThis as unknown as { window?: unknown }).window = { lazynextDesktop: desktopBridge };
try {
...
try {
  const result = await execAgentPathImportTool('import_asset', { path: '/Volumes/asset/A001.mp4' }, projectCtx);
...
(globalThis as unknown as { window?: unknown }).window = { lazynextDesktop: desktopBridge };
try {
  const noProject = await execAgentPathImportTool('import_folder', { path: '/Volumes/asset' }, { getProjectId: () => undefined } as unknown as AgentContext);
...
try {
  const failed = await execAgentPathImportTool('import_asset', { path: '/Volumes/asset/A.mp4' }, projectCtx);
