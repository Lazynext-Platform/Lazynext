...
function describeHit(hit: SearchHit): string {
  ...
  if (hit.kind === 'caption') return `Project ${hit.projectId} captions`;
  return `Project ${hit.projectId} transcript`;
}
...
export async function execSearchTool(name: string, args: Args): Promise<unknown> {
  ...
  try {
    ...
    return {
      ...
      count: hits.length,
      note: hits.length
        ? 'Hits sorted by relevance descending; pass projectId to narrow the scope.'
        : 'No hits. Try different keywords or ≥3-character terms (2-character CJK terms are supported).',
      hits,
    };
  } catch (error) {
