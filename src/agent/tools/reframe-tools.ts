...
// __lazynextReframeCurve = ReframeCurveV1), there is no sample video → detect subject → automatically generate key frames
...
function loadVideo(src: string): Promise<HTMLVideoElement> {
 ...
 const onErr = (): void => {
 cleanup();
 reject(new Error(`auto_reframe: (${src})`));
 };
...
export async function execReframeTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
 ...
 if (!item) {
  return { error: `Clip not found: ${args.itemId ?? 'unknown itemId'}`, available: videos.map((v) => ({ itemId: v.id, name: v.name })) };
 }
 if (!item.src) return { error: `clip ${item.id} (src )` };
 ...
 if (!keyframes.length) {
 return { error: `auto_reframe: clip ${item.id} ()`, keyframes: 0 };
 }
 ...
 : magnification <= 1.05
 ? '≈1Keyframes'
 : 'reframe Keyframes view_timeline_frames ',
 };
 } catch (err: unknown) {
 return { error: err instanceof Error ? err.message : 'auto_reframe ' };
 }
}
