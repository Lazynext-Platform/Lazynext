...
// ── end to end / empty track / single clip → no holes ──
{
  ...
  assert.deepEqual(trackGaps([clip('a', 300, 60)], 'V1'), [], 'singleclipfront');
}
...
// ── Isolate by track: clips from other tracks cannot fill the holes in this track ──
{
  ...
  assert.deepEqual(trackGaps(items, 'A1'), [], 'not exist');
}
...
console.log('track-gaps.verify: ok (//contain//isolation/offlinestate)');
