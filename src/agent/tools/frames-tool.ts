  ...
  try {
    ...
    return {
      ...
      note: [
        ...
        labelLine,
        '(not composited into timeline; each cell ≈ midpoint of the corresponding source time range)',
      ].filter(Boolean).join(' · '),
...
async function viewTimelineFrames(args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  const note = `Timeline "${state.name}" ${frames.length} frames (absolute timeline coordinates f${frames.join(', f')}, total ${total} @${state.fps}fps) — target timeline draft composition (includes uncommitted edits)`;
...
async function viewAssetFrames(args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  const note = `Source asset "${asset.name}" ${frames.length} frames (source coordinates f${frames.join(', f')}, total ${total}) — standalone preview, not composited into timeline · ${windowNote}`;
