...
function parseJsonArray(text: string): unknown {
  ...
  if (start < 0 || end <= start) throw new Error('No JSON array found in model output');
...
export async function execHighlightTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  if (!hasOperationalTranscript(item)) {
    return { error: 'The current timeline has no transcribed video/audio clip; run transcribe_track first, then smart-slice.' };
  }
  ...
  if (!highlights.length) {
    ...
    return {
      error: 'Transcript content is insufficient to select highlights: neither the model nor the heuristic produced candidates. Confirm the clip transcript is complete (use read_transcript to check), or try a clip with richer spoken content.',
    };
  }
