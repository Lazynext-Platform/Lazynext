...
function resolveDims(a: { ratio?: unknown; width?: unknown; height?: unknown }): { width: number; height: number } | null | { error: string } {
  if (typeof a.ratio === 'string' && a.ratio) {
    ...
    return preset ? { width: preset.width, height: preset.height } : { error: `unknown ratio ${a.ratio} (options: ${ASPECT_PRESETS.map((p) => p.label).join('/')})` };
  }
...
export async function execTimelineTool(name: string, args: Args, ctx: AgentContext): Promise<unknown> {
  ...
  switch (String(args.action)) {
    ...
    case 'update': {
      ...
      if (!changed.length) return { error: 'update requires at least one of name / ratio / width+height / fit / hidden' };
      const after = ctx.getDoc();
    ...
    case 'delete': {
      ...
      return { ok: deleted.length > 0, deleted, ...(kept.length ? { kept, note: 'At least one sequence is kept; those referenced by nested instances or not found were skipped' } : {}), ...(blocked.length ? { blocked } : {}) };
    }

    default:
      return { error: `unknown action ${args.action} (options: list/create/duplicate/switch/update/delete/insert)` };
  }
}
