...
async function main(): Promise<void> {
  ...
  const result = await execSearchTool('search_content', { query: 'backgroundvolume' });
  ...
  assert.ok(calls[0]!.url.includes(encodeURIComponent('backgroundvolume')), 'query must be encoded');
  assert.equal(hits[0]!.kind, 'chat');
  assert.equal(hits[0]!.where, 'p-1 3', 'chat ref must map to a message position');
  assert.equal(hits[1]!.where, 'p-2 caption');
  ...
  const failed = await execSearchTool('search_content', { query: 'caption' });
