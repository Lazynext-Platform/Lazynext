...
// arrives as a JSON string in `json`. Backed by Lazynexts captions overlay
...
export async function editCaptions(args: Args, ctx: AgentContext): Promise<Result> {
 ...
 if (!layout) return { error: 'layout ,:{preset:bottom-center}(3×3 /top/bottom/center) {offsetXRatio:0.1,offsetYRatio:-0.05} ;(/) action=positions, layout' };
