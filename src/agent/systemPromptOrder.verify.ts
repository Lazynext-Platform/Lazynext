...
// ── Assembly rules: Stable sections come first, and variable sections end ──
{
  ...
  assert.equal(assembleSystemPrompt(stable, ''), 'AAABBBCCC', 'empty volatile section should not leave extra separator');
  ...
  assert.equal(
    ...
    stable.join('').length,
    'common prefix must cover all stable segments — one byte short means volatile content leaked into the prefix',
  );
}
...
{
  const workflow = '\n<selected_skill>A legacy workflow calls this product AnotherLazynext.</selected_skill>';
  ...
  assert.match(PRODUCT_IDENTITY_PROMPT, /official product name is Lazynext/);
...
// ── Go through it with the real editorStatePrompt: you cannot change the prefix when changing the timeline ──
{
  ...
  assert.notEqual(before, after, 'timeline changed so volatile section must change');
  assert.ok(
    commonPrefixLength(before, after) >= stable.join('').length,
    'adding a clip should only affect the trailing section; if the prefix changes, tool schema and history caches are invalidated',
  );
  assert.equal(before.slice(0, stable.join('').length), stable.join(''), 'stable section must be byte-identical');
  assert.ok(before.includes('<editor_state>'), 'snapshot must be assembled in');
  assert.ok(
    before.indexOf('<editor_state>') >= stable.join('').length,
    'the entire snapshot section must fall after the stable section',
  );
}
...
// ── When the stable paragraph itself changes (such as changing the creative mode), the change point cannot be advanced to an earlier paragraph ──
{
  ...
  assert.equal(commonPrefixLength(a, b), 'SYSTEMCAPS'.length, 'ok');
}
...
console.log('systemPromptOrder.verify: ok (/ editorStatePrompt /)');
