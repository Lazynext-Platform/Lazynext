...
function repetitionFindings(scenes: readonly SceneLike[]): SceneFinding[] {
 ...
 scenes: runScenes,
 message: ` ${sceneRangeMessage(runScenes)} ${types[runStart]}`,
 });
 }
 runStart = index;
 }
 ...
 dimension: 'repetition',
 scenes: affected,
 message: ` ${sceneRangeMessage(affected)} ${description.slice(0, 80)}`,
 });
 }
 return findings;
}
...
export function reviewScenePlan(scenes: readonly SceneLike[]): SceneQualityReport {
 ...
 dimension: 'decorative_visuals',
 scenes: purposeless,
 message: ` ${sceneRangeMessage(purposeless)} shotIntent/informationRole`,
 });
 }
 ...
 scenes: staticScenes,
 message: `${staticScenes.length}/${scenes.length} `,
 });
 }
 ...
 scenes: affected,
 message: ` ${sceneRangeMessage(affected)} ${phrase}`,
 });
 }
