---
name: known-errors
description: Use when a Lazynext tool call fails or returns an unexpected shape.
---
...
- Use `import_media action=create_session`, then run the Lazynext media import helper once with the returned token for client-held files.
...
- Rerun the Lazynext media import helper; it owns frontend-aligned conversion and will surface a user-actionable error if conversion is impossible.
...
- MG code must pass the Lazynext validator.
