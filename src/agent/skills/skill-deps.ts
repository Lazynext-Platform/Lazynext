...
// name services Lazynext does not expose the same way. This module scans a
// skills text for service keywords and maps each hit onto a local capability
...
const RULES: Rule[] = [
 ...
 'image generation', 'imagegen', 'text-to-image', 'image', 'keyframe', 'keyframes',
 ],
 },
 {
 kind: 'video',
 ...
 'text-to-speech', 'tts ', 'voiceover', 'voice clone', 'indextts', 'narration voice',
 ],
 },
 {
 kind: 'music',
 ...
 {
 kind: 'web',
 service: 'web extraction (Web Scraping)',
...
/** Scan a skills text (SKILL.md body + description) for service dependencies. */
 ...
 * (external agent runtime → execute the steps with this environments tools).
...
export function skillDependencyPrompt(
 ...
 + 'You are the Lazynext built-in agent: run the same deterministic steps with THIS environment\'s tools '
 ...
 if (caps[dep.kind]) {
 lines.push(`- ${dep.service}: the skill names its own service (e.g. ${dep.keyword}); THIS app has ${CAP_LABEL[dep.kind]} configured use the local tool instead of the skills foreign service.`);
 } else {
 ...
 `The user selected custom skill ${skill.slug}, written for a different environment. Dependencies were auto-detected from its content and checked against THIS apps configured capabilities:`,
 ...lines,
 missing.length > 0
