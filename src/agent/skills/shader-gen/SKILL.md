...
name: shader-gen
description: |
  AI shader generator for WebGL video effects, transitions, masks, and color grading (LUT / color grading / cinematic look / film look). Use when the user wants a video effect (filter / effect), a transition (crossfade / wipe / cube / 3d), a mask (mask / reveal), a zoom / push-in (push-in / camera push), or a color grade — try the built-in effects (zoom, builtin LUTs) before generating a new shader.
user-invocable: true
---
...
Use the user's language. Chinese: "The new effect/transition generation is still in beta and may have stability issues. If you want to proceed, I can help you with that." Skip if user already acknowledged in the same request.
...
- **Apply is separate and optional.** Only apply when user explicitly asks ("add to video", "apply", "use on the first clip"). When ambiguous, default to library-only.
