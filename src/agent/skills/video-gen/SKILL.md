...
1. **Duration & segments** — total length, how many shots, and whether they live in one clip or several.
   ...
   If the user has already stated a direction ("make one clip", "in one video", "generate separately", "split into N shots", etc.), follow it — don't second-guess.
...
- "one clip / single clip / one clip / one shot / in 1 clip" → never split, even if the description is objectively long.
- "N shots / N segments / N shots" → generate exactly N.
- "use this image / use this image" → use as reference, don't substitute.
...
- **Match the user's intent.** If the user pointed to a specific asset ("use this photo", "like the previous clip"), use that. If they described a character only in words, no anchor exists yet and one must be established.
