...
Grounded in Lazynext’s video adapter (`server/plugins/video.ts` → Kling
...
- In the **prompt**, use `@Image1` / `@Image1` and `@Video1` / `@Video1`. Our server rewrites to `<<<image_N>>>` / `<<<video_N>>>`. Image ordinals: firstFrame → lastFrame → refImages[0]…. Video ordinals start at 1 for `refVideos[0]`.
