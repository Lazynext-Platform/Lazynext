...
- Lazynext polls generation jobs, so provider callback URLs are not exposed to the agent.
...
- Seedance reference videos and MiniMax image subject references require provider-fetchable HTTPS URLs. Lazynext creates temporary signed R2 URLs and never exposes storage credentials.
