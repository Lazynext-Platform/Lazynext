---
name: export
description: Use when a Lazynext video editing or creation workflow needs export, render, download, share, final delivery, subtitle-file export, render choice, local-only asset handling, or export fallback explanation.
---

# Export

Use Lazynext's export tools for delivery. An export request should call `submit_export` (or `submit_render_job` for async), then use `track_export` for status and final delivery when the result is not returned immediately.

Default policy:

- Prefer `submit_export` when the user asks to export/share/finalize the Lazynext timeline.
...
- Do not wrap sandbox `ffmpeg` work as a Lazynext tool. Use sandbox `ffmpeg` for full video processing only when the user explicitly asks for a standalone local-file operation outside a Lazynext editing workflow. For Lazynext editing tasks, do not produce a pre-edited or flattened local render as the primary review/final deliverable; use Lazynext export.
...
1. Ask the user to re-link the asset in My Assets (offline asset banner) or re-import the file.
...
- "Local quick export"
- "Assets are still uploading, please wait before exporting"
- "You need to re-link local assets first"
