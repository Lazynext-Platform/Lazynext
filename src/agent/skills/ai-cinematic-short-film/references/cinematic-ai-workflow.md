...
8. Place selected clips as separate Lazynext timeline items; refine order, trims, pacing, and transitions without flattening the sequence.
...
Prefer one readable action over a brittle list of simultaneous events. Use only controls exposed by the current Lazynext generation capability; do not imply that prompts, seeds, or references make a result deterministic.
...
- A generated reference is a proposal until reviewed. Store approved generated images or clips in the Lazynext media pool before using them downstream.
