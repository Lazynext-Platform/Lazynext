---
name: transcription
description: Use when a video/audio task needs Lazynext transcription, captions, subtitles, subtitle styling, transcript search, transcript readiness checks, or enabling captions, including local or attached videos where the user asks to add captions/subtitles, transcribe, create bilingual subtitles, clean talking-head speech, remove filler words, or trim pauses.
---
...
2. If this is a fresh client-held import, make sure it went through `import_media action=create_session` plus the Lazynext media import helper.
