...
`messagePrefix`), write like Lazynext is a capable video-making partner inviting
...
- Short and scannable. Use one natural sentence for `prompt` and concise
  question labels.
- Honest capability framing. Lazynext can help with many video workflows, but do
  not claim unsupported abilities or guarantee a result before inputs are known.
- The user's language and local product terms. Keep "Lazynext", "B-roll",
...
- Keep user-visible descriptions provider-neutral. Describe the voice the same
  way the native Lazynext audition UI does: gender / age range / tone / use case,
  such as `Female / young / friendly, general` or `Male / middle-aged / deep knowledge narration`.
...
- Use this when the user should choose which Lazynext video workflow to start,
...
- English, Chinese, and Spanish scenario labels/descriptions/starter prompts are
  ...
  and pass those localized values in the option objects. Preserve Lazynext
...
to place directly in a project, ask them to use the Lazynext editor upload UI.
...
```json
{
  ...
  "prompt": "Choose or fill in what you have in mind so Lazynext can pick a good starting direction.",
