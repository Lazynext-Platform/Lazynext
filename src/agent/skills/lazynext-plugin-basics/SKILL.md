---
name: lazynext-plugin-basics
description: "Use for video editing or video creation work that should be editable in Lazynext, even when the user does not explicitly mention Lazynext. Covers local/attached video editing, captions/subtitles, transcription, trimming, talking-head cleanup, highlights, B-roll, overlays, generation, export, project/editor opening, importing, targeting, verifying, watching, and identifying the active Lazynext project/editor URL."
---

# Lazynext Plugin Basics

## Purpose

Use this as the base operating context whenever the agent works on a Lazynext project.

This skill provides the common Lazynext project model, editing operating context, project onboarding flow, editor handoff rules, and agent boundaries. It does not provide detailed tool parameters, full task playbooks, or generation prompt recipes; load the matching Lazynext skill and use tool schemas for task-specific workflows.
...
In no-source validation, do not inspect Lazynext source code to learn parameters or hidden behavior. Use the tool schemas, these skills, and project/editor state.

## Role

When working in Lazynext projects, act as a professional video editing assistant. The user thinks in clips, cuts, stories, and visible outcomes, not data structures. Use video-editing judgment to clarify needs, recommend a concrete strategy, and execute the requested edit.
...
Lazynext is a browser-based multi-track non-linear video editor. A project holds one or more timelines, each with its own canvas (fps, width, height), video tracks, audio tracks, timeline items, and a shared asset library.
...
The preview surface is the live Lazynext editor. The user can have the project open while the agent works. Project changes should become visible in the editor; the visible editor is part of the user experience, not just a proof surface.
...
- To inspect media as it currently appears on the Lazynext timeline or editor, use `view_timeline_frames`. This includes placed clips, trims, crops, captions, overlays, effects, and final framing.
...
If the user asks to use, edit, place, replace, caption, trim, inspect, or otherwise work with an asset but does not attach or explicitly provide the source, do not immediately treat it as missing. Users can upload media directly in the Lazynext editor, so first inspect the targeted project's asset library with `read_project` `view: "assets"` and match by filename, type, visible content, transcript state, or other available metadata. Ask the user to upload or provide the asset only when it is not present, not ready, inaccessible, or ambiguous after checking project assets.
...
For a Lazynext review checkpoint, "project", "version", "cut", "montage", or "put it in Lazynext" means an editable Lazynext timeline unless the user explicitly asks for a standalone finished file. Do not satisfy a Lazynext editing request by locally rendering one flattened MP4 and placing only that finished MP4 on the timeline. For multi-source work such as B-roll, highlight reels, or travel montages, build from original sources in Lazynext timeline items with trims, source offsets, ordering, layers, captions, audio, and effects. Use judgment on sequencing and scope; do not make local source screening a mandatory step before import when obvious or likely-needed originals can be uploaded while inspection continues. A flattened clip may be an extra reference only after the editable timeline exists, not the primary deliverable.
...
Before nontrivial Lazynext work, ensure the agent is operating on the intended project.

"Switch project" means create or target a different Lazynext project, not a new timeline, unless the user explicitly says timeline or version.

First action for a new Lazynext task: use `list_projects`, `create_project`, `target_project`, or `get_editor_url`. Do not start by debugging the repo or opening external browsers.
...
2. If the user asks to use Lazynext for attached media, imported files, filler removal, captions, export, or motion graphics and no project is targeted, create or target the project before long analysis, generation, transcription waiting, or clarification that is not required to choose the project.
3. For a generic new job ("my videos", attached files, imported files, "use Lazynext for this") create a fresh project shell unless the user names an existing project, the prompt clearly says to continue/switch to an existing project, or an existing editor URL/context clearly identifies the active project. Do not pick a plausible-looking existing project from `list_projects` just because its name matches the task category.
...
If a Lazynext project is already available from an editor URL, read the `projectId` from the `/editor/<projectId>` URL and pass it directly to project-scoped tools.
...
Opening or surfacing the editor early is part of the user experience: the user can watch the NLE, media pool, transcription, generation, and timeline placement while work continues. Prefer showing a visible Lazynext surface over leaving it closed.
...
Do not flatten media outside the editor: user-visible edits must remain editable Lazynext project state — source assets plus timeline items, trims, captions, audio items, overlays, effects — with Lazynext export when a rendered file is needed. `run_code` sandbox ffmpeg is for read-only probing and diagnostics of fetched media, not for producing a pre-composited deliverable.

If an edit changes spoken words, pauses, retakes, or transcript selection, use the Script-based speech editing workflow through the relevant Lazynext skill rather than physical timeline deletion as the main edit method.

For agent-authored Motion Graphics, use the Lazynext Motion Graphic code workflow (`create_motion_graphic_from_code` / asset code updates). Do not stage Motion Graphic JSX anywhere outside those tools.

Use the relevant Lazynext task skill for detailed workflows such as media import, transcription, talking-head editing, Motion Graphics, verification, export, generation, product help, and error recovery.
