...
Grounded in Lazynext’s voice tool + server adapter (`server/plugins/voice.ts` → MiniMax `t2a_v2`). Preset table: [voices.md](voices.md) § MiniMax.
...
| Streaming | `stream`; `excludeAggregatedAudio`; `forceCbr` only with streamed MP3. Lazynext still persists one local asset |
...
submit_voice({
  ...
  subtitleEnable: true,
  subtitleType: "word",
  pronunciations: ["Lazynext/(open chat cut)"],
