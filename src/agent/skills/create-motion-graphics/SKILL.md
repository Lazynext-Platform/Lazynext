...
description: "Use whenever the agent needs to add, create, hand-author, patch, or place Motion Graphic JSX assets in a Lazynext project. This is the direct-authoring path: use create_motion_graphic_from_code / edit_asset / edit_item, not motion-graphic-gen or submit_motion_graphic. Covers project/timeline intake, project visual language, editable properties, asset binding, inline JSX authoring, existing asset updates, timeline placement, and verification."
---

# Create Motion Graphics

Use this skill when a Lazynext task requires a Motion Graphic asset that the agent will author or patch as inline JSX.
...
Even though `submit_motion_graphic` exists in the tool list, treat it as the generation route, not this one. Ignore that path for MG work; use `create_motion_graphic_from_code` for new JSX assets and `edit_asset` for existing MG JSX. If the direct-authoring tools are missing, stop and report that the Lazynext tool surface is out of date.

Pass source inline through the asset code tools. Do not stage Motion Graphic code in the Lazynext repository, `ai-working/`, `/tmp`, a local HTTP server, generated code files, or guessed backend workspace paths.
...
- The user has already named a visual style, material, reference, or visual language ("editorial magazine style", "80s retro print style", "magazine style") — use it verbatim as the direction. For multiple MGs, this still enters the representative-MG branch until the user confirms the example.
...
Create new Motion Graphic assets by passing inline JSX and editable property metadata through the current Lazynext asset-creation tool. Use the tool schema for the exact field names and accepted duration format.
...
- For visible changes, inspect a real composed frame using the normal Lazynext visual verification path.
