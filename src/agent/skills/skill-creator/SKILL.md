---
name: skill-creator
description: Create or improve Lazynext custom skills. Use whenever the user asks to create a skill, capture a workflow as a skill, write a SKILL.md, add a reusable workflow, create a skill, turn a workflow into a skill, write a skill, or improve an existing custom skill. Pushy: treat any workflow the user repeats as a skill candidate, even if they do not say "skill".
---

# Skill Creator

Create or improve custom skills for Lazynext. A skill is a SKILL.md file
(plus optional `references/` support docs) that teaches the agent a reusable
workflow. Custom skills live at `~/.lazynext/skills/<slug>/SKILL.md` and
...
4. **Create it** with `manage_skill action=create`. The tool reports the
   install path (`~/.lazynext/skills/<slug>/SKILL.md`).
...
- Reference existing Lazynext tools by their exact tool names
...
## Progressive disclosure in Lazynext
