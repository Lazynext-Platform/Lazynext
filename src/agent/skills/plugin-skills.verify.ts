// Runnable check: `npx tsx src/agent/skills/plugin-skills.check.ts` .verify.ts verify:skills。
...
const SKILLS_DIR = dirname(fileURLToPath(import.meta.url)); // check 
const EXPECTED = [
 ...
 'lazynext-plugin-basics', 'product-ad-video-script',
...
assert.deepStrictEqual(slugs, [...EXPECTED].sort(), '26 ,');
...
for (const slug of slugs) {
 ...
 assert.strictEqual(name, slug, `${slug}: frontmatter name slug`);
 assert.ok(description.length > 20, `${slug}: description `);
 assert.ok(body.trim().length > 100, `${slug}: SKILL.md `);
 // Verbatim: the parsed body is a suffix of the original bytes (nothing rewritten).
 assert.ok(raw.endsWith(body), `${slug}: ）`);
}

// Parser handles all three source description shapes.
const plain = parseSkillFrontmatter('---\nname: a\ndescription: Use when X.\n---\nBODY');
assert.strictEqual(plain.description, 'Use when X.', ' description');
const quoted = parseSkillFrontmatter('---\nname: b\ndescription: "Use for Y, Z."\n---\nBODY');
assert.strictEqual(quoted.description, 'Use for Y, Z.', ' description ');
const block = parseSkillFrontmatter('---\nname: c\ndescription: |\n Line one\n line two.\nuser-invocable: true\n---\nBODY');
assert.strictEqual(block.description, 'Line one line two.', '`|` description key ');
assert.strictEqual(block.body, 'BODY', ' description ');
...
assert.ok(voiceRef.length > 0, 'references ');

console.log(`plugin-skills.check: ok (${slugs.length} / frontmatter / references )`);
