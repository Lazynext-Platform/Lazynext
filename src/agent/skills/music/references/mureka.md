...
Lazynext wires the official Mureka generate/query and file-upload APIs. The configured model is `MUREKA_MUSIC_MODEL` (default `auto`; official choices include `mureka-7.6`, `mureka-o2`, `mureka-8`, `mureka-9`, with endpoint-specific support).
...
Output selection: `audioFormat` may be `mp3`, `wav`, or `flac`. Every returned choice is downloaded and becomes a distinct asset. Lazynext defaults `count` to 1 rather than Mureka's official default 2 to avoid surprise charges.
