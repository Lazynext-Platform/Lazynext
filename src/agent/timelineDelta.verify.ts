...
// ── Read-only: status unchanged → null (do not add any fields to the result) ──
{
 ...
 assert.equal(describeTimelineDelta(snapshotTimeline(s), s), null, 'ok');
}
...
// ── Create new / change track / change length → enter clips one by one ──
{
 ...
 assert.equal(d.clips?.length, 3, ' clips');
 assert.deepEqual(d.clips?.map((c) => c.id).sort(), ['a', 'b', 'c']);
 assert.equal(d.shifted, undefined, 'ok');
 const a = d.clips!.find((c) => c.id === 'a')!;
 assert.equal(a.durationInFrames, 45, 'clips ');
 assert.equal(a.track, 'V1', 'track ( = V1), read_project ');
}
...
// ── Corrugation: Same track and same displacement ≥3 → compressed into one rule, not listed one by one ──
{
 ...
 const d = describeTimelineDelta(before, after)!;
 assert.equal(d.clips, undefined, ' clips');
 assert.deepEqual(d.shifted, [{ track: 'V1', fromFrame: 30, by: -30, count: 4 }], 'ok');
 assert.deepEqual(d.removedItemIds, ['a'], 'ok');
}
...
// ── Sporadic displacement (<3) → Return to enumeration one by one, do not press the rules ──
{
 ...
 const d = describeTimelineDelta(before, after)!;
 assert.equal(d.shifted, undefined, '2 ');
 assert.deepEqual(d.clips?.map((c) => c.id), ['b', 'c'], 'ok');
}
...
// ── Different rails/different displacements are grouped into groups ──
{
 ...
 const d = describeTimelineDelta(before, after)!;
 assert.equal(d.shifted?.length, 2, 'ok');
...
// ── Upper limit: if there are more than 30 entries, only the top 30 will be listed and prompted to re-read──
{
 ...
 const d = describeTimelineDelta(before, after)!;
 assert.equal(d.clips?.length, 30, ' 30 ');
 assert.match(d.notes?.join(' ') ?? '', / 40 /, 'ok');
}
...
// ── New track reporting + track composition change reminder ──
{
 ...
 const d = describeTimelineDelta(before, after)!;
 assert.deepEqual(d.createdTracks, ['V2'], '(: = V2)');
  assert.match(d.notes?.join(' ') ?? '', /V2/, 'ok');
}
...
// ── Ripple removal by real reducer: difference is consistent with actual result ──
{
 ...
 assert.deepEqual(d.removedItemIds, ['a'], ' reducer ');
 assert.equal(d.shifted?.length, 1, 'ok');
 assert.equal(d.shifted![0]!.count, 3, ' 3 ');
 assert.equal(d.shifted![0]!.by, -30, ' 30 ');
 // Rules can infer the true position
 for (const it of s1.items) {
 const was = before.placements.get(it.id)!;
 assert.equal(it.startFrame, was.startFrame + d.shifted![0]!.by, `${it.id} `);
 }
}
