...
export const EDITOR_BOOTSTRAP_HEADER = 'X-Lazynext-Editor-Bootstrap';
...
async function requestEditorBootstrap(signal?: AbortSignal): Promise<EditorBootstrapInfo> {
  ...
  const desktopWindow = typeof window === 'undefined' ? undefined : window as typeof window & {
    lazynextDesktop?: { editorCredentials?: () => Promise<EditorBootstrapInfo> };
  };
  const desktop = desktopWindow?.lazynextDesktop?.editorCredentials;
  if (desktop) {
    value = await desktop();
  } else {
