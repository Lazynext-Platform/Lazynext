...
try {
  ...
  assert.deepEqual(result.at(-1)?.content, [{
    ...
    providerOptions: {
      lazynext: { activatedTools: ['ask_followup_questions'] },
    },
  }]);
...
try {
  ...
  assert.doesNotMatch(String(resumed.at(-1)?.content), /couldn't complete the requested operation|toolfail/,'no failure-report template is injected');
...
try {
  ...
  assert.doesNotMatch(displayed, /couldn't complete the requested operation|toolfail/); }finally { globalThis.fetch = originalFetch; } globalThis.fetch = (async (input) => { const path = typeof input ==='string' ? input : input instanceof URL ? input.href : input.url;
  if (path !== '/api/codex/turn') throw new Error(`Unexpected fetch: ${path}`);
  const payload = [
    { type: 'text-delta', delta: 'The unavailable tool completed successfully.' },
    {
      ...
      name: 'unknown_tool',
      args: { value: 1 },
      result: { error: 'This Lazynext tool call is unavailable.' },
      success: false,
    },
    { type: 'done' },
  ].map((event) => JSON.stringify(event)).join('\n');
