...
// ── After completing "Cancelled", clear it in the air (simulating the end of commitAbortedTurn) ──
{
  ...
  assert.equal(completed.length, 3, 'must');
  assert.deepEqual(unresolvedToolCalls(completed), [], 'aftersessionbottom');
}

console.log('abortedTurn.verify: ok (noneempty/singleempty/concurrencypartial/match/)');
