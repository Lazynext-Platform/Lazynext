...
// ── Grayscale gradient: black and white dots, no saturation, no main color, color shift ≈ 0 ──
{
  ...
  assert.ok(s.blackPoint < 0.03 && s.whitePoint > 0.97, 'dot');
  assert.ok(s.saturationMean < 0.01, 'none');
  assert.deepEqual(s.dominantHues, [], 'nonehue');
  assert.ok(near(s.warmCool, 0, 1e-6) && near(s.greenMagenta, 0, 1e-6), '');
  assert.ok(describeScopeStats(s).some((line) => line.includes('near-neutral')), 'center');
}

// ── Pure orange picture: main hue orange, warm, high saturation ──
{
  const s = analyzeRgbaPixels(rgba([[255, 128, 0, 5000]]), 1);
  assert.equal(s.dominantHues[0], 'orange', 'hue orange');
  assert.ok(s.warmCool > 0.5, '');
  assert.ok(s.saturationMean > 0.9, 'high');
  assert.ok(near(s.hueHistogram.find((b) => b.label === 'orange')!.pct, 1, 1e-6), 'center orange bin');
}
...
// ── Overexposure: 35% pure white → highlight bloom ratio ≈ 0.35, white point = 1 ──
{
  ...
  assert.ok(near(s.clippedHighlightsPct, 0.35, 0.01), `high≈35%(${s.clippedHighlightsPct})`);
  assert.ok(s.whitePoint > 0.99, 'dot');
  assert.ok(s.clippedShadowsPct < 0.01, 'none');
}
...
// ── teal-orange: Both main color clusters are recognized ──
{
  ...
  assert.ok(s.dominantHues.includes('orange') && (s.dominantHues.includes('cyan') || s.dominantHues.includes('azure')),
    `teal-orange (${s.dominantHues.join(',')})`);
}
...
// ── Segmented color cast: dark parts are bluer, bright parts are warmer, and the segmented tilts should be different from each other ──
{
  ...
  assert.ok(s.tilt.shadows.warmCool < -0.05, `(${s.tilt.shadows.warmCool})`);
  assert.ok(s.tilt.highlights.warmCool > 0.05, `(${s.tilt.highlights.warmCool})`);
}
...
// ── Reference shot comparison: signed difference + named knob suggestion after dead zone ──
{
  ...
  assert.ok(compared.targetMinusReference.meanLuma > 0, 'targetvalue');
  assert.ok(compared.targetMinusReference.warmCool > 0, 'targetvalue');
  assert.deepEqual(
    compared.suggestions.map((entry) => [entry.control, entry.direction]),
    [['brightness', 'decrease'], ['saturate', 'decrease'], ['temperature', 'cooler']],
  );
  assert.deepEqual(compareColorScopes(reference, reference).suggestions, [], 'innernone');
}

console.log('scopes.verify: ok (/hue/////emptyinput)');
