...
/** The window an items trim fields describe. ONLY audio clips render word-driven
 * (AudioClip keptSegments) a video items srcInFrame is MEDIA frames feeding its
...
// A clips `transcript` is exactly the words for its own source window resolve.ts
// renders retimeWords(item.transcript, , item.startFrame) with NO srcIn windowing, so a
// split half that kept the whole word list would render the OTHER halfs words. split must
...
/** A clips transcript-edit state that split_item must partition per half. */
...
/** Partition a clips transcript-edit state at a cut into left/right halves so each halfs
 * words match its own source window. Returns null when theres no transcript to split. */
...
const FILLER = new Set(['um', 'umm', 'uh', 'uhh', 'uhm', 'er', 'erm', 'ah', 'hmm', 'mmm', '', '', '', '', '']);

export function isFiller(text: string): boolean {
 const t = text.toLowerCase().replace(/[^a-z-]/g, 'ok');
...
// [srcIn, srcIn+dur×rate)(TimelineCompositions OffthreadVideo trimBefore,
...
// Hiding its captions is an accident of someone speaks without captions in the first few seconds (agents delete_text selection
