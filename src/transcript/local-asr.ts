...
function reportModelProgress(
  ...
  if (progress != null) onWait?.(`Model download ${Math.min(100, Math.round(progress))}%`);
...
export async function assertAsrModelDownloaded(config: AsrConfig): Promise<void> {
  try {
    ...
    if (entry && entry.downloaded === false) {
      throw new TranscriptionError(
        'service-unavailable',
        `The local transcription model was not fully downloaded (${config.modelId}). Please go to Settings → Transcription → Local Model to re-download it, then try again.`,
      );
    }
  } catch (error) {
...
export async function localTranscribePathResumable(
  ...
  if (desktopNativeInferenceEnabled()) {
    ...
    const native = await tryDesktopNativeAsr({
      ...
      onFallback: () => onWait?.('Desktop native inference unavailable, falling back to browser engine'),
    });
    if (native) {
