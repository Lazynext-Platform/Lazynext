...
{
  assert.deepEqual(spans([1]), [[0, 10], [60, 100]], 'dot');
...
{
  assert.deepEqual(spans([1], 20), [[0, 20], [50, 100]], '10 frame');
  assert.deepEqual(spans([1], 1), [[0, 10], [60, 100]], 'half back 0,equals');
}
...
{
  assert.deepEqual(spans([3], 60), [[0, 90]], '→ ,back w3 dot');
  assert.deepEqual(spans([0], 60), [[10, 100]], '→ ,front w0 dot');
  ...
  assert.deepEqual(spans([], 600), [[0, 100]], 'clip');
}
...
// ── The gaps governed by the silent compression rules are not borrowed: the length has been determined by cap ──
{
  const capped = keptSegments(words, new Set(), fps, 0, { cutPadFrames: 60, gapCapsMs: { 1: 50 } })
    .map((s) => [s.srcStartFrame, s.srcEndFrame]);
  assert.deepEqual(capped, [[0, 15], [30, 100]], 'cap back w0 10+5,outer;bottom');
}
...
// ── Incisions caused by rearrangements are also considered incisions──
{
  const reordered = keptSegments(words, new Set(), fps, 0, { cutPadFrames: 60, playOrder: [2, 3, 0, 1] })
    .map((s) => [s.srcStartFrame, s.srcEndFrame]);
  assert.deepEqual(reordered, [[40, 100], [0, 60]], 'mute');
}
...
// ── Timeline positions are still connected end to end: borrowed frames are included in the duration and cannot leave overlaps or holes ──
{
  ...
  for (const seg of segs) {
    assert.equal(seg.fromFrame, cursor, 'top');
    assert.equal(seg.durFrames, seg.srcEndFrame - seg.srcStartFrame, 'durationequalsaftersource');
    cursor += seg.durFrames;
  }
}

console.log('cutPad.verify: ok (default//mutelimit//cap //timeline)');
