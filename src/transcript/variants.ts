...
/** Overlay a variants text onto the source words. Every returned word keeps the
 * source words start/end/speaker EXACTLY; only `.text` is swapped where the
 ...
 * off source indices), so a malformed variant can never touch a word that isnt
...
function defaultLabel(lang: string, kind: TranscriptVariant['kind']): string {
 return kind === 'translation' ? lang : `${lang}()`;
}
 ...
 * reuse-vs-create (ensure semantics) and to resolve a captions chosen variant. */
