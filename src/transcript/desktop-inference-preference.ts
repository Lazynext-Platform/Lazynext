...
export function desktopNativeInferenceEnabled(
  ...
  return typeof window !== 'undefined' && Boolean(window.lazynextDesktop?.inference);
}

async function applyDesktopNativeInferenceEnabled(enabled: boolean): Promise<void> {
  const bridge = typeof window === 'undefined' ? undefined : window.lazynextDesktop?.inference;
