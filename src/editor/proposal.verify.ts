...
for (let i = 0; i < 110; i++) boundedHistory = historyReduce(boundedHistory, { type: 'tl.rename', id: 'tl_a', name: `sequence ${i}` }); assert.strictEqual(boundedHistory.past.length, 100,'history retains only the newest 100 snapshots');

// ── manage_timelines through the draft: create → switch routing → replay ──
const d2 = makeDraft(applied);
const newId = d2.commands.createTimeline({ name: '', width: 1080, height: 1920 }); // activates it
...
assert.strictEqual(applied3.timelines[0].items.length, 3, 'clip followed the switch to 1');
