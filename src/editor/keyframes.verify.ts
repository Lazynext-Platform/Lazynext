...
assert.equal(volumeAtFrame({}, 10), 1, 'none volume default 1');
assert.equal(volumeAtFrame({ volume: 0.4 }, 10), 0.4, 'nonekeyframefallback item.volume');
const ducked = { volume: 0.3, keyframes: { volume: [{ frame: 0, value: 1 }, { frame: 10, value: 0.2 }] } };
assert.equal(volumeAtFrame(ducked, 0), 1, 'keyframevolume');
assert.ok(near(volumeAtFrame(ducked, 5), 0.6), 'linearinterpolationcenterdot');
assert.equal(volumeAtFrame(ducked, 999), 0.2, 'framevalue');
assert.equal(volumeAtFrame(ducked, -5), 1, 'framebeforevalue');
...
assert.ok(rawMid > 2, `overshoot centerdot >2(actual ${rawMid.toFixed(3)})`);
assert.equal(volumeAtFrame(shoot, 5), 2, 'overshoot 2');
...
assert.ok(l?.volume?.length && r?.volume?.length, 'split volume');
for (let f = 0; f < 120; f++) {
  const orig = sampleKeyframes(kfs.volume, f);
  const post = f < 30 ? sampleKeyframes(l!.volume!, f) : sampleKeyframes(r!.volume!, f - 30);
  assert.ok(near(orig, post, 1e-9), `frame ${f} frontback`);
}

// ── Registry: volume in KEYFRAME_PROPS; audio/video supported, image not supported; coerce clamp 0..2 ──
assert.ok(KEYFRAME_PROPS.includes('volume'), 'KEYFRAME_PROPS volume');
const def = getKeyframePropertyDefinition('volume');
assert.deepEqual([...def.valueRange], [0, 2], 'volume valueRange 0..2');
assert.equal(def.getBaseValue(item({ volume: 0.7 })), 0.7, 'getBaseValue item.volume');
assert.ok(supportsKeyframeProperty(item({ kind: 'audio' }), 'volume'), 'audio support volume');
assert.ok(supportsKeyframeProperty(item({ kind: 'video' }), 'volume'), 'video support volume');
assert.ok(!supportsKeyframeProperty(item({ kind: 'image' }), 'volume'), 'image not support volume');
assert.ok(!supportsKeyframeProperty(item({ kind: 'audio' }), 'opacity'), 'audio not support opacity');
const opacityDef = getKeyframePropertyDefinition('opacity');
assert.equal(opacityDef.getBaseValue(item({ kind: 'image', transform: { opacity: 0.45 } })), 0.45, 'opacity transform');
assert.deepEqual(opacityDef.toTransformPatch?.(0.6), { opacity: 0.6 }, 'opacity transform');
const radiusDef = getKeyframePropertyDefinition('borderRadius');
assert.ok(KEYFRAME_PROPS.includes('borderRadius'), 'KEYFRAME_PROPS borderRadius');
assert.ok(supportsKeyframeProperty(item({ kind: 'image' }), 'borderRadius'), 'imagesupportradius');
assert.ok(!supportsKeyframeProperty(item({ kind: 'text' }), 'borderRadius'), 'textradius');
assert.deepEqual(radiusDef.toTransformPatch?.(24), { borderRadius: 24 }, 'radius transform');
assert.equal(coerceKeyframeValue('borderRadius', 1200), 1000, 'radiuskeyframerange');
assert.equal(coerceKeyframeValue('volume', 9), 2, 'coerce top 2');
assert.equal(coerceKeyframeValue('volume', -1), 0, 'coerce bottom 0');
...
assert.deepEqual(s1.items[0].keyframes?.volume, [{ frame: 12, value: 0.5 }], 'reducer audio volume keyframe');
const s2 = reduce(base, { type: 'setKeyframe', id: 'i1', prop: 'volume', frame: 12, value: 0.5 });
assert.equal(s2, base, 'image top volume keyframe(state )');
const s3 = reduce(s1, { type: 'setKeyframe', id: 'a1', prop: 'volume', frame: 20, value: 7 });
assert.deepEqual(s3.items[0].keyframes?.volume?.map((k) => k.value), [0.5, 2], 'reducer valueRange');
...
assert.ok(!('error' in okAudio && okAudio.error), 'audio+volume keyframevalidate');
const badImage = validateGenericUpdate(base, { type: 'update', itemId: 'i1', keyframes: { volume: [{ frame: 0, value: 1 }] } }) as { error?: string };
assert.match(badImage.error ?? '', /not supported on a image/, 'image+volume kind');
...
assert.match(badAudioOpacity.error ?? '', /not supported on a audio/, 'audio+opacity');
const badRange = validateGenericUpdate(base, { type: 'update', itemId: 'a1', keyframes: { volume: [{ frame: 0, value: 3 }] } }) as { error?: string };
assert.match(badRange.error ?? '', /0\.\.2/, 'volume>2 rangevalidate');
