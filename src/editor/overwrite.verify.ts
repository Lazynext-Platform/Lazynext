...
function assertValidLinkGroups(state: TimelineState): void {
 ...
 assert.ok(group.itemIds.length >= 2, `${group.id} group`);
 assert.equal(new Set(group.itemIds).size, group.itemIds.length, `${group.id}`);
 assert.ok(group.itemIds.includes(group.anchorItemId), `${group.id} anchor must`);
 assert.ok(group.itemIds.every((id) => existingIds.has(id)), `${group.id} dangling id`);
 }
}
...
// New range covers old's left edge → retain right fragment and advance source by playback rate.
{
 ...
 assert.deepEqual([right.startFrame, right.durationInFrames, right.srcInFrame], [20, 5, 20], '5 timeline frame 10 source frame');
 assertNoTrackOverlap(next);
}
...
// Word-driven transcript audio keeps its timeline-word window semantics instead of multiplying by playback rate.
{
 ...
 'word-driven audio 5 timeline frame 5 frame');
}
...
// remapping its outgoing endpoint through both splits to the final right fragment.
{
 ...
 assert.equal(right.startFrame + right.durationInFrames, 30, 'right fragment seam');
 ...
 assert.equal(committed.past.length, 1, 'overwrite split/trim/remove/add undo step');
...
// the transition instead of guessing that the inserted clip inherited it.
{
 ...
 `${scenario.name} overwrite outgoing transition right`);
 assertNoTrackOverlap(next);
 }
}
...
// never moves, target fragments vacate the overwrite range, and the stale link is removed.
{
 ...
 assert.ok(plan, `${scenario.name} linked overwrite success`);
 const next = plan.actions.reduce(reduce, source);
...
// A locked linked companion is not part of the target lane transaction and remains untouched.
{
 ...
 assert.ok(plan, 'companion lock lane-only overwrite success');
 const next = plan.actions.reduce(reduce, source);
 assert.deepEqual(next.items.find((item) => item.id === companion.id), companion);
 assert.deepEqual(
 next.items
...
console.log('overwrite.verify: ok (/lane-only linked A/V/split transition endpoint/lockatomic/none/linkGroups / undo)');
