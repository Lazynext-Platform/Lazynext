...
export function buildCommands(dispatch: ProjectDispatch, getDoc: () => ProjectDoc): EditorCommands {
 ...
 id: uid('tl'), name: opts?.name ?? ` ${d.timelines.length + 1}`, order: maxOrder(d) + 1,
 };
 ...
 const newId = uid('tl');
 dispatch({ type: 'tl.duplicate', id, newId, name: opts?.name ?? `${src?.name ?? 'Sequence'} `, retarget: opts?.retarget, activate: opts?.activate });
 return newId;
 },
 ...
 width: asset.width,
 height: asset.height,
 // A clip inherits a copy of the assets ingest transcript,
