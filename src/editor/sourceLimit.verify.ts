...
// ── Transition pre-roll keeps the source position continuous at the cut ──
{
  ...
  assert.equal(sourceFrameAt(item({ srcInFrame: 5 }), -15), 0, 'sourcedotasset');
}
...
// ── Remaining source frames: How much is left after the entry point, variable speed timeline frame = source frame / rate conversion ──
{
  ...
  assert.equal(remainingSourceFrames(item(), 120, assets), 180, 'dotpartial');
  assert.equal(remainingSourceFrames(item({ playbackRate: 2 }), 0, assets), 150, '2 sourcetimelineframe');
  assert.equal(remainingSourceFrames(item({ playbackRate: 0.5 }), 0, assets), 600, '');
  assert.equal(remainingSourceFrames(item(), 999, assets), 1, 'dot 1 frame, 0/');
}

// ── If you can’t determine the length, don’t set a limit, so as not to lock something that can be stretched arbitrarily ──
{
  assert.equal(remainingSourceFrames(item({ kind: 'image', src: '/m/a.png' }), 0, assets), null, 'imagelong');
  assert.equal(remainingSourceFrames(item({ kind: 'motion-graphic', src: undefined }), 0, assets), null, 'MG');
  assert.equal(remainingSourceFrames(item({ kind: 'text', src: undefined }), 0, assets), null);
  assert.equal(remainingSourceFrames(item({ src: '/m/missing.mp4' }), 0, assets), null, 'assettable');
  ...
  assert.equal(
    ...
    null,
    'audio retime editback,topmust not',
  );
}
...
// ── Jingzhen reduce: The right crop is blocked by the tail of the asset, and the freeze frame can no longer be pulled out──
{
  ...
  assert.equal(stretched.items[0]!.durationInFrames, 300, 'fullasset');

  const trimmed = reduce(before, { type: 'retime', id: 'a', durationInFrames: 5000, srcInFrame: 200 });
  assert.equal(trimmed.items[0]!.durationInFrames, 100, 'leftafterlongshort');

  const shorter = reduce(before, { type: 'retime', id: 'a', durationInFrames: 60 });
  assert.equal(shorter.items[0]!.durationInFrames, 60, 'rangeinnercrop');
}
...
console.log('sourceLimit.verify: ok (sourceframe/// reduce righttop)');
