...
// ── 1. The interval is in the middle: other camera positions are double-cut and deleted, leaving [0,60)+[150,300), srcIn is moved forward by 150 ──
{
  ...
  assert.equal(plan.actions.length, 6, '2 1');
  assert.equal(plan.coverageGapFrames, 0, 'targetallnone');
  const s1 = applyPlan(s0, plan.actions);
  assert.deepEqual(spansOf(s1, '/m/b.mp4'), [[0, 60], [150, 300]], 'camB');
  assert.deepEqual(spansOf(s1, '/m/c.mp4'), [[0, 60], [150, 300]], 'camC');
  assert.deepEqual(spansOf(s1, '/m/a.mp4'), [[0, 300]], 'target');
  const bTail = s1.items.find((it) => it.src === '/m/b.mp4' && it.startFrame === 150)!;
  assert.equal(bTail.srcInFrame ?? 0, 150, 'sourcedotfrontdot');
  const bHead = s1.items.find((it) => it.src === '/m/b.mp4' && it.startFrame === 0)!;
  assert.ok(bHead.keyframes?.opacity?.length && bTail.keyframes?.opacity?.length, 'keyframe split');
  const mix = s1.items.find((it) => it.id === 'mix')!;
  assert.equal(`${mix.startFrame}:${mix.durationInFrames}`, '0:300', 'audioall(none)');
}
...
// ── 2. Interval pasting clip header: single cut right boundary, delete [0,150) ──
{
  ...
  assert.equal(plan.actions.length, 2, '1 1');
  assert.deepEqual(spansOf(applyPlan(s0, plan.actions), '/m/b.mp4'), [[150, 300]], '');
}
...
// ── 3. The interval is full clip: zero cut and whole deletion ──
{
  ...
  assert.deepEqual(plan.actions.map((a) => a.type), ['remove'], '');
  assert.deepEqual(spansOf(applyPlan(s0, plan.actions), '/m/b.mp4'), [], 'camB empty');
}
...
// ── 4. Disjoint: zero action; coverage gap is counted in frames ──
{
  ...
  assert.equal(planCamSwitch([s0.items[0]], [far], 60, 150, makeId).actions.length, 0, 'outer');
  const shortTarget = clip('camA2', 'V1', 0, 100, '/m/a.mp4');
  const plan = planCamSwitch([shortTarget], [s0.items[1]], 60, 200, makeId);
  assert.equal(plan.coverageGapFrames, 100, 'target 40 frame, 100 frame');
...
// ── 5. Tool layer: single batch submission + geometric consistency; multiple segments from the same source are considered targets ──
{
  ...
  assert.equal(batches, 1, 'batch(undo)');
  assert.deepEqual(spansOf(s, '/m/b.mp4'), [[0, 60], [150, 300]], 'tool');
  assert.equal((r.removedSegments as unknown[]).length, 2, '');
  ...
  assert.deepEqual(spansOf(s, '/m/b.mp4'), [[0, 60], [150, 180], [240, 300]], '[180,240)');
  assert.deepEqual(spansOf(s, '/m/a.mp4'), [[0, 300]], 'targetcomplete');
}
...
// ── 6. Boundary check: each error path ──
{
  ...
  assert.match(err({ itemIds: ['camA'], targetItemId: 'camA', fromSeconds: 0 }), /at least 2/, '2');
  assert.match(err({ itemIds: ['camA', 'nope'], targetItemId: 'camA', fromSeconds: 0 }), /not found/, 'id');
  assert.match(err({ itemIds: ['camA', 'mix'], targetItemId: 'camA', fromSeconds: 0 }), /must be video/, 'video');
  assert.match(err({ itemIds: ['camA', 'camB'], targetItemId: 'camC', fromSeconds: 0 }), /must be one of/, 'targetinner');
  assert.match(err({ itemIds: ['camA', 'camB'], targetItemId: 'camA', fromSeconds: -1 }), /fromSeconds/, 'dot');
  assert.match(err({ itemIds: ['camA', 'camB'], targetItemId: 'camA', fromSeconds: 5, toSeconds: 5 }), /empty switch range/, 'empty');
  ...
  assert.match((execChangeCam({ itemIds: ['camA', 'camB'], targetItemId: 'camA', fromSeconds: 5, toSeconds: 8 }, ctx2) as { error?: string }).error ?? '', /no clip in the switch range/, 'target');
  ...
  assert.match((execChangeCam({ itemIds: ['camA', 'camB'], targetItemId: 'camA', fromSeconds: 0, toSeconds: 2 }, ctx3) as { error?: string }).error ?? '', /locked/, 'lock');
}
...
// ── 7. Multiple segments from the same source: The remaining segments of the target camera will not be deleted as "other cameras" and will be included in the coverage together ──
{
  ...
  assert.deepEqual(spansOf(s, '/m/a.mp4'), [[0, 100], [100, 300]], 'targetsourceall');
  assert.deepEqual(spansOf(s, '/m/b.mp4'), [[0, 60], [180, 300]], 'camB [60,180)');
  assert.equal(r.coverageGapSeconds, 0, 'none');
}
