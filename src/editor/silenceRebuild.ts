// remove_silences edit planner: translate source audio silent interval into a sequence of split/remove operations
...
export function spansToLocalCuts(
 ...
 // Swallowed whole = The detection layer said its all dead energy but passed the voice reference gate - it shouldnt happen, give up conservatively
...
export function silenceRemovalBlocker(item: TimelineItem): string | null {
 if (item.kind !== 'video' && item.kind !== 'audio') return `kind=${item.kind} `;
 if (!item.src) return '';
 if ((item.playbackRate ?? 1) !== 1) return '(playbackRate≠1) , clean_script';
 if (item.zoom) return '(zoom) , zoom';
 if (hasOperationalTranscript(item) && (item.deletedWordIdx?.length || item.silenceFrames !== undefined || item.gapCapsMs)) {
 return '/ clip clean_script ()';
 }
 return null;
}
