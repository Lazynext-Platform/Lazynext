...
assert.deepEqual(cuts1, [{ fromFrame: 60, toFrame: 90 }, { fromFrame: 234, toFrame: 240 }], 'clamp + srcIn ');
...
assert.deepEqual(cuts2, [{ fromFrame: 30, toFrame: 90 }], '3 ');

// The entire line is quiet: conservative and giving up
assert.deepEqual(spansToLocalCuts(clip({}), [{ startMs: 0, endMs: 10_000 }], FPS), [], 'ok');
...
// ── Planning + true reducer: dead energy in the middle, subsequent clip ripples on the same track shift left ──
{
 ...
 assert.equal(v1.length, 3, ' clip + clip');
 const [a, b, c] = v1;
 assert.deepEqual([a!.startFrame, a!.durationInFrames, a!.srcInFrame ?? 0], [0, 100, 0], ' 1');
 assert.deepEqual([b!.startFrame, b!.durationInFrames, b!.srcInFrame], [100, 140, 160], ' 2 ');
 assert.deepEqual([c!.id, c!.startFrame], ['next', 240], ' clip 60 ');
}
...
// ── The death energy at the beginning + the death energy at the end (the original id is deleted at the beginning, and the whole paragraph at the end is removed) ──
{
 ...
 assert.equal(out.items.length, 1, 'ok');
 const only = out.items[0]!;
 assert.deepEqual([only.startFrame, only.durationInFrames, only.srcInFrame], [0, 155, 45], ',srcIn=45');
}
...
// ── Multiple sections of dead energy: three reserved sections, srcIn jumps section by section, and the last section is aligned ──
{
 ...
 assert.equal(total, 300 - plan.removedFrames, 'ok');
}
...
// ── Fade in and out to the outer edge: keep fadeIn in the first section, fadeOut in the last section, and no fade in the cut ──
{
 ...
 const segs = out.items.sort((a, b) => a.startFrame - b.startFrame);
 assert.equal(segs[0]!.fadeInFrames, 12, ' fadeIn');
 assert.equal(segs[0]!.fadeOutFrames, undefined, 'ok');
 assert.equal(segs[1]!.fadeInFrames, undefined, 'ok');
 assert.equal(segs[1]!.fadeOutFrames, 15, ' fadeOut');
}
...
assert.match(
 ...
 /clean_script/, ' clip clean_script',
);
assert.equal(silenceRemovalBlocker(clip({ transcript: [{ text: 'hi', start: 0, end: 300 }] })), null, ' clip ');
assert.equal(silenceRemovalBlocker(clip({})), null, 'ok');

console.log('silenceRebuild.verify: ok (// reducer )');
