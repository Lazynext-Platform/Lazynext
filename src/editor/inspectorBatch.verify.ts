...
assert.deepEqual(inspectorMixedValue([a, b], (entry) => entry.volume ?? 1), { mixed: false, value: 1 }, 'undefined defaultvaluedefaultvalue mixed');
...
assert.deepEqual(transformed.items.map((entry) => entry.transform?.scale), [1, 2], 'mixed scale must not');
assert.deepEqual(transformed.items.map((entry) => entry.filters?.brightness), [0.8, 1.2], 'mixed all patch');
...
assert.equal(committed.past.length, 1, 'edit undo step');
...
assert.strictEqual(afterRejected, lockedState, 'failmustrollback snapshot，must notpublishfirst item');
...
console.log('inspectorBatch.verify: ok (mixed/default/ patch/atomic batch/locked+exception rollback)');
