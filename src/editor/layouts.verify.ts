...
for (const layout of LAYOUT_IDS) {
  ...
  for (const [name, slot] of Object.entries(slots)) {
    ...
    assert.ok(rectNear(visibleRectOf(t), slot), `${layout}/${name} cover rectangleequals`);
    assert.ok((t.scale ?? 0) > 0, `${layout}/${name} scale > 0`);
    const c = t.crop;
    if (c) {
      for (const v of [c.left, c.top, c.right, c.bottom]) assert.ok((v ?? 0) >= 0 && (v ?? 0) < 1, 'scoreinner');
      assert.ok((c.left ?? 0) + (c.right ?? 0) < 1 && (c.top ?? 0) + (c.bottom ?? 0) < 1, '');
    }
  }
}
...
assert.ok(near(left.scale!, 1) && near(left.x!, -25) && near(left.y!, 0), 'left scale1/x-25');
assert.ok(near(left.crop!.left!, 0.25) && near(left.crop!.right!, 0.25), 'leftleftright 1/4');
const tl = placeInSlot(layoutSlots('grid-4')['top-left']!, 'cover');
assert.equal(tl.crop, undefined, '(grid )none');
assert.ok(near(tl.scale!, 0.5) && near(tl.x!, -25) && near(tl.y!, -25), 'grid');
...
assert.ok(near(keepLeft.crop!.left!, 0) && near(keepLeft.crop!.right!, 0.5), 'anchorX=0 right');
assert.ok(rectNear(visibleRectOf(keepLeft), layoutSlots('2up-horizontal').left!), 'anchor alignment');
const keepBottom = placeInSlot(layoutSlots('2up-vertical').top!, 'cover', 0.5, 1);
assert.ok(near(keepBottom.crop!.top!, 0.5) && near(keepBottom.crop!.bottom!, 0), 'anchorY=1 top');
...
assert.equal(fitLeft.crop, undefined, 'fit');
assert.ok(near(fitLeft.scale!, 0.5), 'fit min(slot.w, slot.h)');
const fitBox = visibleRectOf(fitLeft);
const slotL = layoutSlots('2up-horizontal').left!;
assert.ok(fitBox.x >= slotL.x - 1e-6 && fitBox.x + fitBox.w <= slotL.x + slotL.w + 1e-6, 'fit');
assert.ok(near(fitBox.x + fitBox.w / 2, slotL.x + slotL.w / 2), 'fit centered');

// ── full = reset:scale1 / no displacement / no cropping ──
const full = placeInSlot(layoutSlots('full').full!, 'cover');
assert.ok(near(full.scale!, 1) && near(full.x!, 0) && near(full.y!, 0) && full.crop === undefined && full.rotation === 0, 'full');
...
assert.ok(rectNear(pip.inset!, { x: 0.05, y: 0.05, w: 0.25, h: 0.25 }), 'pip lefttopsmall');
const clamped = layoutSlots('pip', { size: 5, margin: -1 });
assert.ok(near(clamped.inset!.w, 0.6) && near(clamped.inset!.x, 0.4), 'pip param');

// ──Illegal slot rejection──
assert.throws(() => placeInSlot({ x: 0.8, y: 0, w: 0.5, h: 1 }), /invalid slot/, '');
assert.throws(() => placeInSlot({ x: 0, y: 0, w: 0, h: 1 }), /invalid slot/, 'wide');
...
assert.equal(recorded.actions.length, 2, 'batch setTransform');
assert.equal(recorded.actions[0]!.type, 'setTransform');
assert.ok(near(ok.applied[0]!.transform.x!, -25), 'left x=-25');
assert.equal(ok.notes, undefined, 'normal → none');

const bad = execLayoutTool('apply_layout', { layout: 'grid-4', assignments: [{ slot: 'middle', itemId: 'a' }] }, ctx) as { error?: string };
assert.match(bad.error ?? '', /unknown slot/, '');
...
assert.ok(warn.notes?.some((n) => n.includes('BEHIND')), 'inset low main →');
const never = execLayoutTool('apply_layout', {
  layout: '2up-horizontal',
  assignments: [{ slot: 'left', itemId: 'a' }, { slot: 'right', itemId: 'late' }],
}, ctx) as { notes?: string[] };
assert.ok(never.notes?.some((n) => n.includes('overlap in time')), '→');

console.log('layouts.verify: ok (cover /anchor/fit/pip//toolvalidate)');
