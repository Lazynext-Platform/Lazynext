// Right column of the settings panel: Select the providers configuration page (header + field card + test connection row) and field rendering.
...
function LocalModelsPane({ page, fields, ctx }: {
 ...
 const t = useT();
 const title = page.key === 'local/asr' ? 'Local transcription' : page.key === 'local/music/packs' ? 'Music analysis' : 'Semantic search';
 return (
 <div style={pane}>
 <div>
 <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
...
export function FieldRow({ field, ctx }: { field: SettingsField; ctx: FieldCtx }) {
 ...
 // Select uses the default option to clear; toggles off/on itself is set/clear.
...
function DirectoryInput({ field, shown, stagedClear, onStage }: {
 ...
 const picker = window.lazynextDesktop?.selectDirectory;
 ...
 {!picker && <span style={fieldHint}>{t('Folder picker is available in the desktop app only')}</span>}
