...
// Whisper is OpenAIs open-source model, so the official OpenAI mark is used.
...
export function LocalAsrPane({ fields, ctx }: { fields: readonly SettingsField[]; ctx: FieldCtx }) {
 ...
 const hasDesktopInference = Boolean(window.lazynextDesktop?.inference);
 ...
 useEffect(() => {
 let active = true;
 const inference = window.lazynextDesktop?.inference;
 if (inference) {
 ...
 {t(' Windows DirectML macOS CoreML/ CPUTranscription model')}
 </span>
 </span>
 </label>
 )}
