...
// Language toggle removed — Lazynext is English-only.
...
export function TopBar({ projectId, projectName, canUndo, canRedo, exporting, exportJobCount = 0, onHome, onRename, onResumeGeneration }: TopBarProps) {
  const t = useT();
  const isMacDesktop = window.lazynextDesktop?.platform === 'darwin';
  ...
  return (
    <header className={`ln-topbar ln-window-titlebar${isMacDesktop ? ' ln-window-titlebar--mac' : ''}`} style={{ gridColumn: '1 / -1', gridRow: 1, position: 'relative', height: '100%', display: 'flex', alignItems: 'center', padding: '0 6px', borderBottom: `0.5px solid ${theme.border}`, background: theme.panel, gap: 4 }}>
      <DesktopWindowControls />
      {/* home in a rounded chip + a vertical divider */}
      <button className="ln-tip" data-tip={t('Back to projects')} aria-label={t('Back to projects')} onClick={onHome}
      ...
      <div className="ln-topbar-title" style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)', display: 'flex', gap: 8, justifyContent: 'center', alignItems: 'center', fontSize: 12, color: theme.text }}>
        {editing ? (
      ...
      <span id="ln-agent-change-log-slot" style={{ display: 'contents' }} />
