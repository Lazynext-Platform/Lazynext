...
export function SliderRow({
  ...
  return (
    <div className="ln-insp-row" title={disabled ? disabledReason : undefined} style={disabled ? { opacity: 0.45 } : undefined}>
      <span className="ln-insp-label">{label}</span>
      <input
        aria-label={label}
        className="ln-insp-range"
        style={{ '--ln-insp-range-fill': `${progress}%` } as CSSProperties}
...
function KfCell({ kfs, localFrame, inRange, punchValue, onSet, onRemove, onSeekLocal }: {
  ...
  return (
    <span className="ln-insp-kf">
      <button className="ln-insp-kf-btn" type="button" disabled={!prev} title={previousTitle} aria-label={previousTitle} onClick={() => prev && onSeekLocal(prev.frame)}>
        <Icon name="prev" size={10} />
      </button>
      <button
        className={`ln-insp-kf-btn key${at ? ' active' : kfs?.length ? ' keyed' : ''}`}
        type="button"
        disabled={!inRange}
      ...
      <button className="ln-insp-kf-btn" type="button" disabled={!next} title={nextTitle} aria-label={nextTitle} onClick={() => next && onSeekLocal(next.frame)}>
        <Icon name="next" size={10} />
      </button>
      <button className="ln-insp-kf-btn danger" type="button" disabled={!at} title={t('Delete the keyframe at the playhead')} aria-label={t('Delete the keyframe at the playhead')} onClick={() => at && onRemove(localFrame)}>
        <Icon name="trash" size={10} />
      </button>
      {at && (
        <select
          className="ln-insp-kf-easing"
  ...
  return (
    <div className="ln-insp-stack">
      {rows.map((r) => {
        ...
        return (
          <div key={r.id} className="ln-insp-control-line">
