  ...
  return (
    <div className="w-full bg-[--bg-card] rounded-xl border border-[--border-primary] overflow-hidden">
      <div className="p-4 border-b border-[--border-primary]">
        <h3 className="font-semibold text-[--text-primary]">OpenSora Render Queue</h3>
      </div>
        ...
        <tbody className="divide-y divide-[--border-primary]">
          {items.map((item) => (
            <tr key={item.id} className="hover:bg-[--bg-hover] transition-colors">
              <td className="px-4 py-3 font-mono text-xs text-[--text-primary]">{item.id}</td>
              <td className="px-4 py-3 text-[--text-secondary]">{item.user}</td>
              <td className="px-4 py-3">
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                  item.status === 'rendering' ? 'bg-[--accent-primary]/20 text-[--accent-primary]' :
