...
// radiusmustassetactualvisiblerectanglefullproject
// 1:1 contain 16:9 assetvisiblerectangle 1080×607.5
// 405px needsrectangleshort 303.75px
{
 ...
 assert.equal(typeof geometryApi.visibleVisualFrameRect, 'function', 'should provideassetvisiblerectangle');
 assert.equal(typeof geometryApi.clampVisualBorderRadius, 'function', 'assetshortconstraintradius');
...
// A wrong paint-order mirror would select the background instead of the card.
{
 ...
 ['late', 'early', 'v1'],
 'mustcompositetopbackstartcliptop',
 );
}
...
// Edge drag crops (covers) — drag right edge from full frame to x=960 on 1920 canvas → right half hidden.
{
 ...
 assert.ok(half.crop, ' crop');
 assert.ok(Math.abs((half.crop?.right ?? 0) - 0.5) < 1e-6, ' right≈0.5');
 assert.equal(half.crop?.left ?? 0, 0, ' inset ');
 ...
 assert.ok(Math.abs((leftIn.crop?.left ?? 0) - 0.25) < 1e-6, ' left≈0.25');
 assert.equal(leftIn.crop?.right ?? 0, 0, ' inset ');
}
...
assert.equal(typeof keyboardPreviewNudgePlan, 'function', '');
...
assert.match(editorActionsSource, /keyboardPreviewNudgePlan/, '');
assert.match(
 editorActionsSource,
 /previewCanvasHasKeyboardFocus/,
 '',
);
assert.match(editorActionsSource, /setItemTransform/, ' Inspector transform');
assert.match(editorActionsSource, /setItemKeyframe/, '');
const overlaySource = readFileSync(new URL('./PreviewTransformOverlay.tsx', import.meta.url), 'utf8');
assert.match(overlaySource, /tabIndex=\{0\}/, '');
assert.match(overlaySource, /event\.currentTarget\.focus\(/, '');

console.log('previewTransform.verify: ok (////////)');
