...
export function BrandMark({ size = 16 }: { size?: number }) {
  return (
    <img src="/lazynext-icon.png" alt="" aria-hidden width={size} height={size} style={{ display: 'block' }} />
  );
}

/** Lazynext word mark: clean text logo with accent badge. */
export function LazynextWordmark({ width = 126 }: { width?: number }) {
  return (
    <svg
      aria-label="Lazynext"
      role="img"
    ...
    >
      <rect x="0" y="13" width="92" height="92" rx="18" fill="currentColor" />
      <text
