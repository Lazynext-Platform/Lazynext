...
function ContextSection({ run, t }: { run: AgentRunRecord; t: Translate }) {
  ...
  // Cache write only matters for providers that support explicit cache writes (e.g. Anthropic);
  // DeepSeek and others only have server-side prompt caching and never report cache writes.
  // Hide when there is no value to avoid showing a meaningless dash.
  ...
  return <Section title={t('Context and tools')} hint={t('Token usage, cache, and tool surface info for this run and the latest model request')}>
    ...
    <div style={metrics}>
      <Metric label={t('Input')} title={t('Input tokens: input volume of the latest model request (prompt + tool outcomes)')} value={numberText(contextMetric(context, 'inputTokens'))} />
      <Metric label={t('Output')} title={t('Output tokens: text volume returned by the latest model request')} value={numberText(contextMetric(context, 'outputTokens'))} />
    ...
    <div style={metrics}>
      <Metric label={t('Cache read')} title={t('Cache read: number of input tokens served from cache this request; more hits = more savings')} value={numberText(contextMetric(context, 'cacheReadTokens'))} />
      {cacheWriteTokens !== undefined && (
        <Metric label={t('Cache write')} title={t('Cache write: number of tokens written to cache this request (only some models support this)')} value={cacheWriteTokens} />
      )}
      <Metric label={t('Uncached')} title={t('Uncached: input tokens not served from cache, requiring recomputation this request')} value={numberText(contextMetric(context, 'noCacheTokens'))} />
      <Metric label={t('Hit rate')} title={t('Cache hit ratio across run totals')} value={percentText(contextMetric(context, 'cacheHitRatio'))} />
    ...
    <div style={metrics}>
      ...
      <Metric label={t('Schema chars')} title={t('Character count of all tool schemas, measuring the tool surface size')} value={numberText(contextMetric(context, 'toolSchemaChars'))} />
    </div>
    <div style={subheadInSection}>{t('Run totals')}</div>
    <div style={metrics}>
      <Metric label={t('Model requests')} title={t('Number of model requests initiated in run totals')} value={numberText(contextMetric(context, 'modelRequestCount'))} />
      ...
      <Metric label={t('Image input')} title={t('Number of images sent to the model in this run.')} value={numberText(contextMetric(context, 'totalMediaInputs'))} />
    </div>
  </Section>;
}
...
function CheckpointSection({ checkpoint, t }: { checkpoint?: AgentCheckpointRecord; t: Translate }) {
  ...
  return <Section title={t('Context checkpoint')} hint={t('Summary checkpoint after a long session was compressed, for tracing how context was trimmed')}>
...
function InspectorContent({ sidecar, loading, failed, t }: {
  ...
  // latest run's details, but "how many conversation turns / model requests" is a project-wide figure.
  ...
  return <>
    <div style={runSummary}>
      ...
      <div style={subtle}>{t('All conversations: {runs} turns · {requests} model requests total', { runs: numberText(runCount) ?? '0', requests: numberText(totalModelRequests) ?? '0' })}</div>
