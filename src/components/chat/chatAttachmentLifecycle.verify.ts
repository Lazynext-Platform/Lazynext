...
assert.equal(
  referencesAfterComposerTextEdit(chipOnlyReferences, '', '，input'),
  chipOnlyReferences,
  'typing after selecting a chip-only reference must preserve the reference',
);
const inlineToken = refPromptToken(existing);
assert.deepEqual(
  referencesAfterComposerTextEdit(chipOnlyReferences, `${inlineToken}`, ''),
  [],
  'deleting a previously embedded prompt token must still remove its reference',
);
assert.deepEqual(
  referencesAfterComposerTextEdit(chipOnlyReferences, `${inlineToken}`, `${inlineToken} new`),
  chipOnlyReferences,
