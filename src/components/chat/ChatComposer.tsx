...
export function ChatComposer(props: ChatComposerProps) {
 ...
 return [
 { key: 'assets', icon: 'filePlay', label: t('Reference media-pool assets\', \'\': \'No assets in the media pool yet'), sub: `${assets.length}`, action: go('assets') },
 ...
 if (refDrill === 'assets') return { title: t('Reference media-pool assets\', \'\': \'No assets in the media pool yet'), onBack: () => { setRefDrill('root'); setRefIndex(0); } };
 ...
 {t('Creative mode: {name}', { name: skillName(activeSkill) })}
 </span>
 <button
 type="button"
 title={t('Exit creative mode')}
 ...
 // matches (e.g. a typoed skill name).
 event.preventDefault();
 ...
 {modeRow('ask', t('Q&A mode'), t('Answers only never touches the timeline'))}
 </ComposerPopover>
 )}
 {pop === 'model' && (
 ...
 <strong style={{ fontSize: 12.5 }}>{slashExplicit ? t('Skill command') : t('Creative workflows')}</strong>
 ...
 {slashExplicit
 ? t('Unknown skill “{query}” type / to see all creative workflows', { query: slashMatchQuery.trim() })
 : t('No creative workflow matches “{query}”', { query: slashMatchQuery.trim() })}
 </div>
 )}
