...
export function TimelineTabs({ doc, commands }: TimelineTabsProps) {
 ...
 <button onClick={() => commands.duplicateTimeline(doc.activeTimelineId)} title={t('Duplicate current sequence')} style={{ ...tabBtn, display: 'inline-flex', alignItems: 'center', gap: 4 }}><Icon name="copy" size={13} />{t('Duplicate')}</button>
 <button
 onClick={() => commands.duplicateTimeline(doc.activeTimelineId, { retarget: { width: 1080, height: 1920, fit: 'cover' }, name: '' })}
