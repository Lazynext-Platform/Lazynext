  ...
  return (
    <>
      ...
      <div className="cc-timeline-tool-group">
        ...
        <TB
          icon="swap"
          title={t('Slip mode (U): keep timeline position and duration, only drag the source range')}
        ...
        <TB icon="pencil" title={t('Pen mode (P): click on a selected clip to draw keyframes (visual clip = opacity, audio clip = volume; vertical = value, drag points to change frame/value, right-click to delete points)')} active={editMode === 'pen'} onClick={() => invokeAction('interaction-mode-pen', undefined, 'toolbar')} />
      ...
      <button className={`cc-caption-toggle cc-tip cc-tip-r${captionsVisible ? ' active' : ''}`} data-tip={hasCaptions
        ...
        : t('Captions (no captions or text clips yet; transcribe or have the Agent generate them)')} aria-label={t('Captions')} disabled={!hasCaptions} onClick={() => commands.setCaptionsHidden(captionsVisible)}><Icon name="captions" size={17} /><span>{captionsVisible ? t('On') : t('Off')}</span><Icon name="chevronDown" size={13} /></button>
