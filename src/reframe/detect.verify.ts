// function headless :`npx tsx src/reframe/detect.check.ts`。
// none DOM (video/):focalFromEnergyGrid / magnificationForAspect /
// energyGridFromImageData / sampleFrames,validatekeyframe ReframeKeyframe value。
...
// —— focalFromEnergyGrid:compositehigh → center ——
// 3 row 4 column,center (row=2, col=1) → x≈(1+0.5)/4=0.375, y≈(2+0.5)/3≈0.833
const grid: number[][] = [
  [0, 0, 0, 0],
  [0, 0, 0, 0],
  [0, 9, 0, 0],
...
// —— magnificationForAspect:16:9→9:16 large ~3.16 full; = 1;clamp ——
...
// —— energyGridFromImageData:high → high, ——
const W = 8;
const H = 4;
...
// —— sampleFrames:、、 maxSamples constraint ——
...
// —— smoothFocalPath: EMA 、high smooth topframe ——
