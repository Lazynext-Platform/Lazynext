...
const updated = updateManualCue(captions, laneId, 0, 'caption', 1_200, 2_400);
...
assert.equal(captionPages(captions, [], 30)[0]?.words[0]?.text, 'caption');
assert.match(captionsToSrt(captions, [], 30), /00:00:01,200 --> 00:00:02,400\n/);

captions = { ...captions, ...appendManualCue(captions, laneId, 'bottom', 3_000, 4_000) };
...
destination = { ...destination, ...appendManualCueToFirstLane(destination, [], 'caption', 1_000, 3_000) };
destination = { ...destination, ...appendManualCueToFirstLane(destination, [], 'allow', 2_000, 4_000) };
assert.deepEqual(destination.sourceEntries![0]!.words?.map((word) => word.text), ['caption', 'allow']);

const dropped = appendDroppedManualCaption(captions, [], 'tiktok', 'caption', 5_000, {
...
assert.equal(droppedEntry.words?.[0]?.text, 'caption');
...
assert.equal(placeManualCueTiming(occupied, 1_050, 1_000), null, '1000ms cue 200ms → ()');
assert.deepEqual(placeManualCueTiming(occupied, 1_050, 150), { start: 1_050, end: 1_200 }, 'bottomrequest,right');
assert.deepEqual(placeManualCueTiming(occupied, 950, 200), { start: 1_000, end: 1_200 }, 'front → full');
assert.deepEqual(placeManualCueTiming(occupied, 5_000, 700), { start: 5_000, end: 5_700 }, '');
assert.deepEqual(placeManualCueTiming([], -500, 800), { start: 0, end: 800 }, 'empty lane:dot 0,duration');
