...
export function ManualCaptionEditor({ captions, items, onUpdate, getPlayheadMs, onSeekMs }: Props) {
  ...
  return (
    <div className="ln-cap-bilingual">
      <button type="button" className="ln-cap-bilingual-toggle" onClick={() => setOpen((value) => !value)} aria-expanded={open}>
...
function ManualLanePanel(props: LanePanelProps) {
  ...
  return (
    <div className="ln-cap-bilingual-body">
      <div className="ln-cap-manual-toolbar">
        {lanes.length > 0 && <select className="ln-cap-select" value={laneId} onChange={(event) => setLaneId(event.target.value)}>
          {lanes.map((entry) => <option key={entry.id} value={entry.id}>{entry.label ?? t('Manual caption lane')}</option>)}
        </select>}
        <button type="button" className="ln-cap-btn sm" onClick={addLane}>{t('New lane')}</button>
        {lane && <button type="button" className="ln-cap-btn sm ghost" onClick={() => onUpdate(removeManualLane(captions, lane.id))}>{t('Delete lane')}</button>}
      </div>
      {!lane && <p className="ln-cap-hint">{t('Create a manual caption lane, then add captions at the playhead without transcription or AI.')}</p>}
...
function ManualCueList({ captions, lane, onUpdate, getPlayheadMs, onSeekMs }: Omit<Props, 'items'> & { lane: CaptionSourceEntry }) {
  ...
  return (
    <div className="ln-cap-manual-compose">
      <textarea className="ln-cap-input ln-cap-textarea" rows={2} value={text} onChange={(event) => setText(event.target.value)} placeholder={t('Enter caption text')} />
      <div className="ln-cap-manual-controls">
        <label className="ln-cap-time-field"><span>{t('Start')}</span><input className="ln-cap-input" type="number" min="0" step="0.1" value={start} onChange={(event) => setStart(event.target.value)} /><span>s</span></label>
        <label className="ln-cap-time-field"><span>{t('Duration')}</span><input className="ln-cap-input" type="number" min="0.1" step="0.1" value={duration} onChange={(event) => setDuration(event.target.value)} /><span>s</span></label>
        <button type="button" className="ln-cap-btn sm" onClick={syncPlayhead}>{t('Use playhead')}</button>
        <button type="button" className="ln-cap-btn primary sm" disabled={!text.trim()} onClick={add}>{t('Add caption')}</button>
      </div>
      <div className="ln-cap-manual-list">
...
function ManualCueRow({ captions, lane, cue, index, onUpdate, onSeekMs }: Omit<Props, 'items' | 'getPlayheadMs'> & { lane: CaptionSourceEntry; cue: TranscriptWord; index: number }) {
  ...
  return (
    <div className="ln-cap-manual-row">
      <input className="ln-cap-input" aria-label={t('Start time in seconds')} type="number" min="0" step="0.1" value={start} onChange={(event) => setStart(event.target.value)} onClick={() => onSeekMs?.(cue.start)} />
      <input className="ln-cap-input" aria-label={t('End time in seconds')} type="number" min="0.1" step="0.1" value={end} onChange={(event) => setEnd(event.target.value)} />
      <input className="ln-cap-input ln-cap-manual-text" aria-label={t('Caption text')} value={text} onChange={(event) => setText(event.target.value)} />
      <div className="ln-cap-manual-actions">
        <button type="button" className="ln-cap-btn sm" onClick={save}>{t('Save')}</button>
        <button type="button" className="ln-cap-btn sm ghost" onClick={() => onUpdate(removeManualCue(captions, lane.id, index))}>{t('Delete')}</button>
      </div>
    </div>
  );
}
