...
/** Modal particles for CJK caption segmentation (empty for English-only product). */
export const MODAL_PARTICLES = [] as const;

/** Structural particle/adhesion word list for CJK orphan-word avoidance (empty for English-only product). */
export const CJK_PARTICLES = [] as const;

/** CJK particles that cannot start a line (empty for English-only product). */
export const NO_LINE_START = [] as const;
...
/** CJK word suffixes for segmentation (empty for English-only product). */
export const CJK_WORD_SUFFIXES = [] as const;

/** CJK interrogative sentence patterns (disabled for English-only product). */
export const QUESTION_TAIL = /$/u;
export const QUESTION_TAIL_EXCLUDE = /$/u;
export const QUESTION_HEAD = /^$/u;
