...
assert.equal(matches[0]?.sampleTime, 0, 'bottomassetframe');
...
assert.deepEqual(weak.map((item) => item.assetId), ['city'], 'center');
...
assert.equal(orthogonal.length, 3, 'assetframe');
