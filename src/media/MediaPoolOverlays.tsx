...
export function BlankMediaMenuActions(props: BlankMediaMenuActionsProps) {
  ...
  return <>
    ...
    <label><span>{t('Filter')}</span><select aria-label={t('Filter media')} value={props.type} onChange={(event) => props.onType(event.target.value as MediaTypeFilter)}>
      <option value="all">{t('All')}</option><option value="video">{t('Video')}</option><option value="image">{t('Image')}</option><option value="audio">{t('Audio')}</option>
    </select></label>
  </>;
}
...
function AssetMenuActions(props: AssetMenuPortalProps & { asset: MediaAsset }) {
  ...
  return (
    <>
      ...
      {props.onTranscribe && <button type="button" onClick={props.onTranscribe}>{asset.transcribeStatus === 'failed' ? t('Re-transcribe') : t('Transcript')}</button>}
