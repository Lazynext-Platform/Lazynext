...
'prompt' | 'promptValue' | 'onPromptValue' | 'onSubmitPrompt' | 'onClosePrompt'>) {
  ...
  return <div className="ln-modal-backdrop" role="dialog" aria-modal="true" aria-label={t(props.prompt.title)}>
    <form className="ln-modal" onSubmit={submit}>
...
'folderDelete' | 'onDeleteFolder' | 'onCloseFolderDelete'>) {
  ...
  return <div className="ln-modal-backdrop" role="dialog" aria-modal="true" aria-label={t('Delete empty folder')}>
...
'assetDelete' | 'assetDeleteTitle' | 'onDeleteAssets' | 'onCloseAssetDelete'>) {
  ...
  return <div className="ln-modal-backdrop" role="dialog" aria-modal="true" aria-label={t('Delete In-Use Media')} onClick={props.onCloseAssetDelete}>
    <div className="ln-modal" onClick={stop}>
      <strong>{props.assetDeleteTitle}</strong>
      <p className="ln-asset-delete-detail">{detail}</p>
      <p className="ln-asset-delete-detail" title={state.names.join('\n')}>{state.names.join('、')}</p>
