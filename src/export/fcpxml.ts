...
function backgroundFillMetadataXml(item: TimelineItem): string {
  return `<metadata>
          <md key="com.lazynext.backgroundFill" value="1" editable="1" type="boolean"/>
          <md key="com.lazynext.backgroundFillStrength" value="${backgroundFillStrengthOf(item)}" editable="1" type="integer"/>
        </metadata>`;
}
 ...
 * Lazynext timeline (independent absolute frame bits for each track) to FCPX magnetic timeline (connected clips with lane)
...
export function timelineToFcpxml(
  ...
  const title = escapeXml((opts.title ?? '').trim() || 'Lazynext Timeline');
  ...
  const backgroundFillWarning = fcpxmlBackgroundFillCount(state) > 0
    ? xmlComment('WARNING: backgroundFill settings are preserved as Lazynext metadata, but this exporter does not synthesize a portable blurred layer; render a video master to preserve the exact appearance.')
    : '';
  ...
  const eventName = nle === 'fcp_xml_resolve' ? 'Lazynext Export (Resolve)' : 'Lazynext Export';
