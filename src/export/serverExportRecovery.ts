...
export async function persistServerExportJob(record: PersistedServerExportJob): Promise<void> {
 ...
 if (!canonical) throw new RetiredExportRecoveryError('');
 remoteSaved = true;
 ...
 throw localError instanceof Error ? localError : new Error('');
 }
}
...
async function transitionServerExportStage(
 ...
 if (!await updateLocalStage(renderId, stage, claim)) throw new Error('');
}
...
export async function rebindServerExportJob(
 ...
 if (!await checkServerExportDelivery(renderId, claim)) throw new Error('');
