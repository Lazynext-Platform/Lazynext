...
const DATABASE_NAME = 'lazynext-export-destinations';
...
function openDestinationDatabase(): Promise<IDBDatabase> {
  ...
  request.onerror = () => reject(request.error ?? new Error('Unable to open export destination storage'));
  request.onblocked = () => reject(new Error('Export destination storage is blocked by another tab'));
  return promise;
}
...
function transactionComplete(transaction: IDBTransaction): Promise<void> {
  ...
  transaction.onerror = () => reject(transaction.error ?? new Error('Unable to save export destination'));
  transaction.onabort = () => reject(transaction.error ?? new Error('Export destination save cancelled'));
  return promise;
}
...
async function readBrowserDirectory(): Promise<BrowserExportDirectoryHandle | null> {
  ...
  try {
    ...
    request.onerror = () => reject(request.error ?? new Error('Unable to read export destination'));
...
export async function writeUrlToDestination(
  ...
  if (!response.ok) {
    ...
    throw new ExportFailureError(createExportFailure({
      ...
      targetPath,
      message: `Failed to read export file (HTTP ${response.status})`,
    }));
  }
  try {
    signal?.throwIfAborted();
