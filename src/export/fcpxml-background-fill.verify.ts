...
assert.match(xml, /backgroundFill settings are preserved as Lazynext metadata/);
assert.match(xml, /key="com\.lazynext\.backgroundFillStrength" value="73"/,
  'the exact percentage survives in portable custom metadata');
assert.equal((xml.match(/key="com\.lazynext\.backgroundFill"/g) ?? []).length, 1,
