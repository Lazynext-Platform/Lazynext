...
const PLANS = [
  ...
  },
  {
    ...
    price: "$19",
    period: "per month",
    features: ["Unlimited projects", "4K export", "Cloud AI (GPT-4o, Claude)", "Priority support", "50GB Cloud storage"],
    cta: "Upgrade to Pro",
    highlight: false,
  },
  {
    ...
    price: "$99",
    period: "per month",
    features: ["Everything in Pro", "8K Lossless export", "Team collaboration", "18 API access", "Dedicated Render Node"],
  ...
  if (isPending) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[var(--bg-main)]">
        <div className="text-[var(--text-muted)] animate-pulse">Loading SaaS Portal...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[var(--bg-main)] text-[var(--text-primary)]">
      ...
      <div className="mx-auto max-w-6xl px-4 py-16">
        ...
        <div className="mb-12 text-center">
          <h1 className="text-4xl md:text-5xl font-black tracking-tight">Simple pricing for <span className="text-[var(--accent-primary)]">heavy renders</span>.</h1>
          <p className="mt-4 text-[var(--text-muted)] text-lg max-w-2xl mx-auto">
            Unlock the power of 18 integrated AI models and dedicated Rust headless compositors.
          </p>
        </div>
        ...
        {org && (
          <div className="mb-12 bg-[var(--bg-panel)] border border-[var(--border-glass)] rounded-2xl p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl">
            <div>
              ...
              <p className="text-[var(--text-muted)] text-sm mt-1">Your usage resets in 14 days.</p>
            </div>
            <div className="flex gap-8">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[var(--text-muted)] text-xs font-semibold uppercase">AI Credits</p>
                  <p className="font-bold text-xl text-[var(--text-primary)]">{org.aiCreditsUsed.toLocaleString()} / 50K</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[var(--text-muted)] text-xs font-semibold uppercase">Render Hours</p>
                  <p className="font-bold text-xl text-[var(--text-primary)]">{org.renderHoursUsed}h / 200h</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-3 items-stretch">
          {PLANS.map((plan) => (
            ...
            >
              ...
              <h3 className={`text-2xl font-bold ${plan.highlight ? "text-[var(--accent-primary)]" : "text-[var(--text-primary)]"}`}>
                {plan.name}
              </h3>
              
              <div className="mt-4 pb-6 border-b border-[var(--border-glass)]">
                <span className="text-5xl font-black">{plan.price}</span>
                <span className="ml-2 text-sm text-[var(--text-muted)]">{plan.period}</span>
              </div>
              
              <ul className="mt-6 flex-1 space-y-4">
                {plan.features.map((f) => (
                  <li key={f} className="flex items-start gap-3 text-[var(--text-secondary)]">
                    <Check className={`w-5 h-5 shrink-0 mt-0.5 ${plan.highlight ? "text-[var(--accent-primary)]" : "text-emerald-400"}`} />
                    <span>{f}</span>
                  </li>
                ))}
              </ul>
              ...
              <button
                ...
                onClick={() => plan.current ? null : toast.info(`Redirecting to Stripe checkout for ${plan.name}...`)}
