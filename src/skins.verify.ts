...
// ── Registry integrity ──
assert.ok(SKINS.length >= 2, 'default + 1');
assert.equal(new Set(SKINS.map((s) => s.id)).size, SKINS.length, 'id');
assert.ok(SKINS.some((s) => s.id === DEFAULT_SKIN), 'defaultmustregistertable');
for (const s of SKINS) {
  assert.ok(/^[a-z]+$/.test(s.id), `${s.id}: id small`);
  assert.ok(s.nameZh.trim().length > 0, `${s.id}: center`);
  assert.match(s.tokens.accentRgb, /^\d{1,3},\d{1,3},\d{1,3}$/, `${s.id}: accentRgb`);
  assert.match(s.tokens.inkRgb, /^\d{1,3},\d{1,3},\d{1,3}$/, `${s.id}: inkRgb`);
  assert.match(s.tokens.shadowRgb, /^\d{1,3},\d{1,3},\d{1,3}$/, `${s.id}: shadowRgb`);
  for (const [name, value] of Object.entries(s.tokens)) {
    if (name === 'accentRgb' || name === 'inkRgb' || name === 'shadowRgb' || name === 'colorScheme') continue;
    assert.match(value, /^#[0-9a-f]{6}$/, `${s.id}.${name}: 6 small hex(contrast)`);
  }
}
