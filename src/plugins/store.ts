...
// Main storage is ~/.lazynext/plugins; old IndexedDB data is only used for one migration and no browser service check fallback.
...
/** Remove a package (fx/lut/transition/zoom) from the runtime registry; dont touch the persistence layer. Applied content can still be rendered based on state snapshots.*/
...
export function lutShaderDefOf(pack: PluginPack, item: PluginLutItem): SerializableFxDef {
 if (!item.frag) throw new Error(`LUT${item.name} frag`);
 return {
 id: pluginAssetId(pack.id, item.id),
 name: item.name,
 desc: item.desc ?? `${pack.name} LUT`,
