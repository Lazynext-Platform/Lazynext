...
export function upstreamUpdateMessage(state: UpstreamUpdateState, desktopUpdate: boolean): string {
  if (state.phase === 'available') {
    ...
    return desktopUpdate
      ? t('Lazynext {latest} is available; current version: {current}. Download and install it directly.', params)
      : t('Lazynext {latest} is available; current version: {current}. Visit the project repository to review the update.', params);
  }
  ...
  if (state.phase === 'downloading') {
    return t('Downloading Lazynext {latest}: {percent}%', {
      latest: formatDisplayVersion(state.latestVersion),
      percent: Math.round(state.percent),
    });
  }
  if (state.phase === 'downloaded') {
    return t('Lazynext {latest} is downloaded. Restart to finish installing.', {
      latest: formatDisplayVersion(state.latestVersion),
    });
  }
  if (state.phase === 'installing') return t('Restarting to install Lazynext…');
