...
function consumeRun(words: { text: string }[], wi: number, run: ParsedRun, line: number): number {
 ...
 if (acc !== target) {
 throw new Error(`timeline.md ${line} : ${run.text.slice(0, 20)}/`);
 }
 return i;
}

// plan of one transcript items word edits
...
function planSegRows(item: TimelineItem, canonRows: SegRow[], parsedRows: ParsedSegRow[]): WordPlan {
 if (!hasOperationalTranscript(item)) throw new Error(`${item.name}Re-transcribe`);
 const words = item.transcript!;
 const currentDeleted = new Set(item.deletedWordIdx ?? []);
 const bySn = new Map<number, ParsedSegRow>();
 for (const r of parsedRows) {
 if (r.occurrence !== undefined) throw new Error(`timeline.md ${r.line} : [s${r.sn}@${r.occurrence}] Unsupported`);
 if (bySn.has(r.sn)) throw new Error(`timeline.md ${r.line} : [s${r.sn}] Unsupported`);
 bySn.set(r.sn, r);
 }
 const knownSns = new Set(canonRows.map((r) => r.sn));
 for (const r of parsedRows) {
 if (!knownSns.has(r.sn)) throw new Error(`timeline.md ${r.line} : [s${r.sn}] Timeline/Unsupported`);
 }
 ...
 throw new Error(`${item.name}: Unsupported [sN] `);
 }
 ...
 // align runs over the segments FULL word list (deleted words included → restore works)
...
export function applyScript(getState: () => TimelineState, commands: Cmds, md: string, options: ApplyScriptOptions = {}): ApplyResult {
 ...
 throw new Error('timeline.md apply_script ');
 }
 ...
 if (!parsed.stamp) throw new Error(' script-stamp read_script ');
 if (parsed.stamp !== stamp) throw new Error('Timelinestale read_script ');
 ...
 if (!parsedTrack) throw new Error(` ## ${canonTrack.track} `);
 ...
 if (transcriptItemBySource.has(it.name)) throw new Error(`${it.name}v1 UnsupportedRename`);
 ...
 if (!canon) throw new Error(`timeline.md ${row.line} : [c${row.cn}] ${region.source} add `);
 if (row.struck) {
 ...
 if (!itemId) throw new Error(`timeline.md ${row.line} : ${region.source}`);
 ...
 removed.push(`${plan.item.name}`);
 } else {
 if (plan.toDelete.length) changes.push(`${plan.item.name}: ${plan.toDelete.length} `);
 if (plan.toRestore.length) changes.push(`${plan.item.name}: ${plan.toRestore.length} `);
 wordPlans.push(plan);
 }
 }
 ...
 changes.push(`${items.get(plan.row.itemId)?.name ?? plan.row.itemId}: `);
 }
 ...
 // swaps never hit the same-track overlap guards intermediate state.
 ...
 changes.push(`${live.name}: ${cursor}f`);
 }
