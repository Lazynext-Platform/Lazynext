...
const missingNames = SOUND_EFFECTS
  ...
  .filter((name) => !EN[name] && !/^[a-zA-Z0-9\s\-_]+$/.test(name));

// English-only: every built-in sound effect must resolve through EN or already be English.
assert.deepEqual(
  missingNames,
  [],
  'every built-in sound effect must have a display name (English or EN-dictionary key)',
);
