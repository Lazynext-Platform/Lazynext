# Lazynext MCP recovery

## Connection refused

Lazynext is closed, the configured URL is stale, or desktop port 5199 fell
back to another port.

1. Start Lazynext.
2. Read the endpoint from **Settings → MCP** or the startup log.
3. Update the single `lazynext` MCP entry.
4. Call `lazynext_status` again.
...
project, call `lazynext_status`, and refresh the MCP tool list.
...
The draft is ready, but manual approval is still pending in Lazynext. Keep
