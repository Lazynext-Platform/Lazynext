...
Use this flow when the user says “set up Lazynext”, when the `lazynext`
...
Start Lazynext first. Open **Settings → MCP** in the trusted editor window,
...
claude mcp add --transport http \
  ...
  lazynext http://localhost:5199/api/external-mcp/mcp
```
...
`X-Lazynext-Tool-Exposure: progressive`. The initial list keeps connection,
...
Lazynext generates it in private server memory and shows it only through the
...
`LAZYNEXT_MCP_TOKEN` overrides the generated token when a stable deployment
